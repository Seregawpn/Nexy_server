...
from experimental.assistant_runtime_v2 import AssistantRuntimeV2
...
class RequestContext:
    ...
    early_text_preview: str = ""
    early_text_preview_emitted: bool = False
...
class StreamingWorkflowIntegration:
    ...
    def __init__(
        ...
        self._runtime_v2 = AssistantRuntimeV2()
    ...
    async def _process_text_for_tts(self, text_chunk: str, ctx: RequestContext) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        async for event in self._maybe_emit_early_text_preview(ctx, complete_sentences):
            yield event
        ...
        for complete in complete_sentences:
            ...
            if should_emit:
                async for event in self._emit_stream_segment(candidate, ctx):
                    yield event
            else:
                ctx.pending_segment = candidate

    async def _maybe_emit_early_text_preview(
        self,
        ctx: RequestContext,
        complete_sentences: list[str],
    ) -> AsyncGenerator[Dict[str, Any], None]:
        """
        Ранний text-only emit для первого читаемого фрагмента.
        Audio path не трогаем: он по-прежнему стартует только от sentence boundary.
        """
        if ctx.has_emitted or ctx.early_text_preview_emitted or complete_sentences:
            return

        preview_candidate = (
            f"{ctx.pending_segment}{self.sentence_joiner}{ctx.stream_buffer}"
            if ctx.pending_segment and ctx.stream_buffer
            else (ctx.pending_segment or ctx.stream_buffer)
        ).strip()
        if not preview_candidate:
            return

        preview_words = await self._count_meaningful_words(preview_candidate)
        preview_min_words = max(2, self.stream_first_sentence_min_words // 2)
        preview_min_chars = max(24, self.stream_min_chars // 2)
        if preview_words < preview_min_words and len(preview_candidate) < preview_min_chars:
            return

        ctx.early_text_preview = preview_candidate
        ctx.early_text_preview_emitted = True
        logger.info(
            "📝 Early text preview emitted: len=%s words=%s session=%s",
            len(preview_candidate),
            preview_words,
            ctx.session_id,
        )
        yield {
            'success': True,
            'text_response': preview_candidate,
            'sentence_index': 0,
            'text_preview': True,
        }

    async def _emit_stream_segment(
        self,
        segment_text: str,
        ctx: RequestContext,
        *,
        log_label: str = "segment",
        include_audio_chunk_index: bool = False,
    ) -> AsyncGenerator[Dict[str, Any], None]:
        """Единый owner-path для text/audio emission одного сегмента."""
        to_emit = segment_text.strip()
        if not to_emit:
            return
        ...
        if complete_hash in ctx.processed_sentences:
            logger.warning(
                "🔄 ПРОПУСКАЕМ дублированный %s: '%s...' (hash=%s)",
                log_label,
                to_emit[:50],
                complete_hash,
            )
        ...
        preview = ctx.early_text_preview.strip()
        if preview and to_emit.startswith(preview):
            visible_text = to_emit[len(preview):]
            if visible_text.startswith(self.sentence_joiner):
                visible_text = visible_text[len(self.sentence_joiner):]
        ctx.early_text_preview = ""
        ctx.early_text_preview_emitted = False

        if visible_text.strip():
            yield {
                'success': True,
                'text_response': visible_text,
                'sentence_index': sentence_index
            }

        tts_text = to_emit if to_emit.endswith(self.end_punctuations) else f"{to_emit}."
        sentence_audio_chunks = 0
        async for audio_chunk in self._stream_audio_for_sentence(tts_text, sentence_index):
            if not audio_chunk:
                continue
            ...
            if include_audio_chunk_index:
                payload['audio_chunk_index'] = sentence_audio_chunks
            yield payload
        ctx.sentence_audio_map[sentence_index] = sentence_audio_chunks

    def _is_assistant_runtime_v2_enabled(self) -> bool:
        config = get_config()
        return bool(
            config.is_feature_enabled("assistant_runtime_v2")
            and not config.is_kill_switch_active("disable_assistant_runtime_v2")
        )
    ...
    async def process_request_streaming(self, request_data: Dict[str, Any]) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        if self._is_assistant_runtime_v2_enabled():
            _log_request_path_workflow("workflow.route", session_id, runtime="v2")
            async for item in self._runtime_v2.process_request_streaming(
                request_data,
                legacy_handler=self._process_request_streaming_current,
            ):
                yield item
            return

        _log_request_path_workflow("workflow.route", session_id, runtime="current")
        async for item in self._process_request_streaming_current(request_data):
            yield item

    @staticmethod
    def _annotate_runtime_v2_signals(
        request_data: Dict[str, Any],
        *,
        memory_context: Optional[Dict[str, Any]] = None,
        route: Optional[str] = None,
    ) -> None:
        signals = request_data.setdefault("_assistant_runtime_v2_signals", {})
        if not isinstance(signals, dict):
            signals = {}
            request_data["_assistant_runtime_v2_signals"] = signals

        if isinstance(memory_context, dict):
            goal_state = str(memory_context.get("goal_state_context", "") or "").strip().lower()
            current_goal = str(memory_context.get("current_goal_context", "") or "").strip()
            if goal_state:
                signals["goal_state"] = goal_state
            if current_goal:
                signals["current_goal"] = current_goal

        if route is not None:
            normalized_route = str(route or "").strip().lower()
            if normalized_route and not str(signals.get("route") or "").strip():
                signals["route"] = normalized_route

    @staticmethod
    def _annotate_request_timing(
        request_data: Dict[str, Any],
        *,
        stage: str,
        started_at: float,
        ...
        timings = request_data.setdefault("_request_timing_signals", {})
        if not isinstance(timings, dict):
            timings = {}
            request_data["_request_timing_signals"] = timings

        current_now = time.perf_counter() if now is None else now
        entry: Dict[str, float] = {
            "elapsed_ms": round((current_now - started_at) * 1000, 2),
        }
        if absolute_ms is not None:
            entry["absolute_ms"] = round(float(absolute_ms), 2)
        timings[stage] = entry

    @staticmethod
    def _log_request_timing_snapshot(
        session_id: str,
        request_data: Dict[str, Any],
    ) -> None:
        timings = request_data.get("_request_timing_signals")
        ...
        for stage in ordered_stages:
            ...
            if "elapsed_ms" in entry:
                fields[f"{stage}_elapsed_ms"] = entry["elapsed_ms"]
            if "absolute_ms" in entry:
                fields[f"{stage}_absolute_ms"] = entry["absolute_ms"]
        if fields:
            _log_request_path_workflow("workflow.timing", session_id, **fields)

    @staticmethod
    def _derive_runtime_v2_lifecycle_event(goal_state: Optional[str]) -> str:
        mapping = {
            "set": "task_opened",
            "keep": "task_continued",
    ...
    @classmethod
    def _log_runtime_v2_signal_snapshot(
        cls,
        session_id: str,
        request_data: Dict[str, Any],
        *,
        ...
        goal_state = str(signals.get("goal_state") or "").strip().lower()
        current_goal = str(signals.get("current_goal") or "").strip()
        route = str(signals.get("route") or "").strip().lower()
        lifecycle = cls._derive_runtime_v2_lifecycle_event(goal_state)
    ...
    @staticmethod
    def _route_matches_command(route: str, command: str) -> bool:
        route_to_commands = {
            "messages": {"send_message", "read_messages", "find_contact"},
            "whatsapp": {"send_whatsapp_message", "read_whatsapp_messages"},
            "system_control": {"open_app", "close_app"},
            "browser": {"browser_use", "close_browser"},
            "payment": {"buy_subscription", "manage_subscription"},
        }
        route_text = str(route or "").strip().lower()
        command_text = str(command or "").strip().lower()
        if not route_text or route_text == "none" or not command_text:
            return True
        allowed = route_to_commands.get(route_text)
        if allowed is None:
            return True
        return command_text in allowed

    @classmethod
    def _compute_runtime_v2_consistency(
        cls,
        request_data: Dict[str, Any],
        *,
        ...
        signals = request_data.get("_assistant_runtime_v2_signals")
        if not isinstance(signals, dict):
            return {"ok": True, "reason": "no_signals"}
        ...
        text = str(text_response or "").strip().lower()
        looks_like_question = text.endswith("?")

        if command and not cls._route_matches_command(route, command):
            return {
                "ok": False,
                "reason": "route_command_mismatch",
                "route": route or "-",
                "goal_state": goal_state or "-",
                "command": command or "-",
            }

        if goal_state == "clear" and not command and looks_like_question:
            return {
                "ok": False,
                "reason": "clear_goal_state_with_clarification_text",
                "route": route or "-",
                "goal_state": goal_state or "-",
                "command": command or "-",
            }
    ...
    @classmethod
    def _log_runtime_v2_consistency(
        cls,
        session_id: str,
        request_data: Dict[str, Any],
        *,
        ...
        result = cls._compute_runtime_v2_consistency(
            request_data,
            command_payload=command_payload,
            text_response=text_response,
        )
        _log_request_path_workflow(
            "workflow.v2_consistency",
            session_id,
    ...
    async def _process_request_streaming_current(self, request_data: Dict[str, Any]) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        try:
            ...
            request_perf_start = time.perf_counter()
            ...
            self._annotate_runtime_v2_signals(
                request_data,
                memory_context=memory_context,
            )
            self._log_runtime_v2_signal_snapshot(
                str(session_id or "-"),
                request_data,
                phase="memory",
            )
            memory_time = (time.time() - memory_start_time) * 1000
            self._annotate_request_timing(
                request_data,
                stage="memory_done",
                started_at=request_perf_start,
                absolute_ms=memory_time,
            )
            ...
            async for processed_sentence in self._iter_processed_sentences(
                ...
                if first_text_time is None:
                    ...
                    self._annotate_request_timing(
                        ...
                        stage="first_chunk",
                        started_at=request_perf_start,
                        absolute_ms=(time.time() - request_start_time) * 1000,
                    )
            ...
            if ctx.stream_buffer:
                ...
                for complete in complete_sentences:
                    ...
                    if should_emit:
                        async for event in self._emit_stream_segment(
                            candidate,
                            ctx,
                            log_label="final segment",
                        ):
                            yield event
                    else:
                        ctx.pending_segment = candidate
            ...
            if should_force_flush:
                async for event in self._emit_stream_segment(
                    ctx.pending_segment,
                    ctx,
                    log_label="forced final segment",
                    include_audio_chunk_index=True,
                ):
                    yield event
            ...
            self._log_runtime_v2_consistency(
                str(session_id or "-"),
                request_data,
                command_payload=ctx.pending_command_payload,
            ...
            self._annotate_request_timing(
                ...
                stage="request_done",
                started_at=request_perf_start,
                absolute_ms=total_time,
            )
            self._log_request_timing_snapshot(str(session_id or "-"), request_data)
    ...
    async def _stream_text_module(
        ...
        if callable(get_processor):
            ...
            if callable(prepare):
                prepared_request_result = prepare(
                    text=text,
                    session_id=session_id,
                    runtime_memory_context=runtime_memory_context,
                ...
                prepared_request = (
                    await prepared_request_result
                    if inspect.isawaitable(prepared_request_result)
                    else prepared_request_result
                )
