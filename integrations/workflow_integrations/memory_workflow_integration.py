...
class MemoryWorkflowIntegration:
    ...
    def __init__(self, memory_manager=None):
        ...
        # Background memory update can invoke the semantic analyzer and DB write.
        # 1s is too aggressive in live runs and produces false post-state failures.
        self.memory_update_timeout = 5.0
        self._memory_tasks = {}
        ...
        try:
            ...
            update_response = await asyncio.wait_for(
                self._call_memory_module({
                    ...
                    "prompt": prompt,
                    "response": response,
                    "route": memory_data.get("route", ""),
                    "behavior": memory_data.get("behavior", ""),
                    "command_name": memory_data.get("command_name", ""),
                    "command_present": bool(memory_data.get("command_present", False)),
                }),
            ...
            logger.error(
                "❌ Ошибка фонового сохранения в память: timeout after %.2fs",
                self.memory_update_timeout,
            )
        except Exception as e:
    ...
    def _prepare_memory_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            memory_data = {
                ...
                'command_present': bool(data.get('command_present', False)),
            }
        ...
        try:
            async with self._cache_lock:
                ...
                return get_cache_state(
                    ...
                    cache_hard_ttl=self.cache_hard_ttl,
                )
        except Exception as e:
            ...
            async with self._cache_lock:
                ...
                self.memory_cache.update(
                    build_cache_write_entries(
                        ...
                        user_input=user_input,
                        apply_medium_gate=apply_medium_gate,
                        strip_medium_term=self._strip_medium_term_from_context,
                    )
                )
    ...
    async def _schedule_cache_refresh(self, hardware_id: str, apply_medium_gate: bool = True):
        ...
        try:
            ...
            refresh_delay = get_refresh_delay_sec(
                cache_ttl=self.cache_ttl,
                cache_refresh_before_expiry=self.cache_refresh_before_expiry,
            )
