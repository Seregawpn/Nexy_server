...
4. Выполняет действие через ActionExecutor
...
async def test_full_chain_mcp_to_action(event_bus, grpc_integration, action_integration):
    ...
    assert started_event.get("feature_id") == ACTION_FEATURE_ID


@pytest.mark.asyncio
async def test_full_chain_with_mock_executor(event_bus, grpc_integration, action_integration):
    """Тест: Полная цепочка с мокированным executor."""
    # Включаем executor
    action_integration._executor._config.enabled = True
    ...
    with patch.object(action_integration._executor, 'execute') as mock_execute:
        ...
        assert mock_execute.called, "Executor должен быть вызван"
