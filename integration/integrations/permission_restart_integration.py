...
class PermissionRestartIntegration(BaseIntegration):
    ...
    async def _handle_transition(self, transition: PermissionTransition) -> None:
        ...
        current_statuses = self._get_current_permission_statuses()
        if not all(status == PermissionStatus.GRANTED for status in current_statuses.values()):
            logger.info(
                "[PERMISSION_RESTART] Waiting for all critical permissions before restart (granted=%s)",
                {perm.value: status.value for perm, status in current_statuses.items()},
            )
            return
    ...
    async def _on_app_startup_event(self, event: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if test_mode:
                ...
                current_permissions = {
                    "microphone": FirstRunPermissionStatus.GRANTED,
    ...
    def _map_first_run_status(self, status: FirstRunPermissionStatus) -> PermissionStatus:
        if status == FirstRunPermissionStatus.GRANTED:
            return PermissionStatus.GRANTED
        if status == FirstRunPermissionStatus.DENIED:
            return PermissionStatus.DENIED
        if status == FirstRunPermissionStatus.NOT_DETERMINED:
    ...
    def _get_current_permission_statuses(self) -> Dict[PermissionType, PermissionStatus]:
        ...
        for permission in self._config.critical_permissions:
            if permission == PermissionType.ACCESSIBILITY:
                status = self._map_first_run_status(check_accessibility_status())
            elif permission == PermissionType.INPUT_MONITORING:
                status = self._map_first_run_status(check_input_monitoring_status())
            elif permission == PermissionType.SCREEN_CAPTURE:
                status = self._map_first_run_status(check_screen_capture_status())
            elif permission == PermissionType.MICROPHONE:
                status = self._map_first_run_status(check_microphone_status())
            else:
