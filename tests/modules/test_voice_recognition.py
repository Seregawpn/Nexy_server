...
class VoiceRecognitionDiagnostic:
    ...
    async def _test_audio_recording(self):
        ...
        try:
            ...
            if result:
                ...
                if self.recognizer.state == RecognitionState.LISTENING:
                    ...
                    if len(self.recognizer.audio_data) > 0:
                        ...
                        self._add_result("audio_recording_data", False, "Аудио данные не записались",
                                       "Проблема с записью аудио", "Проверить audio callback и sounddevice", {})
                else:
                    self._add_result("audio_recording_start", False, f"Состояние не изменилось: {self.recognizer.state}",
                                   "Проблема с изменением состояния", "Проверить логику состояний", {})
            else:
                self._add_result("audio_recording_start", False, "Не удалось запустить прослушивание",
                               "Проблема с запуском записи", "Проверить input_device_index и sounddevice", {})
    ...
    async def _test_speech_recognition(self):
        ...
        try:
            ...
            if len(self.recognizer.audio_data) == 0:
                self._add_result("speech_recognition", False, "Нет аудио данных для распознавания",
                               "Предыдущий тест записи не прошел", "Сначала исправить запись аудио", {})
                return
            ...
            if result:
                if result.text:
                    ...
                    self._add_result("speech_recognition", False, f"Речь не распознана: {result.error}",
                                   "Google Speech Recognition не распознал аудио", 
                                   "Проверить качество аудио и интернет соединение", 
                                   {"error": result.error})
            else:
                self._add_result("speech_recognition", False, "Ошибка распознавания речи",
                               "Проблема с Google Speech Recognition", 
                               "Проверить интернет соединение и API", {})
