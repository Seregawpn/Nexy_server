...
@pytest.mark.asyncio
async def test_partial_stream_failure_emits_terminal_error_message(
    servicer: NewStreamingServicer,
) -> None:
    sid = _session_id()

    async def _process(_request_data):
        yield {"success": True, "text_response": "partial text"}
        yield {
            "success": False,
            "silent": True,
    ...
    assert [resp.WhichOneof("content") for resp in responses] == [
        "text_chunk",
        "error_message",
    ]
    assert responses[0].text_chunk == "partial text"
