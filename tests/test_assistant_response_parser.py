...
class TestAssistantResponseParser:
    ...
    def test_action_response_no_text(self, parser):
        ...
        with patch.object(parser.logger, 'warning') as mock_warning:
            ...
            # Должен быть непустой fallback-текст
            assert result.text_response == "Opening Telegram."
            assert result.command_payload is not None
            assert result.command_payload['payload']['command'] == 'open_app'
    ...
    def test_text_not_string(self, parser):
        ...
        with patch.object(parser.logger, 'warning') as mock_warning:
            ...
            # Должен подставить непустой fallback
            assert result.text_response == "I can help with that. Please tell me what you want to do."
            assert result.command_payload is None
            # Проверяем, что было предупреждение
            assert mock_warning.called
    ...
    def test_browser_use_legacy_args_are_normalized_to_task(self, parser, monkeypatch):
        """Legacy browser_use args (url/action/query) должны приниматься через task-normalization."""
        ...
        response = {
            ...
            "text": "Opening YouTube and searching for clip."
        }
        ...
        try:
            ...
            assert result.command_payload is not None
            payload = result.command_payload["payload"]
            assert payload["command"] == "browser_use"
            assert isinstance(payload["args"], dict)
            assert payload["args"].get("task"), "task must be synthesized for legacy browser args"
            assert "search clip" in payload["args"]["task"].lower()
        finally:
            reload_config()
