#!/usr/bin/env python3
"""
Диагностический тест для SignalIntegration
...
class SignalIntegrationDiagnostic:
    """Диагностический тест для SignalIntegration"""
    ...
    async def run_diagnostic(self) -> Dict[str, Any]:
        """Запуск полной диагностики SignalIntegration"""
        logger.info("🔍 Диагностика SignalIntegration...")
    ...
    async def _test_initialization(self):
        """Тест инициализации SignalIntegration"""
        ...
        try:
            ...
            # Создаем интеграцию
            self.signal_integration = SignalIntegration(
            ...
            self.results['initialization'] = {
                "success": True,
                "message": "SignalIntegration успешно инициализирован",
                "details": {
                    "event_bus_created": self.event_bus is not None,
                    "config_loaded": app_config is not None,
                    "integration_created": self.signal_integration is not None
                }
            }
    ...
    async def _test_configuration(self):
        ...
        try:
            ...
            config_result = {
                "success": True,
                "message": "Конфигурация сигналов загружена успешно",
                "details": {
    ...
    async def _test_eventbus_integration(self):
        ...
        try:
            ...
            # Ожидаемые события для SignalIntegration
            expected_events = [
                'app.startup',
                'app.shutdown',
                'signal.sigterm',
    ...
    async def _test_signal_handling(self):
        ...
        logger.info("4️⃣ Тест обработки сигналов...")
        
        try:
            signal_result = {
                "success": True,
                "message": "Обработка сигналов работает",
                "details": {}
            }
            
            # Проверяем наличие методов обработки сигналов
            if self.signal_integration:
                ...
                for method in methods:
                    signal_result["details"][f"has_{method}"] = hasattr(
                        self.signal_integration, method
                    )
            
            # Проверяем состояние интеграции
            signal_result["details"]["integration_active"] = (
                self.signal_integration is not None
            )
            
            self.results['signal_handling'] = signal_result
            
        except Exception as e:
            self.results['signal_handling'] = {
                "success": False,
                "error": str(e),
                "message": f"Ошибка обработки сигналов: {e}"
            }
    ...
    def _analyze_results(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "signal_handling": self.results.get('signal_handling', {}).get('success', False),
...
async def main():
    ...
    diagnostic = SignalIntegrationDiagnostic()
    result = await diagnostic.run_diagnostic()
    
    print(f"\n{'='*60}")
    print(f"🔍 РЕЗУЛЬТАТ ДИАГНОСТИКИ SignalIntegration")
    print(f"{'='*60}")
