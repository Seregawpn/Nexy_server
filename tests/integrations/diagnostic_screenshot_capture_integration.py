#!/usr/bin/env python3
"""
Диагностический тест для ScreenshotCaptureIntegration
...
class ScreenshotCaptureIntegrationDiagnostic:
    """Диагностический тест для ScreenshotCaptureIntegration"""
    ...
    async def run_diagnostic(self) -> Dict[str, Any]:
        """Запуск полной диагностики ScreenshotCaptureIntegration"""
        logger.info("🔍 Диагностика ScreenshotCaptureIntegration...")
    ...
    async def _test_initialization(self):
        """Тест инициализации ScreenshotCaptureIntegration"""
        ...
        try:
            ...
            try:
                screen_config = self.config_loader.get_app_config()
            except:
                screen_config = None
            ...
            # Создаем интеграцию
            self.screenshot_integration = ScreenshotCaptureIntegration(
            ...
            # Инициализируем
            await self.screenshot_integration.initialize()
            
            self.results['initialization'] = {
                "success": True,
                "message": "ScreenshotCaptureIntegration успешно инициализирован",
                "details": {
                    "event_bus_created": self.event_bus is not None,
                    "config_loaded": screen_config is not None,
                    "integration_created": self.screenshot_integration is not None
                }
            }
    ...
    async def _test_configuration(self):
        ...
        try:
            ...
            config_result = {
                "success": True,
                "message": "Конфигурация захвата экрана загружена успешно",
                "details": {
                    "screen_config_exists": screen_config is not None,
                    "config_type": type(screen_config).__name__ if screen_config else None
                }
            }
            ...
            if screen_config:
                config_attrs = [
                    'capture_quality', 'save_format', 'save_path', 'auto_save'
                ]
                
                for attr in config_attrs:
                    config_result["details"][f"has_{attr}"] = hasattr(screen_config, attr)
    ...
    async def _test_eventbus_integration(self):
        ...
        try:
            ...
            # Ожидаемые события для ScreenshotCaptureIntegration
            expected_events = [
                'app.startup',
                'app.shutdown',
                'screenshot.capture_request',
    ...
    async def _test_screenshot_capture(self):
        ...
        logger.info("4️⃣ Тест захвата скриншотов...")
        
        try:
            screenshot_result = {
                "success": True,
                "message": "Захват скриншотов работает",
                "details": {}
            }
            
            # Проверяем наличие методов захвата скриншотов
            if self.screenshot_integration:
                methods = [
                    'initialize',
                    'start',
                    'stop',
                    'get_status',
                    '_capture_once',
                    '_on_voice_recording_stop',
                    '_on_mode_changed'
                ]
                
                for method in methods:
                    screenshot_result["details"][f"has_{method}"] = hasattr(
                        self.screenshot_integration, method
                    )
            
            # Проверяем состояние интеграции
            screenshot_result["details"]["integration_active"] = (
                self.screenshot_integration is not None
            )
            
            self.results['screenshot_capture'] = screenshot_result
            
        except Exception as e:
            self.results['screenshot_capture'] = {
                "success": False,
                "error": str(e),
                "message": f"Ошибка захвата скриншотов: {e}"
            }
    ...
    def _analyze_results(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "screenshot_capture": self.results.get('screenshot_capture', {}).get('success', False),
...
async def main():
    ...
    diagnostic = ScreenshotCaptureIntegrationDiagnostic()
    result = await diagnostic.run_diagnostic()
    
    print(f"\n{'='*60}")
    print(f"🔍 РЕЗУЛЬТАТ ДИАГНОСТИКИ ScreenshotCaptureIntegration")
    print(f"{'='*60}")
