#!/usr/bin/env python3
"""
Диагностический тест для InterruptManagementIntegration
...
from integration.integrations.interrupt_management_integration import InterruptManagementIntegration
from integration.core.event_bus import EventBus
from config.unified_config_loader import UnifiedConfigLoader

logger = logging.getLogger(__name__)

class InterruptManagementIntegrationDiagnostic:
    """Диагностический тест для InterruptManagementIntegration"""
    
    def __init__(self):
        self.results = {}
        self.event_bus = None
        self.interrupt_integration = None
        self.config_loader = None
        
    async def run_diagnostic(self) -> Dict[str, Any]:
        """Запуск полной диагностики InterruptManagementIntegration"""
        logger.info("🔍 Диагностика InterruptManagementIntegration...")
        ...
        try:
            ...
            # 4. Тест управления прерываниями
            await self._test_interrupt_management()
    ...
    async def _test_initialization(self):
        """Тест инициализации InterruptManagementIntegration"""
        ...
        try:
            ...
            # Создаем интеграцию
            self.interrupt_integration = InterruptManagementIntegration(
            ...
            # Инициализируем
            await self.interrupt_integration.initialize()
            
            self.results['initialization'] = {
                "success": True,
                "message": "InterruptManagementIntegration успешно инициализирован",
                "details": {
                    "event_bus_created": self.event_bus is not None,
                    "config_loaded": app_config is not None,
                    "integration_created": self.interrupt_integration is not None
                }
            }
    ...
    async def _test_eventbus_integration(self):
        ...
        try:
            ...
            # Ожидаемые события для InterruptManagementIntegration
            expected_events = [
                'app.startup',
                'app.shutdown',
                'interrupt.request',
    ...
    async def _test_interrupt_management(self):
        """Тест управления прерываниями"""
        logger.info("4️⃣ Тест управления прерываниями...")
        
        try:
            interrupt_result = {
                "success": True,
                "message": "Управление прерываниями работает",
                "details": {}
            }
            
            # Проверяем наличие методов управления прерываниями
            if self.interrupt_integration:
                methods = [
                    'initialize',
                    'start',
                    'stop',
                    ...
                    '_on_interrupt_cancel'
                ]
                
                for method in methods:
                    interrupt_result["details"][f"has_{method}"] = hasattr(
                        self.interrupt_integration, method
                    )
            
            # Проверяем состояние интеграции
            interrupt_result["details"]["integration_active"] = (
                self.interrupt_integration is not None
            )
            
            self.results['interrupt_management'] = interrupt_result
            
        except Exception as e:
            self.results['interrupt_management'] = {
                "success": False,
                "error": str(e),
                "message": f"Ошибка управления прерываниями: {e}"
            }
    ...
    def _analyze_results(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "interrupt_management": self.results.get('interrupt_management', {}).get('success', False),
...
async def main():
    ...
    diagnostic = InterruptManagementIntegrationDiagnostic()
    result = await diagnostic.run_diagnostic()
    
    print(f"\n{'='*60}")
    print(f"🔍 РЕЗУЛЬТАТ ДИАГНОСТИКИ InterruptManagementIntegration")
    print(f"{'='*60}")
