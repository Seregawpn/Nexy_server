#!/usr/bin/env python3
"""
Диагностический тест для SpeechPlaybackIntegration
Проверяет инициализацию, конфигурацию, воспроизведение речи и функциональность
"""
...
class SpeechPlaybackIntegrationDiagnostic:
    """Диагностический тест для SpeechPlaybackIntegration"""
    ...
    async def run_diagnostic(self) -> Dict[str, Any]:
        """Запуск полной диагностики SpeechPlaybackIntegration"""
        logger.info("🔍 Диагностика SpeechPlaybackIntegration...")
        ...
        try:
            ...
            # 4. Тест воспроизведения речи
            await self._test_speech_playback()
    ...
    async def _test_initialization(self):
        """Тест инициализации SpeechPlaybackIntegration"""
        ...
        try:
            ...
            speech_config = config_loader.get_speech_playback_config()
            ...
            self.results['initialization'] = {
                'success': True,
                'description': 'SpeechPlaybackIntegration успешно инициализирован',
                ...
                'metrics': {
                    ...
                    'integration_created': self.speech_integration is not None
                }
            }
    ...
    async def _test_configuration(self):
        """Тест конфигурации SpeechPlaybackIntegration"""
        ...
        try:
            if not self.speech_integration:
                ...
                'success': True,
                    'error': 'SpeechPlaybackIntegration не инициализирован',
            ...
            config = self.speech_integration.config
            has_sample_rate = 'sample_rate' in config
            has_channels = 'channels' in config
            has_buffer_size = 'buffer_size' in config
            
            self.results['configuration'] = {
                'success': has_sample_rate and has_channels,
                'description': 'Конфигурация SpeechPlaybackIntegration проверена',
                'cause': 'Конфигурация загружена и содержит необходимые параметры',
                'solution': 'Продолжить тестирование',
                'metrics': {
                    'has_sample_rate': has_sample_rate,
                    'has_channels': has_channels,
                    'has_buffer_size': has_buffer_size,
    ...
    async def _test_eventbus_integration(self):
        ...
        try:
            if not self.speech_integration or not self.event_bus:
            ...
            has_speech_subscriptions = any('speech' in event or 'playback' in event for event in subscribers.keys())
            ...
            self.results['eventbus_integration'] = {
                ...
                'metrics': {
                    ...
                    'has_speech_subscriptions': has_speech_subscriptions,
    ...
    async def _test_speech_playback(self):
        """Тест воспроизведения речи"""
        logger.info("4️⃣ Тест воспроизведения речи...")
        
        try:
            if not self.speech_integration:
                self.results['speech_playback'] = {
                    'success': False,
                    'error': 'SpeechPlaybackIntegration не инициализирован',
                    'cause': 'Пропущен тест инициализации',
                    'solution': 'Сначала выполнить тест инициализации'
            ...
            # Проверяем наличие speech player
            has_player = hasattr(self.speech_integration, '_player')
            has_config = hasattr(self.speech_integration, 'config')
            has_initialized = hasattr(self.speech_integration, '_initialized')
            has_running = hasattr(self.speech_integration, '_running')
            
            self.results['speech_playback'] = {
                'success': has_player and has_config and has_initialized and has_running,
                'description': 'Воспроизведение речи проверено',
                'cause': 'Speech player доступен и готов к работе',
                'solution': 'Продолжить тестирование',
                'metrics': {
            ...
            self.results['speech_playback'] = {
                'success': False,
                'error': str(e),
                'cause': 'Ошибка доступа к speech player',
                'solution': 'Проверить инициализацию speech player'
            }
    
    async def _test_functionality(self):
        """Тест функциональности SpeechPlaybackIntegration"""
        ...
        try:
            ...
            has_start = hasattr(self.speech_integration, 'start')
            has_stop = hasattr(self.speech_integration, 'stop')
            has_handle_speech_request = hasattr(self.speech_integration, 'handle_speech_request')
            
            self.results['functionality'] = {
                'success': has_start and has_stop,
                'description': 'Функциональность SpeechPlaybackIntegration проверена',
                'cause': 'Основные методы доступны',
                'solution': 'Продолжить тестирование',
                'metrics': {
                    'has_start_method': has_start,
                    'has_stop_method': has_stop,
                    'has_handle_speech_request': has_handle_speech_request,
                    'methods_available': 3 if has_start and has_stop and has_handle_speech_request else 0
                }
            }
...
async def main():
    ...
    diagnostic = SpeechPlaybackIntegrationDiagnostic()
    results = await diagnostic.run_diagnostic()
    analysis = diagnostic._analyze_results()
    
    print(f"\n📊 Результаты диагностики SpeechPlaybackIntegration:")
