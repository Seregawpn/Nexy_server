...
def test_goal_stack_gate_registry_has_l1_basic_direct_campaign_shape():
    assert len(L1_BASIC_DIRECT_20_CASE_GATE) == 20

    case_ids = [case.case_id for case in L1_BASIC_DIRECT_20_CASE_GATE]
    assert len(set(case_ids)) == 20

    interaction_shapes = {case.interaction_shape for case in L1_BASIC_DIRECT_20_CASE_GATE}
    assert interaction_shapes <= {"direct", "non_task"}

    categories = Counter(case.category for case in L1_BASIC_DIRECT_20_CASE_GATE)
    assert categories["messages"] >= 3
    assert categories["whatsapp"] >= 2
    assert categories["browser"] >= 2
    assert categories["google_search"] >= 2
    assert categories["system_control"] >= 3
    assert categories["none"] >= 1
    assert categories["capability"] >= 1
    assert categories["describe"] >= 1


def test_goal_stack_gate_registry_has_l2_clarification_slot_fill_campaign_shape():
    assert len(L2_CLARIFICATION_SLOT_FILL_20_CASE_GATE) == 20

    case_ids = [case.case_id for case in L2_CLARIFICATION_SLOT_FILL_20_CASE_GATE]
    assert len(set(case_ids)) == 20

    interaction_shapes = {case.interaction_shape for case in L2_CLARIFICATION_SLOT_FILL_20_CASE_GATE}
    assert interaction_shapes == {"follow_up"}

    categories = Counter(case.category for case in L2_CLARIFICATION_SLOT_FILL_20_CASE_GATE)
    assert categories["messages"] == 4
    assert categories["whatsapp"] == 4
    assert categories["browser"] == 4
    assert categories["google_search"] == 4
    assert categories["system_control"] == 4
