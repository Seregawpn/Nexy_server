...
def test_runtime_memory_policy_includes_memory_relevance_gate():
    ...
    assert "Ask where the work is supposed to happen: inside the operating system app itself, inside a website or web service, inside a messaging channel, or as an answer returned directly by the assistant." in prompt
    assert "Treat names like Safari, Telegram, Notes, YouTube, Reddit, or Wikipedia as supporting signals only after the intended outcome is clear; names alone are not the route decision." in prompt
    ...
    assert "The browser may open as a transport step for a website task, but that still stays browser when the user's real goal is to use the site or service rather than manage the browser app." in prompt
    assert "system_control is for desktop app lifecycle changes such as opening, launching, closing, or quitting an application; browser is for doing work inside a site or service after it opens." in prompt
    ...
    assert "Any future desktop application should follow the same rule: if the user wants the app itself to start, stop, or come to the foreground, prefer system_control." in prompt
    ...
    assert "Any future site or service should follow the same rule: if the user wants to browse, play, search, log in, navigate, or interact inside that service, prefer browser." in prompt
...
def test_intent_classifier_includes_memory_first_decision_policy():
    ...
    assert "The browser category is about where the user wants the task to happen: if the task lives inside a site or service experience, browser is still correct even when a browser app must physically open first." in prompt
    ...
    assert "system_control is about controlling the application itself, not the work that might happen inside that application after it opens." in prompt
    assert "Use the same rule for any future app name: if the request is fundamentally about app lifecycle, foregrounding, or launching the desktop application, prefer system_control." in prompt
...
def test_runtime_memory_policy_forbids_dropping_known_messaging_slots():
    ...
    assert "Apply the shared messaging slot-resolution policy before finalizing any message/WhatsApp follow-up." in prompt
