...
def test_build_cache_write_entries_respects_current_gate_bucket() -> None:
    module = _MemoryModuleWithGate()
    now = datetime.now()

    entries = build_cache_write_entries(
        hardware_id="hw-gated",
        memory_module=module,
        memory_context={
            "short_term_context": "recent",
            "medium_term_context": "allowed",
            "factual_long_term_context": "fact",
        },
        now=now,
        user_input="Summarize my longer-term projects",
    ...
    assert ("hw-gated", "medium") in entries
    assert entries[("hw-gated", "medium")]["context"]["medium_term_context"] == "allowed"
