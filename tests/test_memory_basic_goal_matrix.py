...
L1_MEMORY_GATE_CASES = [case for case in L1_BASIC_DIRECT_20_CASE_GATE if case.memory_prompt is not None]
L1_CONFIDENCE_MEMORY_CASES = [case for case in L1_DIRECT_CONFIDENCE_PROBES if case.memory_prompt is not None]
L2_MEMORY_GATE_CASES = [case for case in L2_CLARIFICATION_SLOT_FILL_20_CASE_GATE if case.memory_prompt is not None]
...
@pytest.mark.asyncio
@pytest.mark.parametrize("case", L1_MEMORY_GATE_CASES, ids=[case.case_id for case in L1_MEMORY_GATE_CASES])
async def test_memory_l1_basic_direct_campaign(case):
    await test_memory_basic_goal_matrix(case)


@pytest.mark.asyncio
@pytest.mark.parametrize("case", L1_CONFIDENCE_MEMORY_CASES, ids=[case.case_id for case in L1_CONFIDENCE_MEMORY_CASES])
async def test_memory_l1_direct_confidence_probes(case):
    await test_memory_basic_goal_matrix(case)


@pytest.mark.asyncio
@pytest.mark.parametrize("case", L2_MEMORY_GATE_CASES, ids=[case.case_id for case in L2_MEMORY_GATE_CASES])
async def test_memory_l2_clarification_slot_fill_campaign(case):
    await test_memory_basic_goal_matrix(case)
