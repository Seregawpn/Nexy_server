...
class _AnalyzerByPrompt:
    async def analyze_conversation(self, prompt, response, **kwargs):
        ...
        if "alpha" in prompt:
            return "", "[2026-03-06] Alpha flow completed.", ""
        return "", "[2026-03-07] Beta flow completed.", ""
...
async def test_parallel_updates_keep_consistent_merged_snapshot_with_user_lock():
    ...
    assert "alpha flow completed" in medium_value.lower() or "beta flow completed" in medium_value.lower()
