...
def _vertex_provider() -> LangChainGeminiProvider:
    return LangChainGeminiProvider(
        {
            "model": "gemini-2.5-flash",
            "temperature": 0.1,
...
@pytest.mark.asyncio
async def test_process_use_google_search_uses_native_vertex_search_when_available():
    provider = _vertex_provider()
    provider.is_initialized = True
    provider.vertex_native_client = object()

    async def _fake_native(**kwargs):
        yield "Grounded native vertex answer."

    provider._stream_vertex_google_search_native = _fake_native  # type: ignore[method-assign]

    chunks = []
    async for chunk in provider.process(
        "What are the latest world news headlines?",
        session_id="sid-vertex-search",
        use_google_search=True,
    ):
        chunks.append(chunk)

    assert "".join(chunks) == "Grounded native vertex answer."


@pytest.mark.asyncio
async def test_process_with_image_use_google_search_uses_native_vertex_search_when_available():
    provider = _vertex_provider()
    provider.is_initialized = True
    provider.vertex_native_client = object()
    ...
    async def _fake_native(**kwargs):
        ...
        yield "Grounded native image answer."
    ...
    async for chunk in provider.process_with_image(
        ...
        session_id="sid-vertex-image-search",
        use_google_search=True,
    ):
        chunks.append(chunk)

    assert "".join(chunks) == "Grounded native image answer."
    assert captured["image_bytes"] == b"fake-image-base64"
