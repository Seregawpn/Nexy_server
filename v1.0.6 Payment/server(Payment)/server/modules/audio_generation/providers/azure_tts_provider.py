    ...
    async def initialize(self) -> bool:
        ...
        try:
            if not AZURE_SPEECH_AVAILABLE:
                logger.error("Azure TTS Provider not available - Azure Speech SDK not installed")
                return False
            
            if not self.speech_key or not self.speech_region:
                logger.error(
                    f"Azure TTS Provider not available - missing credentials: "
                    f"speech_key={'set' if self.speech_key else 'missing'}, "
                    f"speech_region={'set' if self.speech_region else 'missing'}"
                )
                return False
            
            logger.info(
                f"Azure TTS Provider: initializing with region={self.speech_region}, "
            ...
            logger.debug("Azure TTS Provider: synthesizer created, testing connection...")
            ...
            if test_result:
                ...
                logger.error("Azure TTS Provider test synthesis failed - see logs above for details")
                return False
            ...
            logger.debug(f"Azure TTS Provider initialization traceback: {error_details}")
            return False
    ...
    async def _test_connection(self) -> bool:
        ...
        try:
            if not self.synthesizer:
                logger.error("Azure TTS Provider: synthesizer not initialized for connection test")
                return False
            
            # Простой тестовый синтез
            test_text = "Hello, this is a test."
            logger.debug(f"Azure TTS Provider: testing connection with text: '{test_text}'")
            result = self.synthesizer.speak_text_async(test_text).get()
            
            if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
                logger.info("Azure TTS Provider: connection test successful")
                return True
            elif result.reason == speechsdk.ResultReason.Canceled:
                cancellation_details = result.cancellation_details
                error_msg = (
                    f"Azure TTS Provider: connection test canceled - "
                    f"reason={cancellation_details.reason}, "
                    f"error_details={cancellation_details.error_details}"
                )
                logger.error(error_msg)
                return False
            else:
                error_msg = f"Azure TTS Provider: connection test failed with reason={result.reason}"
                logger.error(error_msg)
                return False
            ...
            import traceback
            error_details = traceback.format_exc()
            logger.error(f"Azure TTS Provider connection test failed with exception: {e}")
            logger.debug(f"Azure TTS Provider connection test traceback: {error_details}")
            return False
