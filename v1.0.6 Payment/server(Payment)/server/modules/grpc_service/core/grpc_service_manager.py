...
class GrpcServiceManager(UniversalModuleInterface):
    ...
    async def _initialize_with_coordinator(self) -> None:
        ...
        # Связываем модули между собой (например, memory_management с database)
        await self._link_modules()
    ...
    async def _link_modules(self) -> None:
        """Связывание модулей между собой после инициализации"""
        logger.info("Связывание модулей...")
        
        try:
            # Связываем memory_management с database
            if self.coordinator.has('memory_management') and self.coordinator.has('database'):
                ...
                # Проверяем, что модули поддерживают нужные методы
                if hasattr(memory_module, 'set_database_manager') and hasattr(database_module, 'get_manager'):
                    db_manager = database_module.get_manager()
                    if db_manager:
                        memory_module.set_database_manager(db_manager)
                        logger.info("✅ MemoryManager связан с DatabaseManager")
                    else:
                        logger.warning("⚠️ DatabaseManager.get_manager() вернул None")
                else:
                    logger.debug("Модули не поддерживают связывание (set_database_manager/get_manager)")
        except Exception as e:
            logger.warning(f"⚠️ Ошибка связывания модулей: {e}")
    ...
    async def _create_workflow_integrations(self) -> None:
        ...
        try:
            ...
            self.streaming_workflow = StreamingWorkflowIntegration(
                ...
                coordinator=self.coordinator  # Передаем coordinator для доступа к browser_use
            )
            ...
            # Установка interrupt_workflow для browser_use модуля (если доступен)
            if self.coordinator and self.coordinator.has('browser_use'):
                browser_use_module = self.coordinator.get('browser_use')
                if hasattr(browser_use_module, 'set_interrupt_workflow'):
                    browser_use_module.set_interrupt_workflow(self.interrupt_workflow)
                    logger.info("✅ InterruptWorkflow установлен для browser_use модуля")
