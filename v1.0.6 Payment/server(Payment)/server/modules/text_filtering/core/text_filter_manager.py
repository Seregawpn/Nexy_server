...
class TextFilterManager(UniversalModuleInterface):
    ...
    def _simple_clean_text(self, text: str, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Получаем конфигурацию очистки текста
            cleaning_config = self.config.get_text_cleaning_config()
            
            # Убираем лишние пробелы
            cleaned_text = ' '.join(cleaned_text.split())
            operations.append("remove_extra_whitespace")
            
            # Убираем специальные символы через регулярное выражение
            import re
            allowed_chars = cleaning_config.get("allowed_chars", r'[^\w\s\.\,\!\?\-\:\;\(\)\[\]\{\}\"\'@#$%&*+=<>/|~`]')
            cleaned_text = re.sub(allowed_chars, '', cleaned_text)
            operations.append("remove_special_chars")
            
            # Удаляем символы, требующие явного удаления (например, обратный слеш для TTS)
            chars_to_remove = cleaning_config.get("chars_to_remove_explicitly", [])
            for char in chars_to_remove:
                if char in cleaned_text:
                    cleaned_text = cleaned_text.replace(char, '')
                    operations.append(f"remove_explicit_char_{repr(char)}")
