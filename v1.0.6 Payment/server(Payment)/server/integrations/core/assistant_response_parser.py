...
class AssistantResponseParser:
    ...
    def _parse_dict(self, data: Dict[str, Any], session_id: Optional[str] = None) -> ParsedResponse:
        ...
        if PYDANTIC_AVAILABLE and LLMResponseValidator:
            ...
            if validated_model:
                ...
                if isinstance(validated_model, ActionResponse):
                    ...
                    # ⚠️ NEW: Проверка feature flags перед формированием command_payload (Pydantic путь)
                    feature_id = self._get_feature_id_for_command(validated_model.command)
                    if CONFIG_AVAILABLE and get_config:
                        try:
                            config = get_config()
                            if not config.features.forward_assistant_actions:
                                # Feature flag выключен - возвращаем только текст
                                self.logger.info(
                                    "[%s] Assistant actions disabled by feature flag (Pydantic), returning text only: command=%s",
                                    feature_id,
                                    validated_model.command
                                )
                                return ParsedResponse(
                                    text_response=validated_model.text,
                                    command_payload=None,
                            ...
                            if config.kill_switches.disable_forward_assistant_actions:
                                # Kill-switch активен - полностью отключаем команды
                                self.logger.warning(
                                    "[%s] Forward assistant actions disabled by kill-switch (Pydantic), returning text only: command=%s",
                                    feature_id,
                                    validated_model.command
                                )
                                return ParsedResponse(
                            ...
                            # Если ошибка при получении конфигурации, логируем и продолжаем
                            self.logger.warning(
                                "[%s] Error checking feature flags (Pydantic), continuing with command: %s",
                                feature_id,
                                e
                            )
                    ...
                    self.logger.info(
                        f"[{feature_id}] ✅ Валидирован action-ответ через Pydantic: command={validated_model.command}, "
        ...
        # Валидация команд в зависимости от типа
        if command == 'open_app':
            # Для open_app требуется app_name или app_path
            if 'app_name' not in args and 'app_path' not in args:
                validation_errors.append("Для команды 'open_app' требуется поле 'args.app_name' или 'args.app_path'")
            elif 'app_name' not in args and not args.get('app_path'):
                validation_errors.append("Для команды 'open_app' требуется поле 'args.app_name' или 'args.app_path'")
            ...
            # Для browser_use требуется task
            ...
            # ⚠️ NEW: Для read_messages contact опционален (может быть "last" или имя контакта)
            # Если contact не указан, будет использован "last" по умолчанию на стороне MCP сервера
            # limit опционален (по умолчанию 10)
            # Валидация происходит на стороне MCP сервера, здесь только проверяем формат
            if 'contact' in args and args.get('contact') and not isinstance(args.get('contact'), str):
                validation_errors.append("Для команды 'read_messages' поле 'args.contact' должно быть строкой")
            if 'limit' in args and args.get('limit') is not None:
                try:
                    limit = int(args.get('limit'))
                    if limit < 1:
                        validation_errors.append("Для команды 'read_messages' поле 'args.limit' должно быть положительным числом")
                except (ValueError, TypeError):
                    validation_errors.append("Для команды 'read_messages' поле 'args.limit' должно быть числом")
        elif command == 'send_message':
            # ⚠️ NEW: Для send_message contact и message обязательны
            if 'contact' not in args or not args.get('contact'):
                validation_errors.append("Для команды 'send_message' требуется поле 'args.contact'")
            elif not isinstance(args.get('contact'), str):
                validation_errors.append("Для команды 'send_message' поле 'args.contact' должно быть строкой")
            if 'message' not in args or not args.get('message'):
                validation_errors.append("Для команды 'send_message' требуется поле 'args.message'")
            elif not isinstance(args.get('message'), str):
                validation_errors.append("Для команды 'send_message' поле 'args.message' должно быть строкой")
        elif command == 'create_subscription':
            # MVP 8: Для create_subscription args не обязательны (все данные берутся из контекста)
            # Можно добавить опциональные поля: price_id, success_url, cancel_url
            pass  # Нет обязательных полей
        elif command == 'cancel_subscription':
            # MVP 8: Для cancel_subscription args не обязательны
            pass  # Нет обязательных полей
        elif command == 'manage_subscription':
            # MVP 10: Для manage_subscription args не обязательны (открывает Customer Portal)
            pass  # Нет обязательных полей
        ...
        # ⚠️ NEW: Проверка feature flags перед формированием command_payload
        # Если feature flags отключены, возвращаем только текст без команды
        feature_id = self._get_feature_id_for_command(command)
        if CONFIG_AVAILABLE and get_config:
            try:
                config = get_config()
                if not config.features.forward_assistant_actions:
                    # Feature flag выключен - возвращаем только текст
                    self.logger.info(
                        "[%s] Assistant actions disabled by feature flag, returning text only: command=%s",
                        feature_id,
                        command
                    )
                    return ParsedResponse(
                ...
                if config.kill_switches.disable_forward_assistant_actions:
                    # Kill-switch активен - полностью отключаем команды
                    self.logger.warning(
                        "[%s] Forward assistant actions disabled by kill-switch, returning text only: command=%s",
                        feature_id,
                        command
                    )
                    return ParsedResponse(
                ...
                # Если ошибка при получении конфигурации, логируем и продолжаем
                self.logger.warning(
                    "[%s] Error checking feature flags, continuing with command: %s",
                    feature_id,
                    e
                )
        ...
        self.logger.info(
            f"[{feature_id}] Распознан action-ответ: command={command}, session_id={action_session_id}, "
    ...
    def _get_feature_id_for_command(self, command: str) -> str:
        """
        Определяет feature_id для команды для логирования.
        
        Args:
            command: Название команды
            
        Returns:
            Feature ID для логирования
        """
        if command == 'open_app':
            return 'F-2025-013-open-app'
        elif command == 'close_app':
            return 'F-2025-014-close-app'
        elif command == 'browser_use':
            return 'F-2025-015-browser-use'
        elif command in ('read_messages', 'send_message'):
            return 'F-2025-016-messages-integration'
        elif command in ('create_subscription', 'cancel_subscription', 'manage_subscription'):
            return 'F-2025-017-stripe-payment'
        else:
            return None