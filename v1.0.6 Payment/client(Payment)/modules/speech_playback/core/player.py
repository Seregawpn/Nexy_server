...
class SequentialSpeechPlayer:
    ...
    def __init__(self, config: Optional[PlayerConfig] = None):
        ...
        # ✅ ЦИКЛ 2: AudioStreamManager для управления PortAudio streams
        self._stream_manager = AudioStreamManager(stream_type="output")  # Флаг для lazy start
        ...
        # ✅ АУДИТ: Флаг для отслеживания выполнения асинхронной проверки устройства
        self._device_check_in_progress: bool = False
        self._device_check_task: Optional[asyncio.Task] = None
        self._device_check_result: Optional[tuple[Optional[str], bool]] = None  # (device_name, is_bluetooth)
        
        # ✅ ЦИКЛ 1: Guard защита для переключения устройств (предотвращает concurrent переключения)
        self._switch_in_progress = False
        self._switch_in_progress_lock = threading.RLock()
        self._switch_in_progress_start_time: Optional[float] = None
        self._switch_in_progress_timeout = 10.0  # секунд - защита от залипания guard
        self._pending_switch_device_name: Optional[str] = None  # Для проверки отмены переключения
        self._switch_cancelled = False
        
        # ✅ ЦИКЛ 2: Кэш успешных конфигураций (для быстрого переключения на известные устройства)
        self._last_successful_config: Dict[str, StreamConfig] = {}  # device_name|BT -> StreamConfig
        self._successful_config_lock = threading.RLock()
        
        # ✅ ЦИКЛ 3: Метрики времени переключения устройств
        self._switch_device_times: List[float] = []  # История времени переключения
        self._max_switch_device_history = 100
        self._switch_sequence_counter = 0  # Для аудита последовательностей переключений
        self._last_switch_sequence_id: Optional[int] = None
        
        # ✅ ЦИКЛ 3: Старый polling удален - используем DeviceChangePublisher через EventBus
        # Оставляем только необходимые атрибуты для обратной совместимости
        self._stop_device_monitor = threading.Event()  # Используется в других местах
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            # Мониторинг устройств теперь происходит через события device.default_output_changed
    ...
    def add_audio_data(self, audio_data: np.ndarray, priority: int = 0, metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        try:
            ...
            if should_check_device:
                ...
                if device_changed:
                    # Устройство изменилось - пересоздаём поток
    ...
    def _start_audio_stream(self, *, sync_output: bool = True, device_id: int = None) -> bool:
        ...
        try:
            ...
            with self._stream_lock:
                ...
                if is_bluetooth:
                    bt_prestart_delay = 2.5  # Увеличено до 2.5 секунд для полной инициализации BT устройств
                ...
                for attempt in range(max_retries):
                    ...
                    # ✅ ИСПРАВЛЕНИЕ: Логируем подтверждение только при первой попытке (не в цикле retry)
                    if attempt == 0 and self._audio_stream is None:
                ...
                try:
                    # ✅ ЦИКЛ 2: Используем AudioStreamManager для создания потока
                    logger.info(f"🔍 [OUTPUT] Создаем новый поток через AudioStreamManager: device={stream_config.get('device')}, BT={is_bluetooth}")
                    
                    # Подготавливаем StreamConfig для AudioStreamManager
                    stream_config_obj = StreamConfig(
                        device_id=stream_config.get('device'),
                        device_name=device_name,
                        samplerate=stream_config.get('samplerate'),
                        channels=stream_config.get('channels'),
                        dtype=stream_config.get('dtype'),
                        blocksize=stream_config.get('blocksize'),
                        latency=stream_config.get('latency'),
                        callback=stream_config.get('callback') or self._audio_callback,
                        is_bluetooth=is_bluetooth
                    )
                    
                    # ✅ ЦИКЛ 1: Используем _run_async_in_thread для выполнения async операций
                    # Уменьшенный таймаут для быстрого отклика (3-5s вместо 15-30s)
                    timeout_sec = 5.0 if is_bluetooth else 3.0
                    max_retries_for_create = 2  # Уменьшено с 5 для быстрого fallback
                    
                    result = self._run_async_in_thread(
                        self._stream_manager.create_stream(stream_config_obj, max_retries=max_retries_for_create),
                        timeout_sec,
                        "create_stream (_start_audio_stream)"
                    )
                    ...
                    if self._audio_stream is None:
                        raise RuntimeError("Поток не был создан через AudioStreamManager")
                    
                    self._stream_started = False

                    elapsed = time.time() - stream_start_time
                    # ✅ ИСПРАВЛЕНИЕ: Используем result.attempt из AudioStreamManager (реальная попытка создания потока)
                    audio_manager_attempt = getattr(result, 'attempt', 1)
                    if audio_manager_attempt > 1:
                        logger.info(f"✅ [OUTPUT] Аудио поток создан успешно на попытке {audio_manager_attempt}! (время создания: {elapsed:.2f}с)")
                    else:
                        ...
                        if attempt < max_retries - 1:
                            ...
                            if stream_config.get('device') is not None or (is_bluetooth and stream_config.get('channels', 2) == 2):
                                ...
                                if is_bluetooth:
                                    ...
                                    logger.info(f"🔧 [OUTPUT] Для BT устройства убраны все параметры буферизации (пусть PortAudio выберет сам)")
                                try:
                                    # ✅ ЦИКЛ 2: Используем AudioStreamManager для последней попытки
                                    stream_config_obj = StreamConfig(
                                        ...
                                        blocksize=stream_config.get('blocksize'),
                                        latency=stream_config.get('latency'),
                                        callback=stream_config.get('callback'),  # Callback из stream_config
                                        is_bluetooth=is_bluetooth
                                    )
                                    
                                    # ✅ ЦИКЛ 1: Используем _run_async_in_thread для выполнения async операций
                                    timeout_sec = 5.0 if is_bluetooth else 3.0
                                    max_retries_for_create = 2
                                    
                                    result = self._run_async_in_thread(
                                        self._stream_manager.create_stream(stream_config_obj, max_retries=max_retries_for_create),
                                        timeout_sec,
                                        "create_stream (fallback device=None)"
                                    )
                                    
                                    if not result.success:
                                        raise RuntimeError(f"Не удалось создать поток: {result.error_message}")
                                    ...
                                    self._stream_started = False
                                    logger.info(f"✅ [OUTPUT] Поток создан с системным дефолтом и моно через AudioStreamManager")
    ...
    def _stop_audio_stream(self, is_bluetooth: bool = False):
        ...
        ✅ ЦИКЛ 2: Использует AudioStreamManager для безопасного закрытия потока.
        ...
        try:
            with self._stream_lock:
                if self._audio_stream is not None:
                    ...
                    stream_ref = self._audio_stream
                    
                    # ✅ ЦИКЛ 1: Используем _run_async_in_thread для закрытия потока
                    timeout_sec = 5.0 if is_bluetooth else 3.0
                    result = self._run_async_in_thread(
                        self._stream_manager.close_stream(stream_ref, is_bluetooth=is_bluetooth),
                        timeout_sec,
                        "close_stream (_stop_audio_stream)"
                    )

                    success = getattr(result, "success", bool(result))
                    seq_info = f"seq={self._last_switch_sequence_id}" if self._last_switch_sequence_id is not None else "seq=unknown"
                    if success:
                        logger.info(f"✅ [OUTPUT][{seq_info}] close_stream завершён успешно")
                    else:
                        error_message = getattr(result, "error_message", None)
                        logger.warning(
                            f"⚠️ [OUTPUT] close_stream [{seq_info}] ошибка: "
                            f"{error_message or 'неизвестная причина'}"
                        )
                    
                    # Останавливаем поток если он был стартован
                    if self._stream_started:
                        try:
                            if hasattr(stream_ref, 'active') and stream_ref.active:
                                stream_ref.stop()
                        except Exception:
                            pass
                        self._stream_started = False
                    ...
                    self._audio_stream = None
                    logger.info(f"🛑 Аудио поток остановлен и закрыт через AudioStreamManager (BT={is_bluetooth})")
                    
                    # ✅ ФИНАЛЬНОЕ РЕШЕНИЕ: Задержка после close() для гарантии освобождения устройства
                    # Для BT устройств нужна большая задержка (2.5с), для обычных - меньшая (0.3с)
                    if is_bluetooth:
                        close_delay = 2.5  # 2.5с для BT устройств (увеличено для гарантии освобождения)
                    ...
                    logger.info(f"✅ [OUTPUT] Поток полностью закрыт (задержка: {close_delay:.3f}с)")
    ...
    def _run_async_in_thread(self, coro, timeout_sec: float, operation_name: str = "operation"):
        """
        ✅ ЦИКЛ 1: Выполняет async корутину в отдельном thread с таймаутом
        
        Используется для выполнения async операций AudioStreamManager в синхронном контексте
        ...
        Returns:
            Результат выполнения корутины или StreamOperationResult с ошибкой при таймауте
        """
        ...
        def run_in_thread():
            try:
                ...
                logger.error(f"❌ [OUTPUT] Ошибка выполнения {operation_name} в thread: {e}", exc_info=True)
                # Возвращаем StreamOperationResult с ошибкой
        ...
        logger.debug(f"🔍 [OUTPUT] Запускаем {operation_name} в отдельном thread (timeout={timeout_sec}с)...")
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(run_in_thread)
            try:
                result = future.result(timeout=timeout_sec)
                logger.debug(f"✅ [OUTPUT] {operation_name} завершен успешно")
                return result
            except concurrent.futures.TimeoutError:
                logger.error(f"❌ [OUTPUT] Таймаут {operation_name} ({timeout_sec}с)")
                return StreamOperationResult(
                    success=False,
    ...
    def _check_and_update_output_device(self) -> bool:
        ...
        # ✅ АУДИТ: Проверяем, не выполняется ли уже проверка устройства
        with self._device_tracking_lock:
            if self._device_check_in_progress:
                logger.debug("🔍 [OUTPUT] Проверка устройства уже выполняется, пропускаем")
                return False
            
            # Проверяем, есть ли результат предыдущей проверки
            if self._device_check_result is not None:
                device_name, is_bluetooth = self._device_check_result
                old_device_name = self.output_device_name
                
                # Очищаем результат (одноразовое использование)
                self._device_check_result = None
                ...
                if device_name:
                    ...
                    if old_device_name is None:
                        logger.info(f"🔊 [OUTPUT] Начальное устройство: \"{device_name}\" (BT={is_bluetooth})")
                        return False
                    
                    if device_name != old_device_name:
                        logger.info(
                            f"🔄 [OUTPUT] Смена устройства: \"{old_device_name}\" → \"{device_name}\" (BT={is_bluetooth})"
                        )
                        return True
                    
                    logger.debug(f"✅ [OUTPUT] Устройство не изменилось: \"{device_name}\" (BT={is_bluetooth})")
                else:
                    logger.debug("⚠️ [OUTPUT] SwitchAudioSource не вернул имя устройства")
                
                return False
        
        # ✅ АУДИТ: Запускаем асинхронную задачу для запроса устройства
        try:
            logger.debug("🔍 [OUTPUT] Запуск асинхронной проверки устройства...")
            ...
            try:
                ...
                def run_async_check():
                    ...
                    asyncio.set_event_loop(new_loop)
                    try:
                        new_loop.run_until_complete(self._run_device_check_async())
                    finally:
                        new_loop.close()
                
                thread = threading.Thread(target=run_async_check, daemon=True)
                thread.start()
                return False  # Возвращаем False, так как проверка выполняется асинхронно
            
            # Если event loop существует, создаем задачу
            if loop and loop.is_running():
                loop.create_task(self._run_device_check_async())
            else:
                # Если loop не запущен, запускаем синхронно (fallback)
                asyncio.run(self._run_device_check_async())
            
            return False  # Возвращаем False, так как проверка выполняется асинхронно
            
        except Exception as e:
            logger.warning(f"⚠️ [OUTPUT] Ошибка запуска проверки устройства: {e}")
            with self._device_tracking_lock:
                self._device_check_in_progress = False
            return False
    
    async def _run_device_check_async(self):
        """
        Асинхронная функция для выполнения проверки устройства.
        
        ✅ АУДИТ: Выполняется в фоновом потоке, не блокирует callback
        """
        try:
            logger.debug("🔍 [OUTPUT] Начало асинхронной проверки устройства...")
            
            # Запрашиваем устройство асинхронно
            device_name, is_bluetooth = await self._async_fetch_default_output_device()
            
            # Сохраняем результат
            with self._device_tracking_lock:
                self._device_check_result = (device_name, is_bluetooth)
                self._device_check_in_progress = False
            
            logger.debug(f"✅ [OUTPUT] Асинхронная проверка устройства завершена: \"{device_name}\" (BT={is_bluetooth})")
            
        except Exception as e:
            logger.warning(f"⚠️ [OUTPUT] Ошибка асинхронной проверки устройства: {e}")
            with self._device_tracking_lock:
                self._device_check_in_progress = False
                self._device_check_result = None
                ...
                try:
                    ...
                    if hasattr(default_setting, '__getitem__'):
                        ...
                        if output_device_id is not None:
                            # Получаем имя устройства только для логов
                            try:
                                device_info = sd.query_devices(output_device_id, 'output')
                                device_name = device_info.get('name', 'Unknown') if device_info else 'Unknown'
                                
                                # Проверяем, является ли устройство BT (для логов)
                                is_bluetooth = self._is_bluetooth_device(device_name) if device_name != 'Unknown' else False
                                ...
                                # Для BT устройств возвращаем device_id=None (macOS управляет параметрами)
                                if is_bluetooth:
                                    logger.debug(f"💡 [OUTPUT][V2] BT устройство - используем device_id=None")
                                    return device_name, None
                                
                                return device_name, output_device_id
                            except Exception as e:
                                logger.warning(f"⚠️ [OUTPUT][V2] Ошибка получения имени устройства для ID {output_device_id}: {e}")
                                # Возвращаем ID даже если не удалось получить имя
                                return None, output_device_id
                        else:
    ...
    def _get_output_device_name_via_macos_api_sync(self) -> Optional[str]:
        """
        Синхронная версия получения имени устройства через macOS API (SwitchAudioSource).
        
        ✅ АУДИТ: Вынесена в отдельную функцию для использования в asyncio.to_thread
        ...
        try:
            ...
            logger.debug(f"🔍 [SwitchAudioSource] Запрос текущего default output устройства...")
            ...
            logger.debug(f"🔍 [SwitchAudioSource] Команда выполнена: returncode={result.returncode}, stdout_len={len(result.stdout)}, stderr_len={len(result.stderr)}")
            
            if result.returncode == 0:
                try:
                    device_info = json.loads(result.stdout.strip())
                    device_name = device_info.get('name', '')
                    logger.debug(f"🔍 [SwitchAudioSource] Получен JSON: {device_info}")
                    if device_name:
                        logger.info(f"✅ [OUTPUT] macOS default OUTPUT (через SwitchAudioSource): \"{device_name}\"")
                        logger.debug(f"🔍 [SwitchAudioSource] Успешно получено имя устройства: {device_name}")
                        return device_name
                    else:
                        logger.warning("⚠️ [OUTPUT] SwitchAudioSource вернул пустое имя устройства")
                        logger.debug(f"🔍 [SwitchAudioSource] JSON не содержит 'name': {device_info}")
                except json.JSONDecodeError as e:
                    logger.warning(f"⚠️ [OUTPUT] Ошибка парсинга JSON от SwitchAudioSource: {e}, stdout: {result.stdout[:100]}")
                    logger.debug(f"🔍 [SwitchAudioSource] Полный stdout: {result.stdout}")
            else:
                # ✅ АУДИТ: Логируем ошибку без ожидания команды
                logger.warning(f"⚠️ [OUTPUT] SwitchAudioSource вернул код ошибки {result.returncode}: {result.stderr[:200]}")
                logger.debug(f"🔍 [SwitchAudioSource] Полный stderr: {result.stderr}")
                return None  # ✅ АУДИТ: Возвращаем None сразу, не ждем команду
    ...
    async def _async_fetch_default_output_device(self) -> tuple[Optional[str], bool]:
        """
        Асинхронная функция для получения имени устройства через macOS API.
        
        ✅ АУДИТ: Запускает синхронный SwitchAudioSource в отдельном потоке через asyncio.to_thread
        
        Returns:
            tuple: (device_name, is_bluetooth) или (None, False) если не удалось получить
        """
        try:
            logger.debug("🔍 [OUTPUT] Запуск асинхронного запроса устройства через SwitchAudioSource...")
            
            # ✅ АУДИТ: Запускаем синхронный вызов в отдельном потоке
            loop = asyncio.get_event_loop()
            device_name = await loop.run_in_executor(
                None,  # Используем default executor (ThreadPoolExecutor)
                self._get_output_device_name_via_macos_api_sync
            )
            
            if device_name:
                is_bluetooth = self._is_bluetooth_device(device_name)
                logger.debug(f"✅ [OUTPUT] Асинхронный запрос завершен: \"{device_name}\" (BT={is_bluetooth})")
                return device_name, is_bluetooth
            else:
                logger.debug("⚠️ [OUTPUT] Асинхронный запрос вернул None")
                return None, False
                
        except Exception as e:
            logger.warning(f"⚠️ [OUTPUT] Ошибка асинхронного запроса устройства: {e}")
            return None, False
    ...
    def _get_output_device_name_via_macos_api(self) -> Optional[str]:
        ...
        ✅ АУДИТ: Теперь использует синхронную версию (для обратной совместимости)
        
        Returns:
            Имя устройства или None если не удалось получить
        """
        return self._get_output_device_name_via_macos_api_sync()
    ...
    def _start_output_monitoring(self):
        ...
        Мониторинг устройств теперь происходит через события device.default_output_changed
        от DeviceChangePublisher, который использует Core Audio нотификации или polling fallback.
        """
        logger.debug("ℹ️ [OUTPUT] _start_output_monitoring отключен - используем DeviceChangePublisher через EventBus")
        # Старый код закомментирован для обратной совместимости
    ...
    def _stop_output_monitoring(self):
        ...
        Мониторинг устройств теперь происходит через события device.default_output_changed
        от DeviceChangePublisher, который управляет своим lifecycle.
        """
        logger.debug("ℹ️ [OUTPUT] _stop_output_monitoring отключен - DeviceChangePublisher управляет мониторингом")
        # Старый код закомментирован для обратной совместимости
    ...
    # ✅ ЦИКЛ 3: _output_monitor_loop удален - используем DeviceChangePublisher через EventBus
    
    def _build_stream_config_for_output_device(self, device_name: str, device_id: Optional[int], is_bluetooth: bool = False) -> StreamConfig:
        """
        ✅ ЦИКЛ 2: Создает StreamConfig для OUTPUT устройства с использованием кэша успешных конфигураций
        ...
        """
        # ✅ ЦИКЛ 2: Проверяем кэш успешных конфигураций
        ...
        with self._successful_config_lock:
            if cache_key in self._last_successful_config:
                ...
                logger.info(
                    f"✅ [OUTPUT] Используем кэшированную конфигурацию для \"{device_name}\" "
                    f"(BT={is_bluetooth}, rate={cached_config.samplerate}Hz, channels={cached_config.channels})"
                )
                # Обновляем device_id в кэшированной конфигурации (может измениться)
                cached_config.device_id = device_id
                return cached_config
        
        # Если кэша нет, создаем новую конфигурацию
        logger.debug(f"🔍 [OUTPUT] Создаем новую конфигурацию для \"{device_name}\" (кэш не найден)")
        
        # Получаем информацию об устройстве для определения параметров
        device_info = {}
        if not is_bluetooth and device_id is not None:
            try:
                device_info = sd.query_devices(device_id, 'output')
            except Exception as e:
        ...
        channels_available = int(device_info.get('max_output_channels') or 2)
        channels_target = max(1, self.config.channels)
        
        # Нормализация параметров (если доступен нормализатор)
        if device_info and self._params_normalizer and not is_bluetooth:
            try:
                normalized = self._params_normalizer.select_output_params(device_info)
                actual_rate = normalized.sample_rate
            ...
            if is_bluetooth:
                actual_rate = float(samplerate)
                actual_channels = 1  # BT устройства предпочитают моно
            else:
                actual_rate = float(samplerate)
                actual_channels = max(1, min(channels_available, channels_target))
        
        # Получаем безопасную конфигурацию с учетом истории ошибок
        safe_config = self._get_safe_stream_config(device_name, is_bluetooth, device_id)
        
        return StreamConfig(
            device_id=device_id if not is_bluetooth else None,  # BT устройства всегда используют device=None
            device_name=device_name,
            samplerate=int(actual_rate),
            channels=actual_channels,
            dtype=self.config.dtype,
            blocksize=safe_config.get('blocksize', self.config.buffer_size),
            latency=safe_config.get('latency'),
    ...
    def _switch_output_device(self, new_name: str, new_id: Optional[int], is_bluetooth: bool = None):
        """
        ✅ ЦИКЛ 1: Безопасно переключает output устройство через AudioStreamManager.switch_device()
        
        Защита от concurrent переключений:
        - Guard (_switch_in_progress) предотвращает параллельные вызовы
        - Таймаут guard (10s) защищает от залипания
        - Обработка отмены при быстром изменении устройства
        ...
        """
        sequence_id = (self._switch_sequence_counter + 1) % 100000
        self._switch_sequence_counter = sequence_id
        self._last_switch_sequence_id = sequence_id
        start_time = time.time()
        logger.debug(f"🔢 [OUTPUT][seq={sequence_id}] Прогон _switch_output_device начат")
        
        # ✅ ЦИКЛ 1: Guard защита - атомарная проверка и установка
        with self._switch_in_progress_lock:
            if self._switch_in_progress:
                # Проверяем таймаут guard (защита от залипания)
                if self._switch_in_progress_start_time:
                    ...
                    if elapsed > self._switch_in_progress_timeout:
                        logger.error(
                            f"❌ [OUTPUT] Guard залип на {elapsed:.1f}s > {self._switch_in_progress_timeout}s, "
                            f"принудительно сбрасываем"
                        )
                        self._switch_in_progress = False
                        self._switch_in_progress_start_time = None
                    else:
                        logger.warning(
                            f"⚠️ [OUTPUT] Переключение уже выполняется (прошло {elapsed:.1f}s), "
                            f"игнорируем новое событие для \"{new_name}\""
                        )
                        return
            
            # Проверяем, не изменилось ли устройство снова (отмена переключения)
            if self._pending_switch_device_name and new_name != self._pending_switch_device_name:
                logger.warning(
                    f"⚠️ [OUTPUT] Устройство изменилось во время переключения: "
                    f"\"{self._pending_switch_device_name}\" → \"{new_name}\" (отменяем текущее)"
                )
                self._switch_cancelled = True
                # Не устанавливаем guard - новое переключение будет обработано следующим вызовом
                return
        ...
        try:
            ...
            logger.info(
                f"🔄 [OUTPUT][seq={sequence_id}] Начало переключения устройства через AudioStreamManager: "
            ...
            # 2. Дожидаемся завершения playback thread
            if self._playback_thread and self._playback_thread.is_alive():
                logger.debug("🔄 [OUTPUT] Ожидаем завершения playback thread")
                self._playback_thread.join(timeout=1.0)
            
            # 3. ✅ КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Гарантированно закрываем старый поток перед переключением
            # Определяем тип старого устройства для правильной задержки при закрытии
            old_is_bluetooth_for_close = old_is_bluetooth if old_is_bluetooth is not None else False
            if old_name:
                # Если тип не был определен, пытаемся определить автоматически
                if old_is_bluetooth_for_close is False:
                    old_is_bluetooth_for_close = self._is_bluetooth_device(old_name)
            
            logger.info(
                f"🛑 [OUTPUT][seq={sequence_id}] Закрываем старый поток перед переключением: "
                f"\"{old_name}\" (BT={old_is_bluetooth_for_close})"
            )
            
            # Вызываем _stop_audio_stream с ожиданием завершения
            self._stop_audio_stream(is_bluetooth=old_is_bluetooth_for_close)
            
            # ✅ ПРОВЕРКА: Убеждаемся, что поток действительно закрыт
            with self._stream_lock:
                if self._audio_stream is not None:
                    logger.error(
                        f"❌ [OUTPUT] КРИТИЧЕСКАЯ ОШИБКА: Поток не был закрыт после _stop_audio_stream! "
                        f"_audio_stream={self._audio_stream}"
                    )
                    # Принудительно очищаем ссылку
                    self._audio_stream = None
                    self._stream_started = False
            ...
            # 4. Очищаем буфер после закрытия потока
            self.chunk_buffer.clear_all()
            logger.debug(f"🔄 [OUTPUT][seq={sequence_id}] Буфер очищен после закрытия потока")
            
            # 5. ✅ РЕФАКТОРИНГ: Получаем информацию о новом устройстве для нормализации параметров
            device_info = {}
            ...
            if self._params_normalizer and device_info and not is_bluetooth:
                try:
            ...
            # 7. ✅ РЕФАКТОРИНГ: Создаем StreamConfig для нового устройства
            stream_config = self._build_stream_config_for_output_device(new_name, new_id, is_bluetooth)
            
            # 8. ✅ ПРОВЕРКА: После закрытия старого потока он должен быть None
            with self._stream_lock:
                old_stream = self._audio_stream
            
            if old_stream is not None:
                logger.error(
                    f"❌ [OUTPUT] КРИТИЧЕСКАЯ ОШИБКА: Поток не был закрыт! "
                    f"_audio_stream={old_stream}, принудительно очищаем"
                )
                with self._stream_lock:
                    self._audio_stream = None
                    self._stream_started = False
            ...
            # ✅ ЦИКЛ 1: Проверяем отмену перед началом переключения
            if self._switch_cancelled:
                logger.info("ℹ️ [OUTPUT] Переключение отменено перед началом, новое устройство уже обрабатывается")
                return
            
            # 9. ✅ ЦИКЛ 1: Создаем новый поток через AudioStreamManager (старый уже закрыт)
            # После гарантированного закрытия старого потока всегда создаем новый
            timeout_sec = 5.0 if is_bluetooth else 3.0  # Уменьшено с 10.0/5.0 для быстрого отклика
            max_retries = 2  # Уменьшено с 5 для быстрого fallback
            
            logger.info(
                f"🔍 [OUTPUT][seq={sequence_id}] Создаем новый поток через AudioStreamManager.create_stream "
                f"(старый поток гарантированно закрыт, _audio_stream=None)"
            )
            result = self._run_async_in_thread(
                self._stream_manager.create_stream(stream_config, max_retries=max_retries),
                timeout_sec,
                "create_stream (switch_device)"
            )
            
            # ✅ ЦИКЛ 1: Проверяем отмену перед финальным обновлением
            if self._switch_cancelled:
            ...
            if result.success and result.stream:
                ...
                self.config.sample_rate = stream_config.samplerate
                self.config.channels = stream_config.channels
                
                # ✅ ЦИКЛ 2: Сохраняем успешную конфигурацию в кэш
                cache_key = f"{new_name}|{is_bluetooth}"
                with self._successful_config_lock:
                    self._last_successful_config[cache_key] = stream_config
                    logger.debug(f"✅ [OUTPUT] Конфигурация сохранена в кэш: {cache_key}")
                
                # ✅ ЦИКЛ 3: Записываем метрики времени переключения
                ...
                avg_time = sum(self._switch_device_times) / len(self._switch_device_times) if self._switch_device_times else 0.0
                
                # ✅ ЦИКЛ 3: Decision-лог в каноническом формате
                logger.info(
                    f"decision=switch_device_success seq={sequence_id} "
                    f"ctx={{device={new_name},bt={is_bluetooth},old_device={old_name}}} "
                    f"source=output_device_change "
                    f"duration_ms={duration*1000:.1f} "
                    f"avg_duration_ms={avg_time*1000:.1f}"
                )
                
                # Сбрасываем _stop_event для следующего воспроизведения
                self._stop_event.clear()
                
                logger.info(
                    f"✅ [OUTPUT] Устройство успешно переключено через AudioStreamManager: \"{new_name}\" "
                    f"(ID={new_id}, rate={stream_config.samplerate}Hz, channels={stream_config.channels}, "
                    f"attempt: {result.attempt}, duration: {result.duration_ms:.1f}ms, total: {duration:.2f}s)"
                )
                
                # Предупреждение при долгом переключении
                if duration > 3.0:
                    logger.warning(f"⚠️ [OUTPUT] Долгое переключение: {duration:.2f}s (цель: <3s)")
            else:
                # ✅ ЦИКЛ 3: Decision-лог для ошибки
                duration = time.time() - start_time
                logger.error(
                    f"decision=switch_device_failed seq={sequence_id} "
                    f"ctx={{device={new_name},bt={is_bluetooth},old_device={old_name},error={result.error_code}}} "
                    f"source=output_device_change "
                    f"duration_ms={duration*1000:.1f}"
                )
                
                logger.error(
                    f"❌ [OUTPUT] Не удалось переключить устройство через AudioStreamManager: {result.error_message} "
            ...
            duration = time.time() - start_time
            logger.error(
                f"decision=switch_device_exception seq={sequence_id} "
                f"ctx={{device={new_name},bt={is_bluetooth},error={str(e)}}} "
                f"source=output_device_change "
                f"duration_ms={duration*1000:.1f}"
            )
            ...
            # ✅ ЦИКЛ 1: ВСЕГДА сбрасываем guard, даже при ошибке
            with self._switch_in_progress_lock:
                self._switch_in_progress = False
                self._switch_in_progress_start_time = None
            ...
            # ✅ ЦИКЛ 1: Дополнительная гарантия очистки _stop_event
            if self._stop_event.is_set():
                logger.warning("⚠️ [OUTPUT] _stop_event все еще установлен в finally, принудительно очищаем")
                self._stop_event.clear()

            guard_state = "clear" if not self._switch_in_progress else "set"
            with self._stream_lock:
                stream_state = "set" if self._audio_stream else "cleared"
            logger.info(
                f"🔚 [OUTPUT][seq={sequence_id}] switch complete → guard={guard_state}, stream={stream_state}"
            )
