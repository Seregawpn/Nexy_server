...
class GrpcClient:
    ...
    async def stream_audio(self, prompt: str, screenshot_base64: str, screen_info: dict, hardware_id: str, session_id: Optional[str] = None) -> AsyncGenerator[Any, None]:
        """Стриминг аудио и текста на сервер
        
        Args:
            prompt: Текст запроса
            screenshot_base64: Скриншот в base64
            screen_info: Информация о экране (width, height)
            hardware_id: ID оборудования
            session_id: ID сессии (опционально, если не указан, сервер создаст новый)
        """
        try:
            ...
            # ⚠️ CRITICAL: Передаем session_id в StreamRequest, чтобы сервер использовал тот же session_id
            # Если session_id не указан, сервер создаст новый с префиксом "session_"
            request = streaming_pb2.StreamRequest(
                ...
                session_id=session_id  # Передаем session_id, если он указан
            )
        ...
        *,
        ...
        try:
            ...
            async for response in stub.GenerateWelcomeAudio(request, timeout=rpc_timeout):
                logger.debug(f"🔍 [WELCOME_GRPC] Получен response: {response.WhichOneof('content')}")
    ...
    async def send_mcp_action_result(
        self,
        session_id: str,
        command: str,
        success: bool,
        ...
        """
        Отправляет результат MCP команды на сервер.
        
        Используется для Messages команд (read_messages, send_message), которые выполняются на клиенте.
        Результат отправляется обратно на сервер для обработки через LLM и генерации TTS.
        ...
        Args:
            ...
            result_text: Текст результата от MCP сервера (если success=True)
            error: Текст ошибки (если success=False)
            feature_id: Feature ID для логирования (default: "F-2025-016-messages-integration")
            timeout: Таймаут для RPC (default: 10.0)
            server_name: Имя сервера для отправки (опционально)
            
        Returns:
            Dict с результатом отправки: {"success": bool, "message": str, "error": Optional[str]}
        """
        if not session_id:
        ...
        feature_id = feature_id or "F-2025-016-messages-integration"
        target_server = server_name or self.connection_manager.current_server
        ...
        # Создаем запрос с обязательными полями
        request = streaming_pb2.McpActionResultRequest()
        request.session_id = str(session_id)
        request.command = str(command)
        
        # Устанавливаем success только если поле существует в protobuf
        # В текущей версии protobuf поле success может отсутствовать (нужна регенерация)
        if 'success' in streaming_pb2.McpActionResultRequest.DESCRIPTOR.fields_by_name:
            request.success = bool(success)
        else:
            # Если поля нет, логируем предупреждение
            logger.warning(
                "[%s] McpActionResultRequest.success field not available in protobuf. "
                "Protobuf files need regeneration from streaming.proto. "
                "Continuing without success field.",
                feature_id
            )
        
        # Добавляем опциональные поля только если они не None
        if result_text:
            request.result_text = str(result_text)
        if error:
            request.error = str(error)
        if feature_id:
            request.feature_id = str(feature_id)
        
        # Выполняем RPC вызов
        stub = streaming_pb2_grpc.StreamingServiceStub(self.connection_manager.channel)
        rpc_timeout = timeout or 10.0
        
        logger.info(
            "[%s] Sending MCP result: session=%s, command=%s, success=%s",
            feature_id,
            session_id,
            command,
            success
        ...
        try:
            response = await stub.SendMcpActionResult(request, timeout=rpc_timeout)
            
            logger.info(
                "[%s] MCP result sent successfully: session=%s, server_success=%s",
                feature_id,
                session_id,
                response.success
            )
            ...
            # Проверяем наличие поля error перед чтением (может отсутствовать если protobuf не перегенерирован)
            if 'error' in streaming_pb2.McpActionResultResponse.DESCRIPTOR.fields_by_name:
                result["error"] = response.error if response.HasField('error') else None
            else:
                result["error"] = None
            return result
            ...
            return {
                "success": False,
                "message": f"Error sending MCP result: {str(e)}",
                "error": str(e),
            }
