            ...
            try:
                with self.state_lock:
                    ...
                    if is_active and start_time:
                        ...
                        # ✅ ДИАГНОСТИКА: Логируем прогресс к LONG_PRESS каждые 0.1 секунды
                        if duration > 0.1 and duration < self.long_press_threshold:
                            # Логируем каждые 0.1 секунды (0.1, 0.2, 0.3, ...)
                            if abs(duration % 0.1) < 0.01 or duration >= self.long_press_threshold - 0.05:
                                logger.debug(f"🔍 HOLD_MONITOR: удержание {duration:.2f}s / {self.long_press_threshold}s (is_active={is_active}, _long_sent={self._long_sent})")
                        ...
                        if not self._long_sent and duration >= self.long_press_threshold:
                            ...
                            if not is_still_active or not start_time:
                                logger.warning(f"⚠️ HOLD_MONITOR: комбинация/клавиша была отпущена во время проверки LONG_PRESS (duration={duration:.3f}s), пропускаем")
                                continue
                            ...
                            logger.info(f"✅ HOLD_MONITOR: LONG_PRESS событие отправлено через _trigger_event")
