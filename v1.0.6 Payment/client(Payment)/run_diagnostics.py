...
from diagnostic_browser_use import BrowserUseDiagnostic
...
class MasterDiagnostic:
    ...
    async def run_all_diagnostics(self) -> Dict[str, Any]:
        ...
        try:
            browser_use_diagnostic = BrowserUseDiagnostic()
            browser_use_results = await browser_use_diagnostic.run_diagnostic()
            self.results['browser_use'] = browser_use_results
        except Exception as e:
            logger.error(f"❌ Ошибка тестирования Browser Use: {e}")
            self.results['browser_use'] = {"error": str(e), "success": False}
