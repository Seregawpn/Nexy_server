...
| `device_switch_v2` | Feature Flag | `unified_config.yaml: features.device_switch_v2.enabled` | `modules/speech_playback/core/player.py` | `_query_system_default_output()` | `false` | Упрощенная логика переключения устройств без SwitchAudioSource (Фаза 1-2 рефакторинга) |
| `device_switch_v2` | Feature Flag | `unified_config.yaml: features.device_switch_v2.enabled` | `modules/voice_recognition/core/speech_recognizer.py` | `_get_system_default_input_index()` | `false` | Упрощенная логика переключения устройств без SwitchAudioSource (Фаза 1-2 рефакторинга) |
