...
class GrpcClientIntegration:
    ...
    async def initialize(self) -> bool:
        try:
            ...
            # ⚠️ NEW: Подписка на событие результатов MCP действий (Messages команды)
            await self.event_bus.subscribe("mcp.action.result", self._on_mcp_action_result, EventPriority.HIGH)
    ...
    # ---------------- Event handlers ----------------
    def _normalize_session_id(self, session_id) -> Optional[str]:
        """
        Normalize session_id to ensure it has 'session_' prefix.
        
        This ensures consistent session_id format across the entire flow:
        - Voice recognition → grpc_client_integration → server → MCP results
        
        Args:
            session_id: Original session_id (may be str, float, int, or None)
            
        Returns:
            Normalized session_id with 'session_' prefix, or None if input is None/empty
        """
        if session_id is None:
            return None
        
        # Convert to string if not already (handles float/int from voice recognition)
        if not isinstance(session_id, str):
            session_id = str(session_id)
        
        # Empty string after conversion means invalid input
        if not session_id:
            return None
        
        # If already has prefix, return as-is (avoid double prefix)
    ...
    async def _on_voice_completed(self, event):
        try:
            ...
            # ⚠️ CRITICAL: Normalize session_id once when receiving from event
            # This ensures consistent format throughout the entire flow
            normalized_sid = self._normalize_session_id(sid)
            if not normalized_sid:
                logger.warning("[%s] Received empty session_id from voice.recognition_completed", FEATURE_ID)
                return
            
            # Store normalized session_id in _sessions
            sess = self._sessions.setdefault(normalized_sid, {})
            sess['text'] = text
            await self._maybe_send(normalized_sid)
        except Exception as e:
    ...
    async def _on_screenshot_captured(self, event):
        try:
            ...
            # ⚠️ CRITICAL: Normalize session_id to match voice recognition format
            normalized_sid = self._normalize_session_id(sid)
            if not normalized_sid:
                logger.warning("[%s] Received empty session_id from screenshot.captured", FEATURE_ID)
                return
    ...
    async def _on_mcp_action_result(self, event):
        """
        Обрабатывает результат выполнения MCP действия (Messages команды).
        
        Отправляет результат обратно на сервер через send_mcp_action_result RPC.
        
        Args:
            event: Событие с данными:
                - session_id: ID сессии (REQUIRED)
                - command: Команда ("read_messages" или "send_message")
                - success: Успешно ли выполнена команда
                - result_text: Текст результата (если success=True)
        ...
        try:
            ...
            feature_id = data.get("feature_id", "F-2025-016-messages-integration")
            
            if not session_id:
                logger.warning(
                    "[%s] mcp.action.result event missing session_id",
                    feature_id
                )
                return
            ...
            if not command:
                logger.warning(
                    "[%s] mcp.action.result event missing command",
                    feature_id
                )
                return
            
            if command not in ["read_messages", "send_message"]:
                logger.warning(
                    "[%s] mcp.action.result event with unsupported command: %s",
                    feature_id,
                    command
                )
                return
            ...
            if not self._client:
                logger.warning(
                    "[%s] GrpcClient not available, cannot send MCP result: command=%s, session=%s",
                    feature_id,
                    command,
                    session_id
                )
            ...
            # ⚠️ CRITICAL: Normalize session_id to ensure it matches the one used in StreamRequest
            # This ensures MCP results can be matched with the correct active stream on server
            normalized_sid = self._normalize_session_id(session_id)
            if not normalized_sid:
                logger.error(
                    "[%s] Cannot send MCP result: no session_id provided (command=%s)",
                    feature_id,
                    command
                )
                return
            ...
            logger.info(
                "[%s] Sending MCP result to server: command=%s, session=%s (normalized from %s), success=%s",
                feature_id,
                command,
                normalized_sid,
                session_id,
            ...
            try:
                response = await self._client.send_mcp_action_result(
                    session_id=normalized_sid,  # Use normalized session_id to match server's active stream
                    command=command,
                    success=success,
                ...
                if response.get("success"):
                    logger.info(
                        "[%s] MCP result sent successfully: command=%s, session=%s",
                        feature_id,
                        command,
                        session_id
                    )
                    ...
                    logger.error(
                        "[%s] Failed to send MCP result: command=%s, session=%s, error=%s",
                        feature_id,
                        command,
                        session_id,
                ...
                # Прерывание во время отправки результата
                logger.info(
                    "[%s] MCP result sending cancelled: command=%s, session=%s",
                    feature_id,
                    command,
                    session_id
                )
                ...
                logger.error(
                    "[%s] Error sending MCP result to server: %s, command=%s, session=%s",
                    feature_id,
                    e,
                    command,
                    session_id,
            ...
            await self._handle_error(e, where="grpc.on_mcp_action_result", severity="warning")
    ...
    async def _send(self, session_id):
        ...
        try:
            ...
            got_terminal = False
            chunk_count = 0
            # ⚠️ CRITICAL: session_id is already normalized in _on_voice_completed/_on_screenshot_captured
            # No need to add prefix again - use as-is to ensure exact match with server
            logger.debug(
                "[%s] Sending StreamRequest with normalized session_id: %s",
                FEATURE_ID,
                session_id
            )
            ...
            async for resp in self._client.stream_audio(
                ...
                hardware_id=hwid,
                session_id=session_id,  # Already normalized, use as-is
            ):
                chunk_count += 1
                ...
                if which_oneof == 'text_chunk':
                    ...
                    # Обработка browser-use прогресса (Цикл 4)
                    from modules.grpc_client.proto import streaming_pb2
                    
                    progress = resp.browser_progress
                    feature_id = "F-2025-015-browser-use"
                    
                    try:
                        # ⚠️ CRITICAL: Проверяем наличие обязательных полей перед доступом
                        if not hasattr(progress, 'type') or not hasattr(progress, 'task_id'):
                            logger.error(
                                f"[{feature_id}] BrowserProgressMessage missing required fields (type or task_id). "
                                f"Available fields: {[f for f in dir(progress) if not f.startswith('_')]}"
                            )
                            # Fallback: пытаемся извлечь description как текст
                            if hasattr(progress, 'description') and progress.description:
                        ...
                        # Извлекаем данные из BrowserProgressMessage
                        ...
                        url = progress.url if progress.HasField('url') else None
                        action = progress.action if progress.HasField('action') else None
                        # timestamp - примитивный тип int64 (REQUIRED), не поддерживает HasField в protobuf 3
                        # Поле всегда присутствует, читаем напрямую
                        timestamp = progress.timestamp
                        error = progress.error if progress.HasField('error') else None
                        
                        logger.info(
                            f"[{feature_id}] Browser progress: type={event_type}, task_id={task_id}, "
                            f"step={step_number}, description={description[:50] if description else None}"
                        )
                        
                        # Публикуем событие browser.progress с полными данными
                        ...
                        # Если есть описание, отправляем его в TTS для озвучивания
                        if description:
                            await self.event_bus.publish("grpc.response.text", {
                                "session_id": session_id,
                                "text": description,
                        ...
                        # Если это терминальное событие (COMPLETED, FAILED, CANCELLED), завершаем стрим
                        ...
                        if event_type in (BROWSER_TASK_COMPLETED, BROWSER_TASK_FAILED, BROWSER_TASK_CANCELLED):
                            if event_type == BROWSER_TASK_FAILED:
                                await self.event_bus.publish("grpc.request_failed", {
                                    "session_id": session_id,
                                    "error": error or "Browser task failed"
                                })
                            else:
                        ...
                        logger.error(
                            f"[{feature_id}] AttributeError accessing BrowserProgressMessage: {attr_error}. "
                            f"Proto definition may be outdated. Available fields: {[f for f in dir(progress) if not f.startswith('_')]}"
                        )
                        # Fallback: try to extract description as text
                        try:
                            if hasattr(progress, 'description') and progress.description:
                                await self.event_bus.publish("grpc.response.text", {
                                    "session_id": session_id,
                            ...
                            logger.warning(f"[{feature_id}] Fallback text extraction failed: {fallback_error}")
                        continue
                    except Exception as e:
                        logger.error(f"[{feature_id}] Error processing browser_progress: {e}", exc_info=True)
                        continue
