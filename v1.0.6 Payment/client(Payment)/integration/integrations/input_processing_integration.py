...
class InputProcessingIntegration:
    ...
    def __init__(self, event_bus: EventBus, state_manager: ApplicationStateManager, 
        ...
        # ✅ АУДИТ: Флаг для предотвращения дублирования voice.recording_stop
        self._recording_stop_sent: bool = False
    ...
    async def _handle_press(self, event: KeyEvent):
        ...
        try:
            ...
            self._recording_stop_sent = False  # ✅ АУДИТ: Сбрасываем флаг при старте новой сессии
    ...
    def _reset_session(self, reason: str):
        ...
        self._session_recognized = False
        self._recording_started = False
        self._recording_stop_sent = False  # ✅ АУДИТ: Сбрасываем флаг при сбросе сессии
    ...
    def _is_recording_active(self) -> bool:
        ...
        return self._recording_started or self.state_manager.is_microphone_active()
    ...
    async def _handle_short_press(self, event: KeyEvent):
        ...
        try:
            ...
            if self._recording_started and active_session_id is not None:
                ...
                # ✅ АУДИТ: Проверяем, не было ли уже отправлено voice.recording_stop
                if self._recording_stop_sent:
                    logger.warning(f"⚠️ SHORT_PRESS: voice.recording_stop уже отправлено, пропускаем дублирование")
                    return
                ...
                self._recording_stop_sent = True  # ✅ АУДИТ: Устанавливаем флаг после публикации
    ...
    async def _handle_long_press(self, event: KeyEvent):
        ...
        try:
            ...
            try:
                ...
                # ✅ ЭТАП 0.1: Проверяем состояние через централизованное состояние
                async with self._state_lock:
                    if self._input_state != InputState.PENDING:
                        logger.warning(f"⚠️ LONG_PRESS пришел в неправильном состоянии: {self._input_state.name}, ожидалось PENDING")
                        return
                    
                    # ЗАЩИТА 3: Проверяем, что клавиша ЕЩЕ нажата (атомарно)
                    ...
                    # ✅ ЭТАП 1: ЗАЩИТА 4: Проверяем, что микрофон НЕ активен (защита от повторных LONG_PRESS)
                    if self.state_manager.is_microphone_active():
                        logger.warning(f"⚠️ LONG_PRESS пришел, но микрофон УЖЕ активен - игнорируем повторную активацию")
                        active_session_id = self._get_active_session_id()
                    ...
                    if self._recording_started:
                        ...
                        active_session_id = self._get_active_session_id()
                        logger.warning(f"⚠️ LONG_PRESS: mic_active={self.state_manager.is_microphone_active()}, active_session_id={active_session_id}")
                        return
                ...
                try:
                    await asyncio.wait_for(self._wait_for_mic_closed(), timeout=0.3)
                    logger.debug("✅ LONG_PRESS: Микрофон закрыт")
                except asyncio.TimeoutError:
                    logger.warning("⚠️ LONG_PRESS: таймаут ожидания закрытия микрофона (0.3s), принудительно сбрасываем состояние")
                    # ✅ ЭТАП 1: Принудительно сбрасываем состояние микрофона через state_manager
                    if self.state_manager.is_microphone_active():
                ...
                self._pending_session_id = None
                self._cancel_session_id = None
                # ✅ ЭТАП 0.4: Сбрасываем asyncio.Event (уже сброшен в _reset_session)
    ...
    async def _handle_key_release(self, event: KeyEvent):
        ...
        try:
            ...
            if should_stop_recording:
                ...
                # ✅ АУДИТ: Проверяем, не было ли уже отправлено voice.recording_stop
                if self._recording_stop_sent:
                    logger.warning(f"⚠️ RELEASE: voice.recording_stop уже отправлено, пропускаем дублирование")
                else:
                ...
                # ✅ ЭТАП 2: Уменьшенный таймаут для быстрого отклика
                try:
                    await asyncio.wait_for(self._wait_for_mic_closed(), timeout=0.5)
                    logger.debug("✅ RELEASE: Микрофон закрыт")
                except asyncio.TimeoutError:
                    logger.warning("⚠️ RELEASE: таймаут ожидания закрытия микрофона (0.5s), принудительно сбрасываем состояние")
                    # ✅ ЭТАП 1: Принудительно сбрасываем состояние микрофона через state_manager
                    if self.state_manager.is_microphone_active():
