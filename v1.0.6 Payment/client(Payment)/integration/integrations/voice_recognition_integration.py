...
class VoiceRecognitionIntegration:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # ✅ ЦИКЛ 1: Подписка на события смены INPUT устройства от DeviceChangePublisher
            await self.event_bus.subscribe("device.default_input_changed", self._on_input_device_changed, EventPriority.MEDIUM)
    ...
    async def _on_recording_stop(self, event: Dict[str, Any]):
        try:
            ...
            if not self.config.simulate and self._recognizer is not None:
                # ✅ АУДИТ: Проверяем состояние распознавателя перед вызовом stop_listening
                is_listening = getattr(self._recognizer, 'is_listening', False)
                stream_active = False
                if hasattr(self._recognizer, '_current_stream') and self._recognizer._current_stream:
                    try:
                        with getattr(self._recognizer, '_stream_lock', threading.RLock()):
                            stream_active = self._recognizer._current_stream.active if self._recognizer._current_stream else False
                    except Exception:
                        pass
                
                # ✅ АУДИТ: Если распознаватель не активен, пропускаем вызов stop_listening
                if not is_listening and not stream_active:
                    logger.warning(f"⚠️ VOICE: Распознаватель не активен (is_listening={is_listening}, stream_active={stream_active}), пропускаем stop_listening")
                    # Публикуем microphone.closed для завершения цепочки
                    await self.event_bus.publish("microphone.closed", {"session_id": session_id})
                    return
    ...
    async def _on_microphone_open_requested(self, event: Dict[str, Any]):
        ...
        try:
            ...
            if not self.config.simulate and self._recognizer is not None:
                # ✅ КРИТИЧНО: Проверяем РЕАЛЬНОЕ физическое состояние потока, а не только флаги
                # Проблема: MicrophoneStateManager может считать микрофон открытым, но поток не активен
                is_listening = getattr(self._recognizer, 'is_listening', False)
                recognizer_state = getattr(self._recognizer, 'state', None)
                
                # ✅ КРИТИЧНО: Проверяем физическое состояние потока
                stream_active = False
                stream_exists = False
                if hasattr(self._recognizer, '_current_stream') and self._recognizer._current_stream:
                    stream_exists = True
                ...
                # ✅ КРИТИЧНО: Если поток существует, но не активен - это рассинхронизация, нужно перезапустить
                if stream_exists and not stream_active:
                    logger.warning(f"⚠️ [MIC_STATE] Обнаружена рассинхронизация: поток существует, но не активен (is_listening={is_listening}, stream_active={stream_active}, state={recognizer_state})")
                    logger.info(f"🔄 [MIC_STATE] Принудительно закрываем неактивный поток и перезапускаем для session {session_id}")
                    # Закрываем неактивный поток
                    try:
                        with getattr(self._recognizer, '_stream_lock', threading.RLock()):
                            if self._recognizer._current_stream:
                                self._recognizer._current_stream.stop()
                                self._recognizer._current_stream.close()
                                self._recognizer._current_stream = None
                                logger.info("✅ [MIC_STATE] Неактивный поток закрыт")
                    except Exception as e:
                        logger.warning(f"⚠️ [MIC_STATE] Ошибка закрытия неактивного потока: {e}")
                    # Сбрасываем флаги
                    self._recognizer.is_listening = False
                    # Продолжаем запуск start_listening()
                elif is_listening and stream_active:
                    # ✅ Микрофон действительно активен - пропускаем запуск
                    logger.info(f"✅ [MIC_STATE] Микрофон уже активен (is_listening={is_listening}, stream_active={stream_active}, state={recognizer_state}) для session {session_id}")
                    await self.event_bus.publish("microphone.opened", {"session_id": session_id})
                    return
                elif is_listening and not stream_active:
                    # ✅ Флаг установлен, но поток не активен - рассинхронизация, перезапускаем
                    logger.warning(f"⚠️ [MIC_STATE] Флаг is_listening=True, но поток не активен - рассинхронизация, перезапускаем для session {session_id}")
                    self._recognizer.is_listening = False
                    # Продолжаем запуск start_listening()
    ...
    async def _on_input_device_changed(self, event: Dict[str, Any]):
        """
        ✅ ЦИКЛ 1: Обработчик события смены INPUT устройства от DeviceChangePublisher
        
        Args:
            event: Событие device.default_input_changed с полями:
        ...
        try:
            if not self._recognizer:
                logger.debug("ℹ️ [VOICE] SpeechRecognizer не инициализирован, игнорируем событие смены устройства")
                return
            ...
            logger.info(
                f"🔄 [VOICE] Событие смены INPUT устройства (source={source}): "
                f"\"{old_device_name}\" → \"{device_name}\" (ID: {device_id})"
            )
            
            # ✅ РЕФАКТОРИНГ: Вызываем новый метод on_device_changed через AudioStreamManager
            if hasattr(self._recognizer, 'on_device_changed'):
                is_bluetooth = event.get("is_bluetooth", False)
                # Вызываем async метод on_device_changed
                await self._recognizer.on_device_changed(device_name, device_id, is_bluetooth)
            else:
                logger.warning("⚠️ [VOICE] SpeechRecognizer не имеет метода on_device_changed")
                
        except Exception as e:
            logger.error(f"❌ [VOICE] Ошибка обработки события смены INPUT устройства: {e}", exc_info=True)
