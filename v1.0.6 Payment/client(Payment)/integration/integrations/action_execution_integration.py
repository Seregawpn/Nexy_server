...
class ActionExecutionIntegration(BaseIntegration):
    ...
    def __init__(
        ...
        # McpActionExecutor для close_app и messages (через MCP)
        ...
        messages_mcp = mcp_configs.get('messages', {})  # NEW: Messages MCP конфигурация
        ...
        messages_server_path = messages_mcp.get('server_path', '')  # NEW: Messages server path
        ...
        if messages_server_path and not Path(messages_server_path).is_absolute():
            messages_server_path = str((nexy_root / messages_server_path).resolve())
        ...
        # Messages server path: fallback на Messages/server.py если не указан
        if not messages_server_path:
            messages_server_path = str(nexy_root / "Messages" / "server.py")
        
        mcp_config = McpActionConfig(
            open_app_server_path=open_app_server_path,
            close_app_server_path=close_app_server_path,
            messages_server_path=messages_server_path,  # NEW: Messages server path
    ...
    async def _do_start(self) -> bool:
        ...
        # MVP 9: Разрешаем запуск интеграции даже если open_app отключен
        # Это необходимо для поддержки open_url команды (платежная система)
        # Проверяем только для open_app команд, open_url работает всегда
        if not self._executor.is_enabled():
            logger.info("[%s] Action executor disabled (open_app), but integration will start for open_url support", FEATURE_ID)
    ...
    async def _on_action_received(self, event: Dict[str, Any]):
        ...
        try:
            ...
            # ⚠️ CRITICAL: Используем session_id из события (gRPC стрима), а не из action_data
            # action_data может содержать другой session_id (из LLM), который не соответствует активному стриму
            # Сервер ожидает session_id в формате "session_<timestamp>", который приходит из gRPC стрима
            if "session_id" in action_data:
                logger.debug(
                    "[%s] action_data contains session_id=%s, but using session_id from gRPC stream=%s",
                    feature_id,
        ...
        action_type = command  # "open_app", "close_app", "open_url", etc.
        
        # MVP 9: Проверяем executor только для open_app команд
        # open_url должен работать даже если open_app отключен
        if command != "open_url" and not self._executor.is_enabled():
            logger.info("[%s] Actions disabled, ignoring payload (command=%s)", feature_id, command)
            await self._publish_failure(
                session_id=session_id,
                feature_id=feature_id,
                error_code="disabled",
        ...
        # ⚠️ NEW: Расширяем валидацию типов для поддержки Messages команд и open_url (MVP 9)
        valid_commands = ["open_app", "close_app", "read_messages", "send_message", "open_url"]
        ...
        # ⚠️ NEW: Обновляем feature_id в зависимости от типа действия
        if command == "open_app":
            action_feature_id = "F-2025-013-open-app"
        elif command == "close_app":
            action_feature_id = "F-2025-014-close-app"
        elif command in ["read_messages", "send_message"]:
            action_feature_id = "F-2025-016-messages-integration"
        elif command == "open_url":
            action_feature_id = "F-2025-017-stripe-payment"  # MVP 9: open_url для платежной системы
        else:
            action_feature_id = feature_id
        
        # ⚠️ NEW: Валидация параметров в зависимости от типа действия
        ...
        if command == "open_app":
            ...
            # Для read_messages contact опционален (по умолчанию "last")
            # limit опционален (по умолчанию 10)
            # Валидация происходит на сервере в AssistantResponseParser
            pass
        elif command == "send_message":
            # Для send_message contact и message обязательны
            if not args.get("contact"):
                logger.error(
                    "[%s] Missing contact for send_message",
                    action_feature_id
                )
                await self._publish_failure(
                    session_id=session_id,
                    feature_id=action_feature_id,
                    error_code="missing_parameter",
                    message="Missing contact for send_message",
                    app_name=None,
                )
                return
            if not args.get("message"):
                logger.error(
                    "[%s] Missing message text for send_message",
                    action_feature_id
                )
                await self._publish_failure(
                    session_id=session_id,
                    feature_id=action_feature_id,
                    error_code="missing_parameter",
                    message="Missing message text for send_message",
                    app_name=None,
                )
                return

        # ⚠️ NEW: Преобразуем формат для ActionExecutor
        # Для open_app/close_app ActionExecutor ожидает: {"type": "...", "app_name": "...", "app_path": "..."}
        # Для Messages команд используем action_data напрямую (с command и args)
        if command in ["open_app", "close_app"]:
            ...
            # Для Messages команд передаем action_data как есть (с command и args)
            executor_action_data = action_data
    ...
    async def _execute_action(
        ...
        *,
        ...
        try:
            # ⚠️ NEW: Определяем команду из action_data (может быть command или type)
            command = action_data.get("command") or action_data.get("type", action_type)
            ...
            if command == "open_app":
                ...
                # MVP 9: Обработка команды open_url для открытия checkout/portal URLs
                result = await self._handle_open_url(action_data)
            elif command in ["close_app", "read_messages", "send_message"]:
                # Используем McpActionExecutor для close_app и Messages команд
                ...
                result = ActionResult(
                    success=False,
                    message=f"Unsupported action type: {command}",
                    error="unsupported_action",
                )
            
            # ⚠️ NEW: Для Messages команд отправляем результат обратно на сервер
            if command in ["read_messages", "send_message"]:
                # Пытаемся получить grpc_client через EventBus (публикуем событие)
                # GrpcClientIntegration должен подписаться на это событие и отправить результат
                await self.event_bus.publish(
                    "mcp.action.result",
                    {
                        "session_id": session_id,
                        ...
                        "result_text": result.message if result.success else "",
                        "error": result.error if not result.success else None,
                        "feature_id": feature_id,
                    },
                )
                logger.info(
                    "[%s] MCP result event published: command=%s, session=%s, success=%s",
                    feature_id,
                    command,
                    session_id,
                    result.success
