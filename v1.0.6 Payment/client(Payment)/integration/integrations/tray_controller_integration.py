...
class TrayControllerIntegration:
    ...
    async def _on_mode_changed(self, event):
        ...
        try:
            ...
            logger.info(f"🎯 TRAY DEBUG: new_mode_raw={new_mode_raw} (type: {type(new_mode_raw)})")
            
            # ✅ ИСПРАВЛЕНИЕ: Нормализация new_mode до AppMode Enum
            new_mode = self._normalize_mode(new_mode_raw)
            logger.info(f"🎯 TRAY DEBUG: new_mode нормализован: {new_mode} (type: {type(new_mode)})")
            ...
            if new_mode and new_mode in self.mode_to_status:
    ...
    def _normalize_mode(self, mode_raw) -> Optional[AppMode]:
        """
        Нормализует режим до AppMode Enum
        
        Args:
            mode_raw: Режим как строка, Enum или другой тип
        
        Returns:
            AppMode Enum или None если не удалось нормализовать
        """
        try:
            # Если уже AppMode Enum
            if isinstance(mode_raw, AppMode):
                return mode_raw
            
            # Если строка, пытаемся найти соответствующий Enum
            if isinstance(mode_raw, str):
                mode_str = mode_raw.lower()
                for app_mode in AppMode:
                    if app_mode.value.lower() == mode_str:
                        logger.debug(f"🔍 Нормализация режима: '{mode_raw}' → {app_mode}")
                        return app_mode
                logger.warning(f"⚠️ Неизвестный режим: '{mode_raw}'")
                return None
            
            # Если есть атрибут value (например, Enum с другим именем)
            if hasattr(mode_raw, 'value'):
                mode_str = str(mode_raw.value).lower()
                for app_mode in AppMode:
                    if app_mode.value.lower() == mode_str:
                        logger.debug(f"🔍 Нормализация режима через value: '{mode_raw}' → {app_mode}")
                        return app_mode
            
            logger.warning(f"⚠️ Не удалось нормализовать режим: {mode_raw} (type: {type(mode_raw)})")
            return None
            
        except Exception as e:
            logger.error(f"❌ Ошибка нормализации режима: {e}", exc_info=True)
            return None
    ...
    def _apply_status_ui_sync(self, status: TrayStatus):
        ...
        logger.debug(f"🔍 [UI] Начало обновления иконки трея: status={status.value}")
        ...
        try:
            ...
            logger.debug(f"🔍 [UI] Предыдущий статус: {prev_value}, новый статус: {status.value}")
            ...
            logger.debug(f"🔍 [UI] Иконка создана: {icon_path}, обновляем меню...")
            self.tray_controller.tray_menu.update_icon(icon_path)
            logger.debug(f"🔍 [UI] Иконка обновлена в меню")
            ...
            logger.debug(f"🔍 [UI] Текст статуса обновлен: {human}")
            
            self.tray_controller.current_status = status
            self._ui_dirty = False
            logger.info(f"✅ Tray UI applied: {prev_value} -> {status.value}")
            logger.debug(f"🔍 [UI] Обновление иконки трея завершено успешно")
        except Exception as e:
