...
# MVP 9: Payment Integration для обработки deep links
...
class SimpleModuleCoordinator:
    ...
    async def _create_integrations(self):
        ...
        try:
            ...
            # ✅ ЦИКЛ 1: DeviceChangePublisher Integration - единый монитор устройств
            # Должен быть создан ПЕРЕД voice_recognition и speech_playback,
            # так как они будут подписываться на события device.default_*_changed
            self.integrations['device_change_publisher'] = DeviceChangePublisherIntegration(
                event_bus=self.event_bus,
                state_manager=self.state_manager,
                error_handler=self.error_handler,
            )
            ...
            # MVP 9: Payment Integration - обработка deep links для платежной системы
            self.integrations['payment'] = PaymentIntegration(
                event_bus=self.event_bus,
                state_manager=self.state_manager,
                error_handler=self.error_handler
            )
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            startup_order = [
                ...
                'device_change_publisher', # 9. ✅ ЦИКЛ 1: Единый монитор устройств (ПЕРЕД voice_recognition и speech_playback)
                'voice_recognition',       # 10. Распознавание речи (использует microphone, подписывается на device.default_input_changed)
                'interrupt',               # 11. Управление прерываниями
                'screenshot_capture',      # 12. Захват экрана (использует screen_capture)
                'grpc',                    # 13. gRPC клиент (зависит от hardware_id)
                'action_execution',         # 14. Выполнение MCP команд (зависит от grpc)
                'speech_playback',         # 15. Воспроизведение речи (зависит от grpc, подписывается на device.default_output_changed)
                'signals',                 # 16. Аудио сигналы (должны быть до update_notification)
                'update_notification',     # 17. Голосовые уведомления об обновлениях (ПЕРЕД updater!)
                'updater',                 # 18. Система обновлений (после update_notification)
                'welcome_message',         # 19. Приветственное сообщение (зависит от speech_playback)
                'voiceover_ducking',       # 20. VoiceOver Ducking
                'autostart_manager',       # 21. Автозапуск (ПОСЛЕДНИЙ - не блокирующий)
            ]
