...
class EventBus:
    ...
    async def subscribe(self, event_type: str, callback: Callable, priority: EventPriority = EventPriority.MEDIUM):
        ...
        try:
            ...
            # ✅ АУДИТ: Проверка на дублирование подписки (предотвращает двойную обработку событий)
            for existing_sub in self.subscribers[event_type]:
                if existing_sub["callback"] == callback:
                    logger.warning(f"⚠️ [AUDIT] Попытка повторной подписки на событие {event_type} (callback уже зарегистрирован)")
                    return  # Пропускаем дублирование
            ...
            logger.info(f"📝 Подписка на событие: {event_type} (приоритет: {priority.name}, подписчиков: {len(self.subscribers[event_type])})")
