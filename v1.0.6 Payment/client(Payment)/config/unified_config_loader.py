...
class UnifiedConfigLoader:
    ...
    def get_mcp_config(self) -> Dict[str, Dict[str, Any]]:
        ...
        # Конфигурация для messages MCP сервера (NEW)
        messages_mcp = mcp_cfg.get('messages', {})
        result['messages'] = {
            'server_path': str(messages_mcp.get('server_path', '')),
            'enabled': bool(messages_mcp.get('enabled', True)),
            'timeout_sec': float(messages_mcp.get('timeout_sec', 15.0)),
        }
