...
class BackpressureManager:
    ...
    async def check_message_rate(self, stream_id: str) -> tuple[bool, Optional[str]]:
        ...
        async with self.lock:
            ...
            # Очищаем старые временные метки (старше 1 секунды) без пересборки списка
            while stream_info.message_timestamps and current_time - stream_info.message_timestamps[0] >= 1.0:
                stream_info.message_timestamps.popleft()
