...
class GrpcServiceManager(UniversalModuleInterface):
    ...
    async def initialize(self, config: dict) -> None:
        ...
        async with self._init_lock:
            if self._status.is_ready():
                logger.info("gRPC Service Manager уже инициализирован, повторный запуск пропущен")
                return
    ...
    async def _initialize_with_coordinator(self) -> None:
        ...
        # Wiring: Inject DatabaseManager into SubscriptionModule (singleton owner-path).
        try:
            if self.coordinator.has('database'):
                database_module = self.coordinator.get('database')
                if database_module.status().is_ready():
                    ...
                    if (
                        subscription_module is not None
                        and hasattr(subscription_module, 'set_database_manager')
                        and hasattr(database_module, 'get_manager')
                    ):
                        subscription_module.set_database_manager(database_module.get_manager())
                        logger.info("✅ DatabaseManager wired to SubscriptionModule")
        except Exception as e:
            logger.error(f"❌ Error wiring DatabaseManager to SubscriptionModule: {e}")
    ...
    async def _create_workflow_integrations(self) -> None:
        ...
        try:
            ...
            # Wiring: DatabaseManager -> StreamingWorkflowIntegration
            try:
                database_module = self._get_module('database')
                if (
                    database_module
                    and hasattr(database_module, 'status')
                    and database_module.status().is_ready()
                    and hasattr(database_module, 'get_manager')
                    and hasattr(self.streaming_workflow, 'set_database_manager')
                ):
                    self.streaming_workflow.set_database_manager(database_module.get_manager())
                    logger.info("✅ DatabaseManager wired to StreamingWorkflowIntegration")
            except Exception as wiring_error:
                logger.warning(f"⚠️ Failed to wire DatabaseManager to StreamingWorkflowIntegration: {wiring_error}")
    ...
    async def _initialize_integrations(self) -> None:
        ...
        try:
            ...
            if self.grpc_service_integration:
                ...
                logger.debug("GrpcServiceIntegration initialized")
