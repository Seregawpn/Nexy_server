...
try:
    ...
    logger.info("pydub unavailable; MP3->PCM conversion will use ffmpeg fallback when available")
...
class EdgeTTSProvider(UniversalProviderInterface):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._conversion_capability_logged = False
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if test_result:
                ...
                self._log_conversion_capability()
    ...
    def _log_conversion_capability(self) -> None:
        """Log one canonical message about MP3->PCM conversion mode."""
        if self._conversion_capability_logged:
            return
        self._conversion_capability_logged = True
        level, message = build_conversion_mode_log_message(self._get_conversion_mode())
        getattr(logger, level)(message)

    def _get_conversion_mode(self) -> str:
        return resolve_conversion_mode(
            ...
            try:
                ...
                if self.convert_to_pcm and FFMPEG_AVAILABLE:
                    ...
                    if self.convert_to_pcm:
                        ...
                        # Разбиваем на чанки
                        for chunk in iter_chunk_bytes(pcm_data, self.streaming_chunk_size):
                            yield chunk
                            has_yielded = True
                    else:
                        # Возвращаем MP3 как есть
                        for chunk in iter_chunk_bytes(mp3_data, self.streaming_chunk_size):
                            yield chunk
                            has_yielded = True
                ...
                if not should_retry_process_attempt(
                ...
                await asyncio.sleep(get_retry_delay_sec(attempt))
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        base_status.update(
            build_edge_status_payload(
                ...
                conversion_mode=self._get_conversion_mode(),
            )
        )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        base_metrics.update(build_edge_metrics_payload(self))
    ...
    def get_audio_info(self) -> Dict[str, Any]:
        ...
        """
        return build_edge_audio_info(self, conversion_mode=self._get_conversion_mode())
