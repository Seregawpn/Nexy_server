...
class TextProcessor:
    ...
    async def _build_classifier_context(
        ...
        if runtime_memory_context:
            current_goal = self._extract_memory_section(runtime_memory_context, "Current goal")
            if current_goal and current_goal.lower() != "no active goal":
    ...
    async def _resolve_route_and_reason(
        ...
        route = await self._classify_route(intent_text, session_id, runtime_memory_context)
    ...
    async def prepare_generation_request(
        ...
        route_flags, user_input = await self._build_prompt_for_text(
            text,
        ...
        if resolved_use_google_search is None:
            ...
            resolved_use_google_search = bool(
                config.features.google_search_enabled and route_flags.get("route") == "google_search"
            )
        return {
            "route_flags": route_flags,
            "user_input": user_input,
            "route": str(route_flags.get("route", "none")),
    ...
    async def _build_prompt_for_text(
        ...
        if has_image and not intent_text.strip():
            route, fallback_reason = "describe", "image_forced_describe"
        else:
            route, fallback_reason = await self._resolve_route_and_reason(
        ...
        logger.info(
            ...
            route_flags.get("route", "none"),
            fallback_reason,
            len(intent_text),
        )
        return route_flags, intent_text
    ...
    async def process_text_streaming(
        ...
        try:
            ...
            route_flags: Optional[Dict[str, bool]] = None
            llm_input_text = text
            route = "none"
            if text:
                ...
                route_flags = dict(prepared["route_flags"])
                llm_input_text = str(prepared["user_input"])
                route = str(prepared.get("route", "none") or "none")
                use_google_search = bool(prepared["use_google_search"])
                logger.info(
                    "TextProcessor: route_flags=%s use_google_search=%s",
                    route_flags,
                    use_google_search
                )
            use_image_for_generation = bool(image_data)
            if image_data and route_flags is not None:
                # Image is passed to LLM only for describe route.
                use_image_for_generation = bool(route_flags.get("describe", False))
            logger.info(
                "TextProcessor: use_image_for_generation=%s has_image=%s route=%s",
                use_image_for_generation,
                bool(image_data),
                str((route_flags or {}).get("route", "none")),
            )
