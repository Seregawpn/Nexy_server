    ...
    async def process(self, request: Dict[str, Any]) -> Union[Dict[str, Any], AsyncIterator[Dict[str, Any]]]:
        ...
        Args:
            request: Запрос на обработку текста
                ...
                - use_google_search: bool (опционально) - использовать Google Search
        ...
        try:
            ...
            use_google_search = request.get("use_google_search")
            session_id = request.get("session_id")
            hardware_id = request.get("hardware_id")
            runtime_memory_context = request.get("runtime_memory_context")
            prepared_request = request.get("prepared_request")
            if "use_search" in request:
                logger.warning(
                    "Legacy request field ignored: use_search (use use_google_search)",
                    extra={
                        "scope": "module",
                        "decision": "legacy_input_ignored",
                        "ctx": {"field": "use_search", "session_id": session_id},
                    },
                )
            if "web_search" in request:
                logger.warning(
                    "Legacy request field ignored: web_search (use use_google_search)",
                    extra={
                        "scope": "module",
                        "decision": "legacy_input_ignored",
                        "ctx": {"field": "web_search", "session_id": session_id},
                    },
                )
