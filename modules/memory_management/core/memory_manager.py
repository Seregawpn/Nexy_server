...
class MemoryManager:
    ...
    async def initialize(self):
        ...
        try:
            if self.config.validate_config():
                ...
                logger.info(
                    "MemoryAnalyzer disabled (optional): invalid Vertex/auth config; fallback=heuristic"
                )
    ...
    async def update_memory_background(
        ...
        try:
            ...
            async with lock:
                ...
                goal_state, current_goal = self._override_goal_state_from_runtime_outcome(
                    route=route,
                    behavior=behavior,
                    command_name=command_name,
                    command_present=command_present,
                    analyzed_goal_state=goal_state,
                    analyzed_current_goal=current_goal,
                )
    ...
    def _override_goal_state_from_runtime_outcome(
        self,
        *,
        route: str,
        behavior: str,
        ...
        analyzed_goal_state: str,
        analyzed_current_goal: str,
    ) -> Tuple[str, str]:
        normalized_route = str(route or "").strip().lower()
        normalized_behavior = str(behavior or "").strip().lower()
        normalized_command = str(command_name or "").strip().lower()
        state = str(analyzed_goal_state or "").strip().lower()
        goal = str(analyzed_current_goal or "").strip()

        # Informational routes must never keep a pending goal.
        if normalized_route in {"none", "capability", "describe"}:
            return "empty", ""

        # Terminal direct actions must close goal state immediately.
        if normalized_behavior == "execute" and (command_present or normalized_command):
            return "empty", ""

        # Clarification keeps the task open only when there is a concrete pending goal.
        if normalized_behavior == "clarify":
            return ("set", goal) if goal else ("empty", "")
    ...
    def _build_memory_context_response(
        ...
        return {
            "goal_state_context": str(snapshot.get("goal_state", "") or ""),
