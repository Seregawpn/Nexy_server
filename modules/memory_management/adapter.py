    ...
    async def process(self, request: Dict[str, Any]) -> Union[Dict[str, Any], AsyncIterator[Dict[str, Any]]]:
        ...
        try:
            ...
            if action == "get_memory":
                ...
                updated = await self._manager.update_memory_background(
                    hardware_id,
                    prompt,
                    response,
                    route=str(request.get("route", "") or ""),
                    behavior=str(request.get("behavior", "") or ""),
                    command_name=str(request.get("command_name", "") or ""),
                    command_present=bool(request.get("command_present", False)),
                )
                if updated:
