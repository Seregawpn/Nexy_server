...
class MemoryConfig:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.vertex_api_key = self.config.get(
            'vertex_api_key',
            getattr(unified_config.text_processing, 'vertex_api_key', ''),
        )
        ...
        self.memory_analysis_current_goal_universal_policy = """
           ...
           - Highest priority: if the task is unfinished after this turn, CURRENT_GOAL must stay semantically correct and active for the exact unfinished task.
           - Highest priority: if the task is already finished in this turn, no active CURRENT_GOAL should remain after the turn.
        ...
        self.memory_analysis_current_goal_lifecycle_rules = """
           ...
           - For product correctness, goal continuity while the task is unfinished is more important than the exact terminal label after the task is already finished.
           ...
           - If an active app-opening clarification goal existed before the turn and the user supplies the app name (for example "Safari") and the assistant opens that app in this turn, use clear and return CURRENT_GOAL as EMPTY.
           ...
           - If the user continues an active browser task with a short or noisy follow-up and the assistant immediately performs that browser action in the same turn, use clear and return CURRENT_GOAL as EMPTY.
           ...
           - If the user provides a short topical follow-up that fills the active search task's missing topic and the assistant returns the factual search answer in the same turn, use clear and return CURRENT_GOAL as EMPTY.
           ...
           - If the user abandons the old task and starts a different explicit task in the same turn, use replace only when that new task remains active after the turn.
           - If the user abandons the old task and the new explicit task is also fully completed in the same turn, use clear because no active executable goal remains after the turn.
           ...
           - If an active messaging or WhatsApp goal already knows the recipient and a short or noisy follow-up provides the message body, then sending that message in the same turn is clear and CURRENT_GOAL must be EMPTY.
           ...
           - If assistant already completed the task in this turn, CURRENT_GOAL being EMPTY is mandatory even if clear vs empty is later interpreted differently by downstream product checks.
           ...
           - Cancellation acknowledgements like "Okay, I've cancelled that for you.", "Okay, I will not continue that task.", or "Never mind" indicate clear when an active goal existed before the turn.
           ...
           - If the same turn abandons the old goal and immediately executes a different explicit task such as "No, open Safari instead" -> "Opening Safari.", use clear and return CURRENT_GOAL as EMPTY because no executable goal remains active after the turn.
           - If the same turn abandons the old goal and immediately returns the final factual answer for a new search task, use clear and return CURRENT_GOAL as EMPTY because no executable goal remains active after the turn.
        """
        ...
        self.memory_analysis_current_goal_outcome_rules = """
           ...
           - While the task remains unfinished, prefer the correct active CURRENT_GOAL over any shortcut that would drop or reset the goal too early.
           ...
           - For completed turns, the important terminal outcome is that CURRENT_GOAL is EMPTY after the turn.
        ...
        self.memory_analysis_current_goal_examples = """
           ...
           - EXISTING_CURRENT_GOAL: User wants to send a note to Sophia. Missing detail: message text. / USER: Tell her I'm running behind / ASSISTANT: Sending message to Sophia. -> GOAL_STATE: clear / CURRENT_GOAL: EMPTY
           - EXISTING_CURRENT_GOAL: User wants to open YouTube and find something. Missing detail: search query. / USER: swap it over to sleep sounds / ASSISTANT: Opening YouTube to play sleep sounds. -> GOAL_STATE: clear / CURRENT_GOAL: EMPTY
           - EXISTING_CURRENT_GOAL: User wants to find news. Missing detail: news type. / USER: global affairs / ASSISTANT: Here are some of the latest global affairs headlines. -> GOAL_STATE: clear / CURRENT_GOAL: EMPTY
           - EXISTING_CURRENT_GOAL: User wants to search YouTube. Missing detail: search query. / USER: yt ambient rain swap / ASSISTANT: Opening YouTube to play ambient rain sounds. -> GOAL_STATE: clear / CURRENT_GOAL: EMPTY
           - EXISTING_CURRENT_GOAL: User wants to send a message to Sophia. Missing detail: message text. / USER: actually global headlines instead / ASSISTANT: Here are the latest global headlines. -> GOAL_STATE: clear / CURRENT_GOAL: EMPTY
           ...
           - USER: Message Mom on WhatsApp / ASSISTANT: What message do you want to send to Mom? -> GOAL_STATE: set / CURRENT_GOAL: User wants to send a WhatsApp message to Mom. Missing detail: message text.
           ...
           - EXISTING_CURRENT_GOAL: User wants to send a WhatsApp message to Mom. Missing detail: message text. / USER: Tell Mom I got here safely / ASSISTANT: Sending WhatsApp to Mom. -> GOAL_STATE: clear / CURRENT_GOAL: EMPTY
           ...
           - EXISTING_CURRENT_GOAL: User wants to open YouTube and play jazz. / USER: make it sleep music instead / ASSISTANT: Opening YouTube to play sleep music. -> GOAL_STATE: clear / CURRENT_GOAL: EMPTY
           ...
           - EXISTING_CURRENT_GOAL: User wants to open an app. Missing detail: app name. / USER: Safari / ASSISTANT: Opening Safari. -> GOAL_STATE: clear / CURRENT_GOAL: EMPTY
           - USER: To John: running late / ASSISTANT: Sending message to John. -> GOAL_STATE: empty
           - USER: Send hello to Sophia / ASSISTANT: Sending message to Sophia. -> GOAL_STATE: empty / CURRENT_GOAL: EMPTY
           ...
           - USER: Show me current world news / ASSISTANT: Here are the latest world news headlines. -> GOAL_STATE: empty / CURRENT_GOAL: EMPTY
           ...
           - USER: What on screen now / ASSISTANT: Describing what is currently on the screen. -> GOAL_STATE: empty / CURRENT_GOAL: EMPTY
           - USER: Open Reddit / ASSISTANT: Opening Reddit. -> GOAL_STATE: empty / CURRENT_GOAL: EMPTY
           ...
           - EXISTING_CURRENT_GOAL: User wants news. Missing detail: topic. / USER: world news / ASSISTANT: Here are the latest world news headlines. -> GOAL_STATE: clear / CURRENT_GOAL: EMPTY
           - EXISTING_CURRENT_GOAL: User wants latest news. Missing detail: topic. / USER: world headlines / ASSISTANT: Here are the latest world headlines. -> GOAL_STATE: clear / CURRENT_GOAL: EMPTY
           ...
           - EXISTING_CURRENT_GOAL: User wants to send a message to Sophia. Missing detail: message text. / USER: nah just world headlines instead / ASSISTANT: Here are the latest world headlines. -> GOAL_STATE: clear / CURRENT_GOAL: EMPTY
           ...
           - EXISTING_CURRENT_GOAL: User wants news. Missing detail: topic. / USER: Actually open Notes instead / ASSISTANT: Opening Notes. -> GOAL_STATE: clear / CURRENT_GOAL: EMPTY
           ...
           - EXISTING_CURRENT_GOAL: User wants to send a message to Sophia. Missing detail: message text. / USER: No, never mind / ASSISTANT: Okay, I've cancelled that for you. -> GOAL_STATE: clear / CURRENT_GOAL: EMPTY
           - EXISTING_CURRENT_GOAL: User wants to send a message to Sophia. Missing detail: message text. / USER: snd her ill b outside in 2 / ASSISTANT: Sending message to Sophia. -> GOAL_STATE: clear / CURRENT_GOAL: EMPTY
