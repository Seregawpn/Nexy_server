...
1. Обновить конфигурацию `text` с новым текстом на нужном языке
2. Настроить параметр `voice` для соответствующего голоса (например, "ru-RU-DmitryNeural")
3. Обновить тесты
4. Сервер автоматически сгенерирует аудио на новом языке
...
2. **Высокая задержка**
   ...
   - Проверить размер генерируемого аудио
