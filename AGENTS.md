...
## 9) Архитектурные гейты (обязательно)

Перед любым merge/change-set обязательно пройти 4 проверки:

1. Single Owner Gate
- Для каждой изменяемой оси (mode/session/interrupt/update/flags) указывается один владелец (Source of Truth).
- Если появляется второй путь принятия решений, изменение недопустимо.

2. Zero Duplication Gate
- Если новая логика >=70% повторяет существующую, нужно объединять, а не копировать.
- В отчете фиксировать: что было дубликатом и что стало единым владельцем.

3. Anti-Race Gate
- Для shared mutable state обязателен guard: `single-flight`, `lock`, `state-guard` или `idempotency`.
- Для event-driven сценариев обязательно проверить out-of-order/повторные события.

4. Flag Lifecycle Gate
- Каждый флаг должен иметь runtime-потребителя и документированное поведение.
- "Мертвые" флаги (в конфиге есть, в runtime не участвуют) запрещены.

Минимум для PR/изменения:
- Указать owner оси.
- Указать что удалено/объединено как дубликат.
- Указать race guard.
- Указать статус флагов (used/removed).
