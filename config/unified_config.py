...
_REPO_ROOT = _PROJECT_SERVER_DIR.parent
_CLIENT_UNIFIED_CONFIG_PATH = _REPO_ROOT / "client" / "config" / "unified_config.yaml"


def _load_shared_client_runtime_config() -> Dict[str, Any]:
    """
    Canonical product runtime owner for cross-stack feature toggles in monorepo mode.

    When server runs inside the root workspace, the shared client unified_config.yaml
    is the single manual config entrypoint. Standalone server deployments may not have
    this file; in that case compatibility fallback to env remains available.
    """
    if not _CLIENT_UNIFIED_CONFIG_PATH.exists():
        return {}
    try:
        with _CLIENT_UNIFIED_CONFIG_PATH.open("r", encoding="utf-8") as fh:
            return yaml.safe_load(fh) or {}
    except Exception as exc:
        logger.warning(
            "Не удалось загрузить shared runtime config %s: %s",
            _CLIENT_UNIFIED_CONFIG_PATH,
            exc,
        )
        return {}


def _get_shared_feature_enabled(feature_name: str) -> Optional[bool]:
    config = _load_shared_client_runtime_config()
    features = config.get("features", {}) if isinstance(config, dict) else {}
    raw = features.get(feature_name) if isinstance(features, dict) else None
    if isinstance(raw, dict) and "enabled" in raw:
        return bool(raw.get("enabled", False))
    return None


def _get_shared_feature_enabled_any(*feature_names: str) -> Optional[bool]:
    for feature_name in feature_names:
        resolved = _get_shared_feature_enabled(feature_name)
...
def get_version_from_file() -> str:
    ...
    return os.getenv('SERVER_VERSION', '2.0.0.40')
...
class TextProcessingConfig:
    ...
    # Runtime system prompt is assembled inside the provider from route.
    ...
    def from_env(cls) -> 'TextProcessingConfig':
        ...
        return cls(
            ...
            use_vertex_ai=os.getenv('GEMINI_USE_VERTEX_AI', 'false').lower() == 'true',
            vertex_project=os.getenv('GOOGLE_CLOUD_PROJECT', ''),
            vertex_location=os.getenv('GOOGLE_CLOUD_LOCATION', 'global'),
            vertex_api_key=os.getenv('VERTEX_API_KEY', os.getenv('GOOGLE_API_KEY', '')),
...
class FeaturesConfig:
    ...
    assistant_runtime_v2: bool = False  # Experimental assistant runtime v2
    messages_enabled: bool = True  # iMessage/Messages integration
    google_search_enabled: bool = True  # Google web search
    
    @classmethod
    def from_env(cls) -> 'FeaturesConfig':
        shared_actions = _get_shared_feature_enabled("actions")
        shared_messages = _get_shared_feature_enabled("messages")
        return cls(
            ...
            forward_assistant_actions=(
                shared_actions
                if shared_actions is not None
                else os.getenv('FORWARD_ASSISTANT_ACTIONS', 'false').lower() == 'true'
            ),
            assistant_runtime_v2=os.getenv('ASSISTANT_RUNTIME_V2', 'false').lower() == 'true',
            messages_enabled=(
                shared_messages
                if shared_messages is not None
                else os.getenv('MESSAGES_ENABLED', 'true').lower() == 'true'
            ),
            google_search_enabled=(
                os.getenv('GOOGLE_SEARCH_ENABLED', os.getenv('WEB_SEARCH_ENABLED', 'true')).lower() == 'true'
            ),
        )
...
class KillSwitchesConfig:
    ...
    disable_assistant_runtime_v2: bool = False
    ...
    def from_env(cls) -> 'KillSwitchesConfig':
        return cls(
            ...
            disable_assistant_runtime_v2=os.getenv('NEXY_KS_DISABLE_ASSISTANT_RUNTIME_V2', 'false').lower() == 'true',
        )
...
class BrowserUseConfig:
    ...
    def from_env(cls) -> 'BrowserUseConfig':
        shared_enabled = _get_shared_feature_enabled_any("browser_use", "browser")
        if shared_enabled is not None:
            return cls(enabled=shared_enabled)
        return cls(
...
class SubscriptionConfig:
    ...
    def from_env(cls) -> 'SubscriptionConfig':
        ...
        shared_payment_enabled = _get_shared_feature_enabled("payment")
        ...
        return cls(
            enabled=(
                shared_payment_enabled
                if shared_payment_enabled is not None
                else os.getenv('SUBSCRIPTION_ENABLED', 'false').lower() == 'true'
            ),
...
class WhatsappConfig:
    ...
    def from_env(cls) -> 'WhatsappConfig':
        shared_enabled = _get_shared_feature_enabled("whatsapp")
        if shared_enabled is not None:
            return cls(enabled=shared_enabled)
        return cls(
...
class UnifiedServerConfig:
    ...
    def _validate_config(self) -> None:
        ...
        if self.text_processing.use_vertex_ai:
            if not self.text_processing.vertex_project:
                errors.append("GOOGLE_CLOUD_PROJECT не установлен для Vertex режима")
            if not self.text_processing.vertex_location:
                errors.append("GOOGLE_CLOUD_LOCATION не установлен для Vertex режима")
        elif not self.text_processing.gemini_api_key:
    ...
    def is_feature_enabled(self, feature_name: str) -> bool:
        ...
        feature_map = {
            ...
            'assistant_runtime_v2': self.features.assistant_runtime_v2,
            'messages_enabled': self.features.messages_enabled,
            'google_search_enabled': self.features.google_search_enabled,
        }
    ...
    def is_kill_switch_active(self, kill_switch_name: str) -> bool:
        ...
        kill_switch_map = {
            ...
            'disable_assistant_runtime_v2': self.kill_switches.disable_assistant_runtime_v2,
        }
        
        return kill_switch_map.get(kill_switch_name, False)

    def is_action_forwarding_enabled(self) -> bool:
        """
        Canonical action-forwarding gate.
        Returns True only when forwarding feature is enabled and kill-switch is inactive.
        """
        return bool(
            self.features.forward_assistant_actions
            and not self.kill_switches.disable_forward_assistant_actions
        )
    ...
    def get_provider_config(self, provider_name: str) -> Dict[str, Any]:
        ...
        provider_configs = {
            'langchain': {
                ...
                'fallback_api_key': self.text_processing.gemini_fallback_api_key,
                'use_vertex_ai': self.text_processing.use_vertex_ai,
                'vertex_project': self.text_processing.vertex_project,
                'vertex_location': self.text_processing.vertex_location,
                'vertex_api_key': self.text_processing.vertex_api_key,
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'text_processing': {
                ...
                'vertex_project': self.text_processing.vertex_project,
                'vertex_location': self.text_processing.vertex_location,
                'vertex_api_key_set': bool(self.text_processing.vertex_api_key),
