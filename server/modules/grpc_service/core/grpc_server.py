        ...
        try:
            ...
            async for item in self.grpc_service_manager.process(request_data):
                ...
                if not success:
                    ...
                    # После частичных данных сервер обязан завершать стрим явным terminal
                    # сообщением, иначе клиент получает protocol ambiguity.
                    if sent_any:
                        reason = "silent_rate_limit" if is_silent else "error_after_partial_data"
                        _log_request_path(
                            "stream.terminated",
                            session_id=session_id,
                            hardware_id=hardware_id,
                        ...
                        logger.warning(
                            "⚠️ Ошибка после начала стрима для %s: отправляем terminal error_message без gRPC статуса",
                            session_id,
                            extra={
                                'scope': 'grpc',
                                'method': 'StreamAudio',
                                'decision': 'terminal_error_after_partial_data',
                                'ctx': {
                                    'session_id': session_id,
                                    'hardware_id': hardware_id,
