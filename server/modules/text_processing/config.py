...
class TextProcessingConfig:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.search_policy = self.config.get('search_policy', 'assistant_fallback')

        self.google_search_enabled = getattr(unified_config.features, 'google_search_enabled', True)
    ...
    def get_provider_config(self, provider_name: str) -> Dict[str, Any]:
        ...
        if provider_name == 'langchain':
            ...
            dynamic_prompt = build_system_prompt(
                ...
                google_search_enabled=unified_config.features.google_search_enabled,
            )
    ...
    def validate(self) -> bool:
        ...
        """
        # Проверяем auth-конфиг (единый owner: centralized config)
        if self.use_vertex_ai:
            if not self.vertex_project:
                print("⚠️ GOOGLE_CLOUD_PROJECT не установлен для Vertex режима")
                return False
            if not self.vertex_location:
                print("⚠️ GOOGLE_CLOUD_LOCATION не установлен для Vertex режима")
                return False
