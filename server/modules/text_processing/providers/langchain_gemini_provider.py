...
from config.prompts import build_intent_classifier_prompt
...
try:
    ...
    logger.warning("⚠️ google.genai SDK не найден - classifier path недоступен")
...
class LangChainGeminiProvider(UniversalProviderInterface):
    ...
    def __init__(self, config: Dict[str, Any], token_usage_tracker: Optional['TokenUsageTracker'] = None):
        ...
        self.is_available = (
            GOOGLE_GENAI_AVAILABLE and bool(self.vertex_project) and bool(self.vertex_location)
            if self.use_vertex_ai
            else LANGCHAIN_AVAILABLE and bool(self._api_keys)
        )
        self.is_initialized = False
        
        logger.info(f"LangChainGeminiProvider initialized: available={self.is_available}")

    def _iter_stream_chunks(self, text: str):
        """
        Split text into deterministic small chunks for streaming transport.
        Keeps upstream/downstream contracts unchanged (still text fragments).
        """
        if not text:
            return
        payload = str(text)
        if len(payload) <= self.streaming_chunk_size:
            yield payload
            return
        for i in range(0, len(payload), self.streaming_chunk_size):
            part = payload[i:i + self.streaming_chunk_size]
            if part:
                yield part
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # Vertex mode работает через google.genai SDK и OAuth/ADC (или vertex key если поддерживается).
            # Developer mode работает через LangChain ChatGoogleGenerativeAI.
            ...
            if self.use_vertex_ai:
                # Проверяем инициализацию Vertex-клиента заранее (без внешнего запроса).
                self._create_genai_client()
                self.llm_no_tools = None
                self.llm_with_tools = None
                self.llm = None
            else:
    ...
    def _create_genai_client(self):
        if not GOOGLE_GENAI_AVAILABLE or genai is None:
            raise RuntimeError("google.genai SDK is unavailable")

        if self.use_vertex_ai:
            if self.vertex_project and self.vertex_location:
                if self.vertex_api_key:
                    logger.warning(
                        "VERTEX_API_KEY ignored in Vertex project/location mode (OAuth/ADC required by SDK)."
                    )
                return genai.Client(
                    vertexai=True,
            ...
            if self.vertex_api_key:
                return genai.Client(api_key=self.vertex_api_key)
            raise RuntimeError("Vertex mode requires GOOGLE_CLOUD_PROJECT/GOOGLE_CLOUD_LOCATION or VERTEX_API_KEY")

        if not self._api_keys:
            raise RuntimeError("No Gemini API keys configured")
        return genai.Client(api_key=self._api_keys[self._active_key_index])

    def _build_genai_config(self, system_prompt: str, use_search: Optional[bool]):
        ...
        if use_search is True:
            # SDK поддерживает dict tool descriptor.
    ...
    @staticmethod
    def _extract_runtime_memory_context(input_data: str) -> tuple[str, str]:
        """
    ...
    async def process(
        ...
        try:
            ...
            if runtime_memory_context is None:
                ...
                runtime_memory_context, clean_user_input = self._extract_runtime_memory_context(input_data)
            ...
            if self.use_vertex_ai:
                ...
                config = self._build_genai_config(system_prompt=system_prompt, use_search=use_search)
                ...
                if text:
                    for text_chunk in self._iter_stream_chunks(text):
                        yield text_chunk
                accumulated_usage = self._extract_usage_from_response(response)
            else:
                # Формируем сообщения согласно требованиям: SystemMessage + HumanMessage
                # System prompt содержит policy + runtime memory context from workflow.
                messages = []
        ...
        try:
            ...
            if self.use_vertex_ai:
                try:
                    ...
                    raise ValueError(f"Invalid base64 image_data: {decode_exc}") from decode_exc
                ...
                payload = [
                    {
                        ...
                        "parts": [
                            {"text": content[0]["text"]},
                            {"inline_data": {"mime_type": self.image_mime_type, "data": image_bytes}},
                        ],
                    }
                ]
                response = await asyncio.to_thread(
    ...
    def _build_classifier_prompt_with_context(
        ...
        """
        Inject short-term context directly into classifier system prompt.
        This keeps context tied to classification instructions, as required.
        """
        if not classifier_context:
            return base_prompt

        parts = [base_prompt.rstrip(), "", "[Short-Term Context For This Classification]"]
        parts.append(
            "Read order contract: top-to-bottom is newest-to-oldest; CURRENT_TURN is newest; use TIME_UTC for temporal grounding."
        )
        ordered_keys = ("short_term_memory",)
        ...
        parts.append("")
        parts.append(
            "Use this context to disambiguate the current user goal from recent dialogue."
        )
        parts.append(
    ...
    async def classify_intent_decision(
        ...
        raw_input = str(input_data or "").strip()
        if not raw_input:
    ...
    def _classify_intent_decision_sync(
        ...
        if not GOOGLE_GENAI_AVAILABLE:
            return {"category": "none"}
        if not self.use_vertex_ai and not self._api_keys:
            return {"category": "none"}
