...
class SessionRegistry:
    ...
    def register_session(self, session_data: SessionData) -> bool:
        ...
        with self._lock:
            if session_data.session_id in self._active_sessions:
                ...
                return False
            cached_context = self._session_context.get(session_data.session_id)
            if cached_context:
                merged_context = dict(cached_context)
                merged_context.update(session_data.context or {})
                session_data.context = merged_context
    ...
    def get_session_context(self, session_id: str) -> Dict[str, Any]:
        """Возвращает копию контекста сессии (active или cached runtime-context)."""
        with self._lock:
            active = self._active_sessions.get(session_id)
            if active:
                return dict(active.context or {})
    ...
    def update_session_context(
        ...
        """Обновляет runtime-контекст сессии через единый owner-path SessionRegistry."""
        if not session_id or not isinstance(updates, dict) or not updates:
            return False
        with self._lock:
            active = self._active_sessions.get(session_id)
            if active:
                active.context.update(updates)
                return True
            cached = dict(self._session_context.get(session_id, {}))
            cached.update(updates)
            if hardware_id and "hardware_id" not in cached:
                cached["hardware_id"] = hardware_id
            self._session_context[session_id] = cached
            return True
