...
class EdgeTTSProvider(UniversalProviderInterface):
    ...
    def _log_conversion_capability(self) -> None:
        ...
        if not self.convert_to_pcm:
            logger.info("EdgeTTS conversion mode: convert_to_pcm=false (streaming MP3 passthrough)")
            return

        if FFMPEG_AVAILABLE:
            logger.info("EdgeTTS conversion mode: ffmpeg streaming MP3->PCM")
            return

        if PYDUB_AVAILABLE:
            logger.info("EdgeTTS conversion mode: pydub MP3->PCM (non-streaming fallback)")
            return

        logger.warning(
            "⚠️ EdgeTTS conversion degraded: no ffmpeg/pydub; streaming MP3 passthrough enabled"
        )
