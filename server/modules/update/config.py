...
def _resolve_default_updates_dir() -> Path:
    """
    Resolves canonical updates dir across known layouts:
    - root workspace: <repo>/server/updates
    - server subtree/deploy: <server-root>/updates
    """
    current_file_dir = Path(__file__).resolve().parent
    workspace_candidate = current_file_dir.parent.parent.parent / "updates"
    subtree_candidate = current_file_dir.parent.parent / "updates"
...
class UpdateConfig:
    ...
    def __post_init__(self):
        ...
        if self.updates_dir is None:
            self.updates_dir = str(_resolve_default_updates_dir())
    ...
    def from_env(cls) -> 'UpdateConfig':
        ...
        return cls(
            ...
            beta_enabled=os.getenv('UPDATE_BETA_ENABLED', 'true').lower() == 'true',
