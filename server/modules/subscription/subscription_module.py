...
class SubscriptionModule:
    ...
    def set_database_manager(self, database_manager: Optional[Any]) -> None:
        """
        Inject DatabaseManager from module coordinator wiring.
        This keeps user persistence under existing database owner-path.
        """
        self._database_manager = database_manager

    @staticmethod
    def _is_real_hardware_id(hardware_id: str) -> bool:
        """
        Filter out synthetic/test IDs so only real app users are persisted.
        """
        value = (hardware_id or "").strip()
        if not value:
            return False
        lowered = value.lower()
        if lowered in {"unknown", "smoke_test_hardware_id"}:
            return False
        if lowered.startswith("test"):
            return False
        if "smoke_test" in lowered:
            return False
        if "test-memory" in lowered:
            return False
        return True

    async def _ensure_user_anchor(self, hardware_id: str) -> bool:
        """
        Ensure hardware_id exists in users table as soon as app is used.
        Idempotent by read-before-create and read-after-create fallback.
        """
        db = self._database_manager
        if db is None:
            return False
        if not self._is_real_hardware_id(hardware_id):
            return False
        try:
            ...
            user_id = await db.create_user(
                hardware_id,
                metadata={"created_via": "subscription_can_process"},
            )
            if user_id:
                return True

            # Race-safe fallback: if concurrent create won, user now exists.
            existing = await db.get_user_by_hardware_id(hardware_id)
            return bool(existing)
        except Exception as e:
            logger.warning(
                f"[F-2025-017] Failed to persist user anchor for {hardware_id[:8]}...: {e}"
            )
            return False
    ...
    def _ensure_subscription_anchor(self, hardware_id: str) -> bool:
        """
        Ensure hardware_id is persisted in subscriptions as the runtime owner axis.
        Idempotent by design via repository UPSERT.
        """
        if self._repository is None:
            return False
        create_or_update = getattr(self._repository, "create_or_update_subscription", None)
        ...
        try:
            ...
            logger.warning(
                f"[F-2025-017] Failed to persist subscription anchor for {hardware_id[:8]}...: {e}"
            )
            return False
    ...
    async def can_process(self, hardware_id: str) -> CanProcessResult:
        ...
        try:
            # Persist user identity early (best-effort) to avoid losing first-use registration.
            await self._ensure_user_anchor(hardware_id)
            ...
            # Persist new users early so hardware_id survives interrupted requests.
            if quota_result.get("reason") == "new_user":
                if self._ensure_subscription_anchor(hardware_id):
                    quota_result = self._quota_checker.check_quota(hardware_id)
