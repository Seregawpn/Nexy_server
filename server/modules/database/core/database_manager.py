...
class DatabaseManager:
    ...
    async def _initialize_provider(self):
        ...
        try:
            ...
            if await self.postgresql_provider.initialize():
                logger.debug("PostgreSQL Provider initialized")
            else:
    ...
    async def create_session(
        ...
        Args:
            ...
            session_id: Внешний ID сессии (опционально)
        ...
        try:
            ...
            return await self.postgresql_provider.create_session(user_id, metadata, session_id)
    ...
    async def ensure_session(
        ...
        """
        Идемпотентное создание/обновление сессии по внешнему session_id.
        
        Args:
            user_id: ID пользователя
            session_id: Внешний ID сессии
            metadata: Метаданные сессии
        ...
        try:
            ...
            return await self.postgresql_provider.ensure_session(user_id, session_id, metadata)
    ...
    async def ensure_command(
        ...
        """
        Идемпотентное создание команды в пределах request_key.
        """
        try:
            if not self.is_initialized:
                raise Exception("DatabaseManager not initialized")
            ...
            return await self.postgresql_provider.ensure_command(session_id, prompt, metadata, language)

        except Exception as e:
            logger.error(f"Error ensuring command: {e}")
            return None
    ...
    async def ensure_llm_answer(
        ...
        """
        Идемпотентное создание/обновление ответа LLM для команды.
        """
        try:
            if not self.is_initialized:
                raise Exception("DatabaseManager not initialized")
            ...
            return await self.postgresql_provider.ensure_llm_answer(
                command_id,
                prompt,
                response,
                model_info,
            ...
            logger.error(f"Error ensuring LLM answer: {e}")
            return None
