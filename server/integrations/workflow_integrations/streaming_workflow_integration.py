...
class StreamingWorkflowIntegration:
    ...
    async def process_request_streaming(self, request_data: Dict[str, Any]) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        try:
            ...
            if ctx.pending_command_payload and not ctx.command_payload_sent:
                ...
                resolved_route = self._canonical_primary_route(ctx.primary_route) or "none"
    ...
    async def _iter_processed_sentences(
        ...
        if self.text_module and hasattr(self.text_module, 'process'):
            ...
            try:
                ...
                stream, primary_route = await self._stream_text_module(
                    text,
                    screenshot_data,
                    session_id,
                ...
                if request_ctx is not None:
                    request_ctx.primary_route = primary_route or "none"
    ...
    async def _stream_text_module(
        ...
        if callable(get_processor):
            ...
            if callable(prepare):
                ...
                payload["text"] = str(prepared_request.get("normalized_text", text))
                payload["use_google_search"] = bool(prepared_request.get("use_google_search", False))
                primary_route = str(prepared_request.get("primary_route", "none") or "none")
        ...
        return _generator(), primary_route
    ...
    def _build_runtime_memory_context(
        ...
        if memory_context:
            ...
            logger.info(
                "MEMORY_PROMPT_BRIDGE short_len=%d medium_len=%d long_len=%d medium_included=%s",
            ...
            logger.info("MEMORY_PROMPT_BRIDGE short_len=0 medium_len=0 long_len=0 medium_included=False")
    ...
    @staticmethod
    def _topic_kind_for_route(primary_route: str) -> str:
        route = str(primary_route or "").strip().lower()
        mapping = {
            "whatsapp": "messaging",
            "messages": "messaging",
            "browser": "browser",
    ...
    def _canonical_primary_route(self, route_value: Optional[str]) -> Optional[str]:
