...
class MemoryWorkflowIntegration:
    ...
    def _strip_medium_term_from_context(self, memory_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'formatted_prompt' in sanitized:
            short_term = sanitized.get('short_term_context', '') or 'No data'
            long_term = sanitized.get('factual_long_term_context', '') or 'No data'
            sanitized['formatted_prompt'] = (
                "🧠 MEMORY CONTEXT\n\n"
                f"Short-term memory:\n{short_term}\n\n"
                "Medium-term memory:\nNo data\n\n"
                f"Long-term memory:\n{long_term}"
            )
        if sanitized.get('memory_gate') == 'disabled':
            sanitized['memory_gate'] = 'none'
        return sanitized
