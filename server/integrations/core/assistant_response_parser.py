#!/usr/bin/env python3
"""
Assistant Response Parser - парсинг и валидация ответов ассистента с поддержкой MCP команд

Поддерживает формат:
{
  "session_id": "<uuid>",
  "command": "open_app",
  "args": {
    "app_name": "Telegram"
  },
  "text": "Открываю Telegram, дайте знать, если понадобится что-то ещё."
}
"""

import json
import logging
import re
from typing import Dict, Any, Optional, Tuple, Union
from dataclasses import dataclass
from integrations.core.assistant_response_parser_bridge import (
    build_command_payload,
    extract_json_from_markdown,
    normalize_action_args,
    normalize_text_response,
    validate_action_payload,
)

# Try to import Pydantic for validation (optional dependency)
try:
    from .response_models import LLMResponseValidator, TextResponse, ActionResponse
    PYDANTIC_AVAILABLE = True
except ImportError:
    PYDANTIC_AVAILABLE = False
    LLMResponseValidator = None
    TextResponse = None
    ActionResponse = None

logger = logging.getLogger(__name__)
_MESSAGE_GLUE_SUFFIXES = ("soon", "today", "tomorrow", "please", "again", "later", "now")

def _log_request_path_parser(stage: str, session_id: Optional[str], **fields: Any) -> None:
    payload = " ".join(f"{k}={fields[k]}" for k in sorted(fields))
    logger.info(
        "REQUEST_PATH stage=%s component=assistant_response_parser session=%s %s",
        stage,
        session_id or "-",
        payload,
    )

def _get_runtime_allowlist() -> list[str]:
    """Resolve allowed commands via centralized runtime helper."""
    import_errors: list[str] = []
    for import_path in (
        "config.command_allowlist",
        "server.server.config.command_allowlist",
    ):
        try:
            module = __import__(import_path, fromlist=["get_allowed_commands_runtime"])
            getter = getattr(module, "get_allowed_commands_runtime", None)
            if callable(getter):
                return getter()
        except Exception as exc:
            import_errors.append(f"{import_path}: {exc}")
    raise RuntimeError("; ".join(import_errors) or "get_allowed_commands_runtime import failed")


@dataclass
class ParsedResponse:
    """Результат парсинга ответа ассистента"""
    text_response: str
    command_payload: Optional[Dict[str, Any]] = None
    session_id: Optional[str] = None
    raw_args: Optional[Dict[str, Any]] = None


class AssistantResponseParser:
    """
    Парсер и валидатор ответов ассистента
    
    Обрабатывает как обычные текстовые ответы, так и комбинированные ответы
    с MCP командами (command + args + text).
    """
    
    def __init__(self):
        """Инициализация парсера"""
        self.logger = logger

    def _normalize_command(self, command: Any) -> Any:
        """Нормализует имя команды к каноническому виду"""
        if not isinstance(command, str):
            return command
        normalized = command.strip().lower()
        normalized = normalized.replace("-", "_").replace(".", "_").replace(" ", "_")
        while "__" in normalized:
            normalized = normalized.replace("__", "_")
        return normalized

    def _normalize_action_payload(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Возвращает копию data с нормализованной командой (если есть)"""
        if 'command' not in data or not isinstance(data.get('command'), str):
            return data
        normalized = self._normalize_command(data['command'])
        if normalized == data['command']:
            return data
        normalized_data = data.copy()
        normalized_data['command'] = normalized
        return normalized_data

    def _looks_like_action_intent(self, text: Any) -> bool:
        """Быстрый эвристический детектор action-like текста для диагностики."""
        if not isinstance(text, str):
            return False
        text_l = text.lower()
        markers = (
            "send_message",
            "read_messages",
            "find_contact",
            "open_app",
            "close_app",
            "browser_use",
            "close_browser",
            "send_whatsapp_message",
            "read_whatsapp_messages",
            "buy_subscription",
            "manage_subscription",
            "отправ",
            "сообщен",
            "откро",
            "закро",
            "команд",
        )
        return any(marker in text_l for marker in markers)

    def _fallback_text_for_command(self, command: Any, args: Any) -> str:
        if not isinstance(command, str):
            return "I can help with that. Please tell me the exact action."
        cmd = command.strip().lower()
        if cmd == "open_app":
            app = args.get("app_name") if isinstance(args, dict) else None
            return f"Opening {app}." if app else "Opening the app."
        if cmd == "close_app":
            app = args.get("app_name") if isinstance(args, dict) else None
            return f"Closing {app}." if app else "Closing the app."
        if cmd == "send_message":
            return "Sending your message."
        if cmd == "read_messages":
            return "Reading your messages."
        if cmd == "find_contact":
            return "Searching for the contact."
        if cmd == "send_whatsapp_message":
            return "Sending a WhatsApp message."
        if cmd == "read_whatsapp_messages":
            return "Checking WhatsApp messages."
        if cmd == "browser_use":
            return "Starting the browser task."
        if cmd == "close_browser":
            return "Closing the browser."
        if cmd == "manage_subscription":
            return "Opening subscription management."
        if cmd == "buy_subscription":
            return "Opening the subscription purchase page."
        return "Done."

    def _normalize_browser_use_args(self, args: Any) -> Dict[str, Any]:
        """
        Backward-compatible normalization for browser_use.
        Accepts legacy shape (url/action/query) and converts to canonical args.task.
        """
        if not isinstance(args, dict):
            return {}
        normalized_args = dict(args)
        task = str(normalized_args.get("task", "") or "").strip()
        if task:
            return normalized_args

        action = str(normalized_args.get("action", "") or "").strip()
        query = str(normalized_args.get("query", "") or "").strip()
        url = str(normalized_args.get("url", "") or "").strip()

        # Canonical fallback synthesis for legacy payloads.
        synthesized_task = ""
        if action and query and url:
            synthesized_task = f"{action} {query} on {url}"
        elif action and query:
            synthesized_task = f"{action} {query}"
        elif action and url:
            synthesized_task = f"{action} on {url}"
        elif url:
            synthesized_task = f"Open {url}"
        elif query:
            synthesized_task = query

        if synthesized_task:
            normalized_args["task"] = synthesized_task
        return normalized_args

    def _normalize_message_args(self, args: Any) -> Dict[str, Any]:
        """Canonical normalization for message command args."""
        if not isinstance(args, dict):
            return {}
        normalized_args = dict(args)
        contact = normalized_args.get("contact")
        message = normalized_args.get("message")
        if isinstance(contact, str):
            normalized_args["contact"] = " ".join(contact.strip().split())
        if isinstance(message, str):
            normalized_args["message"] = self._normalize_message_text(message)
        return normalized_args

    def _normalize_message_text(self, text: str) -> str:
        cleaned = " ".join(str(text).strip().split())
        if not cleaned:
            return ""
        for suffix in _MESSAGE_GLUE_SUFFIXES:
            cleaned = re.sub(
                rf"([A-Za-z']{{3,}}){suffix}\b",
                rf"\1 {suffix}",
                cleaned,
                flags=re.IGNORECASE,
            )
        return cleaned

    def parse(self, response: Union[str, Dict[str, Any]], session_id: Optional[str] = None) -> ParsedResponse:
        """
        Парсинг ответа ассистента
        
        Args:
            response: Строка (JSON или обычный текст) или словарь
            session_id: Optional session ID to inject if missing in action responses
            
        Returns:
            ParsedResponse с text_response и опциональным command_payload
        """
        # Если уже словарь, используем напрямую
        if isinstance(response, dict):
            _log_request_path_parser(
                "parser.input",
                session_id,
                input_type="dict",
                has_command=bool(response.get("command")),
                has_text=bool(response.get("text")),
            )
            return self._parse_dict(response, session_id=session_id)

        # Пытаемся распарсить как JSON
        if isinstance(response, str):
            _log_request_path_parser(
                "parser.input",
                session_id,
                input_type="str",
                size=len(response),
            )
            return self._parse_string(response, session_id=session_id)
        
        # Неизвестный тип - возвращаем как текст
        self.logger.warning(f"Неизвестный тип ответа: {type(response)}, возвращаем как текст")
        return ParsedResponse(
            text_response=str(response),
            command_payload=None
        )
    
    def _extract_json_from_markdown(self, text: str) -> str:
        return extract_json_from_markdown(text)
    
    def _parse_string(self, response_str: str, session_id: Optional[str] = None) -> ParsedResponse:
        """
        Парсинг строкового ответа
        
        Args:
            response_str: Строка (может быть JSON или обычный текст)
            session_id: Optional session ID to inject if missing in action responses
            
        Returns:
            ParsedResponse
        """
        # Удаляем markdown-разметку перед парсингом
        cleaned_str = self._extract_json_from_markdown(response_str)
        
        # Пытаемся распарсить как JSON
        try:
            data = json.loads(cleaned_str)
            if isinstance(data, dict):
                return self._parse_dict(data, session_id=session_id)
            # Если JSON, но не словарь - возвращаем как текст
            return ParsedResponse(
                text_response=response_str,
                command_payload=None
            )
        except (json.JSONDecodeError, ValueError):
            # Не JSON - возвращаем как обычный текст
            return ParsedResponse(
                text_response=response_str,
                command_payload=None
            )
    
    def _parse_dict(self, data: Dict[str, Any], session_id: Optional[str] = None) -> ParsedResponse:
        """
        Парсинг словаря ответа с валидацией через Pydantic (если доступен)
        
        Args:
            data: Словарь с полями ответа
            session_id: Optional session ID to inject if missing in action responses
            
        Returns:
            ParsedResponse
        """
        normalized_data = self._normalize_action_payload(data)
        # Always propagate or enforce session_id (even for text-only JSON).
        if session_id:
            if normalized_data.get("session_id") and normalized_data.get("session_id") != session_id:
                self.logger.warning(
                    "⚠️ LLM session_id override: response=%s request=%s",
                    normalized_data.get("session_id"),
                    session_id,
                )
                try:
                    from utils.metrics_collector import record_decision_metric
                    record_decision_metric("assistant_response", "session_id_override")
                except Exception:
                    self.logger.debug("Failed to record session_id_override metric")
            normalized_data["session_id"] = session_id

        # Попытка валидации через Pydantic (если доступен)
        if PYDANTIC_AVAILABLE and LLMResponseValidator and ActionResponse is not None:
            # Используем session_id из данных, если есть, иначе из параметра
            context_session_id = normalized_data.get('session_id') or session_id
            validated_model, error_msg = LLMResponseValidator.validate_response(normalized_data, session_id=context_session_id)
            
            if validated_model:
                # Валидация успешна - используем валидированную модель
                if isinstance(validated_model, ActionResponse):
                    # Action-ответ
                    # Используем session_id из модели, если есть, иначе из параметра
                    final_session_id = validated_model.session_id or session_id
                    
                    if not final_session_id:
                        # Нет session_id - не можем выполнить команду
                        self.logger.warning(
                            f"⚠️ Action-ответ не может быть обработан: отсутствует session_id "
                            f"(command={validated_model.command}). Возвращаем только текст."
                        )
                        return ParsedResponse(
                            text_response=validated_model.text,
                            command_payload=None,
                            session_id=None,
                            raw_args=validated_model.args
                        )
                    
                    command_payload = {
                        'event': 'mcp.command_request',
                        'payload': {
                            'session_id': final_session_id,
                            'command': validated_model.command,
                            'args': validated_model.args
                        }
                    }
                    
                    action_text = (validated_model.text or "").strip()
                    if not action_text:
                        action_text = self._fallback_text_for_command(validated_model.command, validated_model.args)

                    self.logger.info(
                        f"✅ Валидирован action-ответ через Pydantic: command={validated_model.command}, "
                        f"session_id={final_session_id}, text_len={len(action_text)}"
                    )
                    
                    return ParsedResponse(
                        text_response=action_text,
                        command_payload=command_payload,
                        session_id=final_session_id,
                        raw_args=validated_model.args
                    )
                else:
                    # Text-only ответ
                    text_only = (validated_model.text or "").strip() or "I can help with that. Please tell me what you want to do."
                    self.logger.debug(f"✅ Валидирован text-only ответ через Pydantic: text_len={len(text_only)}")
                    return ParsedResponse(
                        text_response=text_only,
                        command_payload=None,
                        session_id=context_session_id,
                        raw_args=None
                    )
            else:
                # Валидация не прошла - логируем и используем fallback
                self.logger.warning(
                    f"⚠️ Pydantic валидация не прошла: {error_msg}. "
                    f"Используем fallback парсинг. Данные: {normalized_data}"
                )
                # Продолжаем с обычным парсингом ниже
        
        # Fallback: обычный парсинг без Pydantic (или если Pydantic недоступен)
        # Извлекаем text (обязательное поле, но может быть пустым)
        text_response = normalize_text_response(normalized_data.get('text', ''))
        if not text_response and not isinstance(normalized_data.get('text', ''), str):
            self.logger.warning("Поле 'text' не является строкой, подставляем пустую строку")

        # Проверяем наличие команды
        command = normalized_data.get('command')
        if command is None or command == '':
            # Обычный текстовый ответ без команды
            if not text_response:
                text_response = "I can help with that. Please tell me what you want to do."
            if self._looks_like_action_intent(text_response):
                self.logger.warning(
                    "[ACTION_PIPELINE][SERVER] stage=parse_no_command session=%s text_preview=%s",
                    normalized_data.get('session_id'),
                    text_response.replace("\n", "\\n")[:220],
                )
                _log_request_path_parser(
                    "parser.no_command_action_like_text",
                    normalized_data.get("session_id") or session_id,
                    text_preview=text_response.replace("\n", " ")[:120],
                )
            else:
                _log_request_path_parser(
                    "parser.text_only",
                    normalized_data.get("session_id") or session_id,
                    text_len=len(text_response),
                )
            return ParsedResponse(
                text_response=text_response,
                command_payload=None,
                session_id=normalized_data.get('session_id'),
                raw_args=None
            )
        
        # Action-ответ с командой
        # Используем session_id из данных, если есть, иначе из параметра
        action_session_id = normalized_data.get('session_id') or session_id
        args = normalize_action_args(
            command=command,
            args=normalized_data.get('args', {}),
            normalize_browser_use_args=self._normalize_browser_use_args,
            normalize_message_args=self._normalize_message_args,
        )
        normalized_data['args'] = args
        if not text_response:
            text_response = self._fallback_text_for_command(command, args)

        # Проверяем, что команда разрешена фичами (fallback без Pydantic)
        try:
            allowed_commands = _get_runtime_allowlist()
        except Exception as config_error:
            # Fail-closed: если не удалось получить allowlist, action path запрещаем.
            allowed_commands = []
            validation_errors = [
                f"Не удалось проверить allowlist команд (fail-closed): {config_error}"
            ]
        else:
            validation_errors = validate_action_payload(
                command=command,
                args=args,
                session_id=action_session_id,
                allowed_commands=allowed_commands,
            )

        
        # Логируем ошибки валидации
        if validation_errors:
            self.logger.warning(
                f"Ошибки валидации action-ответа (command={command}, session_id={action_session_id}): "
                f"{'; '.join(validation_errors)}"
            )
            self.logger.warning(
                "[ACTION_PIPELINE][SERVER] stage=parse_validation_failed session=%s command=%s args=%s errors=%s",
                action_session_id,
                command,
                args,
                validation_errors,
            )
            _log_request_path_parser(
                "parser.action_rejected",
                action_session_id,
                command=command,
                errors="|".join(validation_errors),
            )
            # Возвращаем только текст, без команды (fallback)
            return ParsedResponse(
                text_response=text_response,
                command_payload=None,
                session_id=action_session_id,
                raw_args=args
            )
        
        # Формируем command_payload для MCP
        command_payload = build_command_payload(
            session_id=action_session_id,
            command=command,
            args=args,
        )
        
        self.logger.info(
            f"Распознан action-ответ: command={command}, session_id={action_session_id}, "
            f"text_len={len(text_response)}"
        )
        self.logger.info(
            "[ACTION_PIPELINE][SERVER] stage=parse_success session=%s command=%s args=%s",
            action_session_id,
            command,
            args,
        )
        _log_request_path_parser(
            "parser.action_accepted",
            action_session_id,
            command=command,
            args_keys=",".join(sorted(args.keys())),
        )

        return ParsedResponse(
            text_response=text_response,
            command_payload=command_payload,
            session_id=action_session_id,
            raw_args=args
        )
