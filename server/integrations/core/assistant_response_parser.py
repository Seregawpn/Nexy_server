...
class AssistantResponseParser:
    ...
    def _get_allowed_commands_from_config(self) -> list[str]:
        """Возвращает allowlist команд из unified_config с устойчивым импортом."""
        get_config = None
        get_allowlist = None
        import_errors = []
        for import_path in (
            ...
            try:
                ...
                base_pkg = import_path.rsplit(".", 1)[0]
                allowlist_module = __import__(
                    f"{base_pkg}.command_allowlist",
                    fromlist=["get_allowed_commands_from_config"],
                )
                get_allowlist = getattr(allowlist_module, "get_allowed_commands_from_config", None)
                if callable(get_config):
                    break
            except Exception as exc:
                import_errors.append(f"{import_path}: {exc}")

        if not callable(get_config) or not callable(get_allowlist):
            raise RuntimeError("; ".join(import_errors) or "get_config import failed")

        cfg = get_config()
        return get_allowlist(cfg)
    ...
    def _parse_dict(self, data: Dict[str, Any], session_id: Optional[str] = None) -> ParsedResponse:
        ...
        try:
            allowed_commands = self._get_allowed_commands_from_config()
