...
async def test_short_term_memory_is_saved_as_chat_summary_without_db():
    ...
    assert "Recent chat summary:" in short_memory
    assert "- User:" in short_memory
    assert "- Assistant:" in short_memory

    context = await manager.get_memory_context("hw-chat")
    assert isinstance(context, dict)
    assert "Recent chat summary:" in context.get("recent_context", "")
