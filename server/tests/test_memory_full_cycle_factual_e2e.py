...
class _MatrixProvider:
    ...
    async def classify_intent_decision(self, user_input: str = "", classifier_context: Optional[Dict[str, str]] = None, **kwargs):
        ...
        if lowered in {
            ...
            category = "messages"
        elif lowered in {"nvm lost news insted", "nah just world headlines insted", "actually world headlines instead"}:
        ...
        if lowered in {
            ...
            category = "browser"
        elif lowered in {"sleep music", "rain sounds", "make it sleep music instead", "make it rain sounds instead", "switch it to sleep sounds", "switch it to sleep music"} and "youtube" in goal_norm:
            category = "browser"
            ...
            category = "google_search"
        elif lowered in {"world news", "world headlines", "world politics", "bitcoin news"} and "news" in goal_norm:
            category = "google_search"
        ...
        elif lowered in {"safari", "safari please", "safari app"} and ("open an app" in goal_norm or "open a program" in goal_norm):
            category = "system_control"
            ...
            category = "none"
        elif lowered in {"open", "open something for me", "open a program for me"}:
            category = "none"
        elif lowered in {
            "no",
            "not now",
            ...
            category = "none"
        elif lowered in {"no, open safari instead", "actually, open safari instead", "actually just launch safari"}:
            category = "system_control"
        elif lowered == "i want something else, find world news":
            category = "google_search"
        elif lowered in {"no, send a message to sophia instead", "actually, message sophia instead", "actually text sophia instead"}:
            category = "messages"
    ...
    async def process(
        ...
        elif lowered in {"snd her im outside", "snd her ill b outside in 2", "txt her im downstairs"}:
            response = self._response(
                session_id=session_id,
                text="Sending your message to Sophia.",
                command="send_message",
                args={
                    ...
                    "message": (
                        ...
                        else ("im downstairs" if lowered == "txt her im downstairs" else "im outside")
                    ),
                },
            )
        ...
        elif lowered in {"send a message to sophia", "i need to send sophia a message", "could you help me text sophia"}:
        ...
        elif lowered in {"tell her i will be late", "tell her i'll be late tonight", "let her know i'll be there in ten"} and route == "messages" and ("sophia" in runtime_goal and "message" in runtime_goal):
            response = self._response(
                session_id=session_id,
                text="Sending your message to Sophia.",
                command="send_message",
                args={
                    ...
                    "message": (
                        ...
                        if lowered == "tell her i'll be late tonight"
                        else ("I'll be there in ten" if lowered == "let her know i'll be there in ten" else "I will be late")
                    ),
                },
            )
        ...
        elif lowered in {"send a whatsapp message to mom", "can you whatsapp mom for me", "i need to whatsapp mom"}:
            response = self._response(session_id=session_id, text="What message do you want to send to Mom?")
        elif lowered in {"tell mom i arrived safely", "tell mom i got here safely", "tell mom dinner's ready"} and route == "whatsapp" and "whatsapp message to mom" in runtime_goal:
            response = self._response(
                session_id=session_id,
                ...
                args={
                    ...
                    "message": (
                        "I got here safely"
                        if lowered == "tell mom i got here safely"
                        else ("Dinner's ready" if lowered == "tell mom dinner's ready" else "I arrived safely")
                    ),
                },
            )
        ...
        elif lowered in {"open youtube and play jazz", "go to youtube and put on some jazz", "hop onto youtube and play some jazz"}:
            response = self._response(
                session_id=session_id,
                text=(
                    "Opening YouTube and putting on some jazz."
                    if lowered == "go to youtube and put on some jazz"
                    else ("Opening YouTube and playing some jazz." if lowered == "hop onto youtube and play some jazz" else "Opening YouTube and playing jazz.")
                ),
                command="browser_use",
                args={
                    "task": (
                        "go to youtube and put on some jazz"
                        if lowered == "go to youtube and put on some jazz"
                        else ("hop onto youtube and play some jazz" if lowered == "hop onto youtube and play some jazz" else "open youtube and play jazz")
                    )
                },
            )
        ...
        elif lowered in {"yt lofi insted", "yt lofi beats insted", "yt rain ambi insted"}:
            response = self._response(
                session_id=session_id,
                text=(
                    "Opening YouTube and playing lofi beats."
                    if lowered == "yt lofi beats insted"
                    else ("Opening YouTube and playing rain ambience." if lowered == "yt rain ambi insted" else "Opening YouTube and playing lofi.")
                ),
                command="browser_use",
                args={
                    "task": (
                        ...
                        if lowered == "yt lofi beats insted"
                        else ("open youtube and play rain ambience" if lowered == "yt rain ambi insted" else "open youtube and play lofi")
                    )
                },
            )
        ...
        elif lowered in {"sleep music", "rain sounds", "make it sleep music instead", "make it rain sounds instead", "switch it to sleep sounds", "switch it to sleep music"} and route == "browser" and "youtube" in runtime_goal:
            response = self._response(
                session_id=session_id,
                text=(
                    "Opening YouTube to find sleep music."
                    if lowered in {"make it sleep music instead", "switch it to sleep sounds", "switch it to sleep music"}
                    else "Opening YouTube."
                ),
                command="browser_use",
                args={
                    "task": (
                        "open youtube and find sleep music"
                        if lowered in {"switch it to sleep sounds", "switch it to sleep music", "make it sleep music instead"}
                        else (
                            "open youtube and find rain sounds"
                            if lowered == "make it rain sounds instead"
                            else f"open youtube and play {lowered}"
                        )
                    )
                },
            )
        ...
        elif lowered in {"find latest world news", "what's the latest world news", "what's the latest world news?", "whats the latest world news", "give me the latest world headlines"}:
        ...
        elif lowered in {"nvm lost news insted", "nah just world headlines insted", "nah jus global headlines insted", "actually world headlines instead"}:
            response = self._response(
                session_id=session_id,
                text="Here are the latest global headlines." if lowered == "nah jus global headlines insted" else "Here are the latest world news headlines.",
            )
        elif lowered == "btc price now":
            response = self._response(session_id=session_id, text="Here is the current Bitcoin price.")
        elif lowered in {"world news", "world headlines"} and route == "google_search":
            response = self._response(session_id=session_id, text="Here are the latest world news headlines.")
        ...
        elif lowered in {"deskribe my scren", "wts on my scrn rn", "whats on my screen rn", "wuts on screen rn"}:
            response = self._response(session_id=session_id, text="Describing your screen.")
        elif lowered in {"open safari", "could you open safari", "could you open safari?", "safari please", "please launch safari", "safari app"}:
            response = self._response(
                session_id=session_id,
                text="Opening Safari.",
                command="open_app",
        ...
        elif lowered in {"safari", "safari please"} and route == "system_control":
            response = self._response(
                session_id=session_id,
                text="Opening Safari.",
                command="open_app",
        ...
        elif lowered in {"what can you do", "what all can you help with", "what all can you help with?", "what sort of things can you do", "what sort of things can you do?"}:
            response = self._response(
                session_id=session_id,
                text="I can help with apps, browser, messages, search, screen description, and device actions.",
            )
        elif lowered in {"how are you?", "how are you", "how's it going?", "how's it going", "hows it going?", "hows it going", "you doing okay?", "you doing okay"}:
