...
Options:
  ...
  --channel NAME          Update channel: stable|beta|alpha (default: stable)
...
case "$CHANNEL" in
  ...
  alpha) MANIFEST_NAME="manifest_alpha.json" ;;
  *)
    echo "❌ Unsupported --channel: $CHANNEL (allowed: stable|beta|alpha)"
    exit 1
    ;;
esac
