...
CHANNELS: dict[str, ChannelConfig] = {
    ...
    "alpha": ChannelConfig(
        name="alpha",
        update_tag="UpdateAlpha",
        app_tag="AppAlpha",
        manifest_file=MANIFESTS_DIR / "manifest_alpha.json",
    ),
}
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    publish = subparsers.add_parser("publish", help="Publish release inbox artifacts to selected channel")
    publish.add_argument("--channel", default="stable", choices=sorted(CHANNELS), help="Target update channel")
...
def main() -> int:
    ...
    # Backward compatible default behavior: no subcommand == publish stable
    ...
    try:
        ...
        if command == "publish":
            channel_name = getattr(args, "channel", "stable")
