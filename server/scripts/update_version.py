...
Editable source of truth for update runtime: server/updates/manifests/manifest.json.
VERSION/unified_config are derived and synced by this script.
...
Usage:
  ...
  python3 server/scripts/update_version.py --from-manifest
...
def _read_manifest_version() -> str:
    if not SERVER_MANIFEST.exists():
        raise FileNotFoundError(f"Manifest file not found: {SERVER_MANIFEST}")
    data = json.loads(SERVER_MANIFEST.read_text(encoding="utf-8"))
    version = str(data.get("version", "")).strip()
    if not version:
        raise ValueError(f"Manifest has empty version: {SERVER_MANIFEST}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--from-manifest",
        action="store_true",
        help="Use server/updates/manifests/manifest.json as source version",
    )
    ...
    try:
        ...
        if args.from_manifest:
            if args.version:
                parser.error("version positional argument is not allowed with --from-manifest")
            version = _read_manifest_version()
        else:
            if not args.version:
                parser.error("version is required unless --read/--from-manifest is used")
        ...
        source = "manifest.json" if args.from_manifest else "CLI version argument"
        print(f"✅ Version synchronized from source ({source}): {version}")
        if unique_changed:
            print("Updated files:")
