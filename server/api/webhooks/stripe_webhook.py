...
async def stripe_webhook_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        # Verify signature
        webhook_secrets = _collect_webhook_secrets(config=config, request=request)
...
async def _verify_and_construct_event(
    payload: bytes,
    sig_header: str,
    webhook_secrets: str | Iterable[str],
    ...
    """
    try:
        candidate_secrets = _normalize_webhook_secrets(webhook_secrets)
        ...
        if last_signature_error is not None:
            logger.error(
                "[F-2025-017] Signature verification failed for all configured webhook secrets "
                f"(count={len(candidate_secrets)}): {last_signature_error}"
            )
        return None
...
def _normalize_webhook_secrets(value: str | Iterable[str] | None) -> tuple[str, ...]:
    """Normalize webhook secrets from string/csv/iterable to a deduplicated tuple."""
    if value is None:
        return tuple()
    chunks: list[str] = []
...
def _collect_webhook_secrets(config: Any, request: web.Request) -> tuple[str, ...]:
    """
    Resolve all accepted webhook secrets (single owner-path for signature verification).
    Priority:
    1) Optional server source override header: X-Nexy-Webhook-Source -> STRIPE_WEBHOOK_SECRET_<SOURCE>
    2) Active config secret (mode-bound)
    3) STRIPE_WEBHOOK_SECRETS (comma-separated shared secrets)
    4) Explicit test/live secrets (cross-runtime compatibility)
    """
    candidates: list[str] = []
    source = str(request.headers.get("X-Nexy-Webhook-Source", "")).strip().upper()
    if source:
        normalized_source = re.sub(r"[^A-Z0-9_]", "_", source).strip("_")
        if normalized_source:
            source_key = f"STRIPE_WEBHOOK_SECRET_{normalized_source}"
            source_secret = os.getenv(source_key, "")
            if source_secret:
                candidates.append(source_secret)

    candidates.append(str(getattr(config, "stripe_webhook_secret", "") or ""))
    candidates.append(os.getenv("STRIPE_WEBHOOK_SECRETS", ""))
    candidates.append(os.getenv("STRIPE_TEST_WEBHOOK_SECRET", ""))
    candidates.append(os.getenv("STRIPE_LIVE_WEBHOOK_SECRET", ""))
    return _normalize_webhook_secrets(candidates)
