...
## 🔀 ГРАНИЦЫ PIPELINE (ОБЯЗАТЕЛЬНО)

- **Code + Deploy pipeline:** `https://github.com/Seregawpn/Nexy_server`
  - Этот документ описывает только этот pipeline.
  - Назначение: хранение серверного кода и деплой на Azure VM.
...
**Подробности:** раздел "УСТРАНЕНИЕ ПРОБЛЕМ" в этом документе и `../../Docs/RELEASE_AND_UPDATE_GUIDE.md`.
...
- ❌ Не используйте `--force` без аварийной необходимости и явного согласования
...
## 🛠️ **НАДЕЖНЫЙ РУЧНОЙ ДЕПЛОЙ (AZURE CLI)**

Если GitHub Actions недоступен или требуется полный контроль над обновлением (включая обновление зависимостей), используйте этот метод.
...
- ✅ **Точный контроль версии** (checkout по тегу)
- ✅ **Обновление зависимостей** (`pip install --upgrade`)
- ✅ **Изоляция** (выполняется внутри VM)
- ✅ **Логирование** (вывод команды показывает результат)
...
## 🚀 **КЛИЕНТСКИЕ АРТЕФАКТЫ (ОТДЕЛЬНЫЙ PIPELINE)**

Этот документ не является каноном для публикации DMG/PKG.

Для клиентских артефактов используйте только:
- `../../Docs/RELEASE_AND_UPDATE_GUIDE.md`

Кратко:
- Репозиторий артефактов: `Seregawpn/Nexy_production/releases`
- Фиксированные теги: `Update` (DMG), `App` (PKG)
- Проверка канала обновлений: `https://nexy-server.canadacentral.cloudapp.azure.com/updates/appcast.xml`
