...
- `Docs/VERSION_MANAGEMENT.md` — канон системы обновлений (включая форматы версий) — единственный источник истины
- `Docs/VERSION_MANAGEMENT.md` — канон обновлений, синхронизация размеров с GitHub CDN
- `Docs/SERVER_DEPLOYMENT_GUIDE.md` — план раскатки трафика, гвардрайлы, этапы
...
- `server/server/modules/*` — только бизнес-логика. Оркестрация и сценарии располагаются в `server/server/integrations/{core,service_integrations,workflow_integrations}`.
- Прямых импортов между модулями нет; все обращения идут через `ModuleCoordinator` (`server/server/integrations/service_integrations/module_coordinator.py`).
...
- Единственный proto-файл — `server/server/modules/grpc_service/streaming.proto`. Изменения обновляют его и `Docs/GRPC_PROTOCOL_AUDIT.md`.
- Лимиты, таймауты и флаги читаются из `server/server/config/unified_config.py` или env-override; хардкоды запрещены.
...
- [ ] Локальные тесты пройдены: `pytest server/server/tests/test_pr2_1_coordinator.py` и `python server/server/scripts/grpc_smoke.py localhost 50051` (или прикладываем логи из CI)

---

## 2. Версии и обновления: один канон

**Источник истины:** `Docs/VERSION_MANAGEMENT.md` — раздел "КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Форматы версий" — единственный источник истины
...
**Детальная проверка:** см. `Docs/VERSION_MANAGEMENT.md` — раздел "КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Форматы версий" (таблица компонентов, проверки, скрипты)
...
**Источник истины:** `Docs/VERSION_MANAGEMENT.md`
...
**Канон:** `Docs/ARCHITECTURE_OVERVIEW.md` — итоговый summary с актуальным статусом реализации MCP интеграции.
...
- `Docs/ARCHITECTURE_OVERVIEW.md` — канон (итоговый summary)
- `Docs/_archive/MCP_COMMAND_INTEGRATION_PLAN.md` — исторический план (для справки, не канон)

---

## 7. Политика совместимости gRPC (PR-3)

**Канон:** proto-файл хранится только в `server/server/modules/grpc_service/streaming.proto`; альтернативные пути запрещены.
...
- `scripts/grpc_smoke.py` (production) и `server/server/scripts/grpc_smoke.py` (локально) — smoke-тест для проверки совместимости
...
- Статусы в `Docs/assistant_exchange`, `Docs/ARCHITECTURE_OVERVIEW.md`, релиз-заметки и отчёты используют только факты: ссылки на тест-планы, логи запусков, хэши билдов.
...
4. **Post mortem (5-минутное окно логов)**
   ...
   - Документировать причину отката в `Docs/assistant_exchange`
...
**Детальный runbook:** см. также `Docs/_archive/CANARY_CHECKLIST.md` (reference) → раздел "Rollback"
...
| 🔄 Обновления | `Docs/SERVER_DEPLOYMENT_GUIDE.md` | Пайплайн деплоя, откат, kill-switch |
| 🧩 FSM/States | `Docs/STATE_CATALOG.md` | Оси состояний, метрики, владельцы |
| ⚙️ Роллаут | `Docs/SERVER_DEPLOYMENT_GUIDE.md` | План раскатки трафика, гвардрайлы, этапы |
...
| ✅ Canary | `Docs/_archive/CANARY_CHECKLIST.md` | Исторический чеклист canary (reference) |
| 🚀 Beta Gate | `Docs/_archive/BETA_GATE_CHECKLIST.md` | Исторический чеклист beta gate (reference) |
...
| **Область** | **Документ** | **Описание** |
|------------|--------------|--------------|
| 🔴 Версионирование | `Docs/VERSION_MANAGEMENT.md` | **Канон системы обновлений** (включая форматы версий) — источник истины |
| 🔧 Update Fixes | `Docs/VERSION_MANAGEMENT.md` | **Канон обновлений** — синхронизация размеров с GitHub CDN |
...
- Шаблон PR-чеклиста: `Docs/_archive/PR_CHECKLIST_TEMPLATE.md` (reference)
...
**План раскатки:** см. `Docs/SERVER_DEPLOYMENT_GUIDE.md` (этапы, гвардрайлы, метрики)
...
1. **Форматы версий:** только строковые `"1.0.1"`, источник — `Docs/VERSION_MANAGEMENT.md` (раздел "КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Форматы версий")
2. **Размеры и подписи:** всегда сверяются с GitHub CDN (`Content-Length`, `SHA256`, `Ed25519`) — см. `Docs/VERSION_MANAGEMENT.md`
