...
- 🚀 **Текущий этап: Система обновлений** - GitHub интеграция + AppCast XML + тестирование
- ✅ **Система обновлений исправлена** - размеры файлов синхронизированы, локальные конфликты устранены
...
- gRPC Server (modules/grpc_service/core/grpc_server.py)
  ...
  - **Backward compatibility** - строгое соблюдение совместимости протокола (PR-3)

- GrpcServiceManager (modules/grpc_service/core/grpc_service_manager.py)
  - Управление жизненным циклом gRPC сервиса
  - Координация с другими модулями сервера через ModuleCoordinator (PR-2.1)
  - Обработка запросов и ответов
  - Поддержка feature-flags для версионирования (v1/v2)

- gRPC Protocol (`modules/grpc_service/streaming.proto`)
  - **Синтаксис:** proto3
  - **Сервис:** StreamingService
  - **RPC методы:**
    - `StreamAudio` (unary_stream) - стриминг аудио и текста
    - `InterruptSession` (unary_unary) - прерывание активной сессии
  - **Правила совместимости:**
    - ✅ Разрешены только добавления optional полей
    - ✅ Разрешены новые RPC методы с совместимой сигнатурой
    - ❌ Breaking changes → v2 сервис + feature-flag
    - ❌ Удаление полей/методов запрещено
  - **Документация:** `Docs/GRPC_PROTOCOL_AUDIT.md` (PR-3)
...
- Feature Flags и Kill-Switches (server/config/unified_config.py)
  - **use_module_coordinator** (bool, default=True) - включение ModuleCoordinator для изоляции модулей
  - **disable_module_coordinator** (bool, default=False) - экстренное отключение координатора (kill-switch)
  - **Назначение:** Безопасный rollout новой архитектуры с возможностью быстрого отката
  - **Правила:**
    - При `disable_module_coordinator=True` → откат к legacy прямым импортам
    - При `use_module_coordinator=False` → координатор не используется, но модули доступны
    - Feature flags загружаются из переменных окружения: `FEATURE_USE_MODULE_COORDINATOR`, `KILL_SWITCH_DISABLE_MODULE_COORDINATOR`
  - **Мониторинг:** Все переключения feature flags логируются с `scope=feature_flags decision=toggle`
...
### ✅ **gRPC INTERCEPTORS И ОБРАБОТКА ОШИБОК (PR-7)**

#### **Карта интерсепторов (исключения → коды):**

| **Исключение** | **gRPC Код** | **Классификация** | **Примечание** |
|---------------|--------------|-------------------|----------------|
| `ConnectionError` | `UNAVAILABLE` | transient | Временная сетевая ошибка |
| `TimeoutError` | `DEADLINE_EXCEEDED` | transient | Превышен таймаут |
| `asyncio.TimeoutError` | `DEADLINE_EXCEEDED` | transient | Асинхронный таймаут |
| `ValueError` | `INVALID_ARGUMENT` | permanent | Невалидные входные данные |
| `TypeError` | `INVALID_ARGUMENT` | permanent | Неправильный тип данных |
| `MemoryError` | `RESOURCE_EXHAUSTED` | transient | Нехватка памяти |
| `RuntimeError` | `INTERNAL` | permanent | Внутренняя ошибка |
| `Exception` (по умолчанию) | `INTERNAL` | permanent | Неизвестная ошибка |
...
**Таблица: исключение → gRPC-код → client-action:**

| **Исключение** | **gRPC Код** | **Классификация** | **Client Action** | **Retry Strategy** |
|---------------|--------------|-------------------|-------------------|-------------------|
| `ConnectionError` | `UNAVAILABLE` | transient | Показать "Соединение потеряно", retry с exponential backoff | Exponential backoff: 1s, 2s, 4s, 8s, max 30s |
| `TimeoutError` | `DEADLINE_EXCEEDED` | transient | Показать "Таймаут", retry с jitter | Exponential backoff + jitter: 2s±1s, 4s±2s, max 60s |
| `asyncio.TimeoutError` | `DEADLINE_EXCEEDED` | transient | Показать "Таймаут", retry с jitter | Exponential backoff + jitter: 2s±1s, 4s±2s, max 60s |
| `ValueError` | `INVALID_ARGUMENT` | permanent | Показать "Ошибка ввода", не retry | Нет retry, пользователь должен исправить входные данные |
| `TypeError` | `INVALID_ARGUMENT` | permanent | Показать "Ошибка ввода", не retry | Нет retry, пользователь должен исправить входные данные |
| `MemoryError` | `RESOURCE_EXHAUSTED` | transient | Показать "Нехватка ресурсов", retry через 30s | Fixed delay: 30s, max 3 попытки |
| `RuntimeError` | `INTERNAL` | permanent | Показать "Внутренняя ошибка", не retry | Нет retry, логировать и отправить отчет |
| `Exception` (по умолчанию) | `INTERNAL` | permanent | Показать "Внутренняя ошибка", не retry | Нет retry, логировать и отправить отчет |
| Stream limit exceeded | `RESOURCE_EXHAUSTED` | transient | Показать "Сервер перегружен", retry через 60s | Fixed delay: 60s, max 2 попытки |
| Rate limit exceeded | `RESOURCE_EXHAUSTED` | transient | Показать "Слишком много запросов", retry через 10s | Fixed delay: 10s, max 3 попытки |

**Файл:** `modules/grpc_service/core/grpc_interceptor.py`

---

### ✅ **BACKPRESSURE ДЛЯ СТРИМОВ (PR-7)**

#### **Политики backpressure (лимиты/таймауты/коды отказа):**

| **Параметр** | **Значение** | **Описание** |
|-------------|--------------|--------------|
| `max_concurrent_streams` | 50 | Максимум одновременно открытых StreamAudio |
| `idle_timeout_seconds` | 300 (5 минут) | Таймаут для неактивных стримов |
...
**Коды отказа:**
- `RESOURCE_EXHAUSTED` — превышен лимит стримов или rate limit
- `DEADLINE_EXCEEDED` — превышен idle timeout
...
**Таблица: дефолтные лимиты backpressure по окружениям:**
...
**Конфигурация:**
- Лимиты загружаются из `unified_config.yaml` → секция `backpressure`
- Переопределение через переменные окружения: `BACKPRESSURE_MAX_STREAMS`, `BACKPRESSURE_IDLE_TIMEOUT`, `BACKPRESSURE_MAX_RATE`, `BACKPRESSURE_GRACE_PERIOD`
- Автоматический выбор по `NEXY_ENV` (dev/stage/prod)

**Файл:** `modules/grpc_service/core/backpressure.py`

---

### ✅ **GRACEFUL SHUTDOWN (PR-7)**

#### **Схема graceful-shutdown (этапы, тайминги, финальные логи):**

**Этапы:**

1. **Получение сигнала** (SIGTERM/SIGINT)
   - Время: < 1 секунда
   - Действие: Установка `shutdown_event`
   - Лог: `scope=server decision=shutdown ctx={"signal": 15}`

2. **Остановка приёма новых RPC**
   - Время: < 1 секунда
   - Действие: Сервер перестаёт принимать новые запросы
   - Лог: `scope=server decision=shutdown ctx={"action": "stop_accepting"}`

3. **Ожидание активных RPC**
   - Время: до 5 секунд (grace period)
   - Действие: Ожидание завершения активных запросов
   - Лог: `scope=server decision=shutdown ctx={"active_requests": 3, "grace_period": 5}`

4. **Остановка backpressure manager**
   - Время: < 1 секунда
   - Действие: Остановка фоновой задачи очистки
   - Лог: `scope=backpressure decision=stop ctx={}`

5. **Логирование итоговых метрик**
   - Время: < 1 секунда
   - Действие: Финальный снапшот метрик
   - Лог: `scope=metrics decision=snapshot ctx={"p95_latency": {...}, "error_rate": {...}, "decision_rate": {...}}`

6. **Очистка всех серверов**
   - Время: < 2 секунды
   - Действие: Остановка HTTP, gRPC, Update серверов
   - Лог: `scope=server decision=stop ctx={"reason": "graceful_shutdown"}`

**Общее время:** ≤ 10 секунд

**Файл:** `main.py` (функции `setup_signal_handlers()`, `graceful_shutdown()`)
...
### **Серверная часть:**
- **UnifiedServerConfig** (`server/config/unified_config.py`) — единый источник конфигурации для всех модулей сервера
- **Переменные окружения** (`server/config.env`) — все настройки читаются из переменных окружения
- **Пример конфигурации** (`server/config.env.example`) — шаблон с полным списком переменных

**Текущее состояние модулей:**
- ✅ `text_processing/config.py` — использует `unified_config`
- ✅ `audio_generation/config.py` — использует `unified_config`
- ✅ `memory_management/config.py` — использует `unified_config`
- ⚠️ Остальные модули читают напрямую из env (можно улучшить в будущих PR)

**Правила:**
- Все модули должны использовать `config.unified_config.get_config()` для получения конфигурации
- Никаких прямых загрузок YAML/JSON файлов вне `unified_config.py` (кроме данных, например манифестов обновлений)
- Все параметры читаются из переменных окружения (`config.env`)
- Секреты (API ключи, пароли) не коммитятся в репозиторий
...
### 🌐 ВАЖНО: Сетевая схема входа (Ingress) — обновление (октябрь 2025)

- Внешний доступ теперь осуществляется через HTTPS порт 443 с реверс‑прокси Nginx (HTTP/2):
  - gRPC: корневой путь `/` (без `/grpc`), проксируется на `127.0.0.1:50051` через `grpc_pass`
  - Update Server: путь `/updates/…` проксируется на `127.0.0.1:8081` (HTTP/REST)
- Прежние внешние порты `50051` и `8081` могут блокироваться сетями/провайдерами и больше не публикуются наружу.
- TLS: используется временный self‑signed сертификат c SAN=IP (`20.151.51.172`) до подключения домена/Let’s Encrypt.

**Сводная таблица портов и путей:**

| **Тип доступа** | **Порт/Протокол** | **Путь** | **Назначение** | **Проксирование** |
|----------------|-------------------|----------|---------------|-------------------|
| **Внешний** | `443/HTTPS` (HTTP/2) | `/` | gRPC API | `grpc_pass grpc://127.0.0.1:50051` |
| **Внешний** | `443/HTTPS` (HTTP/2) | `/updates/` | Update Server | `proxy_pass http://127.0.0.1:8081/` |
| **Внешний** | `443/HTTPS` (HTTP/2) | `/updates/health` | Update Server Health | `proxy_pass http://127.0.0.1:8081/health` |
| **Внешний** | `443/HTTPS` (HTTP/2) | `/health` | Health check | `proxy_pass http://127.0.0.1:8080/health` |
| **Внешний** | `443/HTTPS` (HTTP/2) | `/status` | Status API | `proxy_pass http://127.0.0.1:8080/status` |
| **Внутренний** | `50051/gRPC` | - | gRPC Server | Только localhost (через Nginx) |
| **Внутренний** | `8080/HTTP` | `/health`, `/status` | Health/Status API | Только localhost (через Nginx) |
| **Внутренний** | `8081/HTTP` | `/`, `/health`, `/appcast.xml` | Update Server | Только localhost (через Nginx) |

**Важно:**
- Внешние порты `50051` и `8081` **не публикуются наружу** (могут блокироваться сетями/провайдерами)
- Все внешние запросы идут через `443/HTTPS` (Nginx reverse proxy)
- Внутренние порты слушаются только на `127.0.0.1` (localhost)

Требования к клиенту:
- gRPC endpoint: `https://20.151.51.172` (HTTP/2, TLS)
- Update endpoints: `https://20.151.51.172/updates/...`
- На время self‑signed TLS: либо доверить сертификат, либо временно отключить строгую проверку.
...
| Nginx Proxy | `443` | HTTPS/HTTP2 | Reverse proxy → gRPC (50051) + HTTP (8080) + Updates (8081) | ✅ Активен |
| Update Server | `8081` | HTTP | Sparkle обновления клиентов | ✅ Готов |

#### **Nginx HTTP/2 gRPC Passthrough (PR-7):**

**Конфигурация:**
- **HTTP/2 включен** — `listen 443 ssl http2`
- **gRPC passthrough** — `grpc_pass grpc://grpc_backend` на корне `/`
- **Таймауты:**
  - `grpc_read_timeout 300s` — для длительных стримов
  - `grpc_send_timeout 300s` — для отправки
  - `grpc_connect_timeout 10s` — на подключение
- **Буферизация отключена** — `proxy_request_buffering off` для стримов
- **Cache-Control:**
  - `appcast.xml`: `max-age=60` (1 минута)
  - `/updates/health`: `max-age=30` (30 секунд)
  - `/health`, `/status`: `max-age=30` (30 секунд)

**Файл:** `nginx/grpc-passthrough.conf`
...
Все модули должны реализовывать `UniversalModuleInterface` и регистрироваться через `ModuleCoordinator`.

| **Модуль** | **Capability** | **Интеграция/Оркестратор** | **Статус** |
|------------|---------------|---------------------------|------------|
| `grpc_service` | `grpc` | `GrpcServiceIntegration` + `ModuleCoordinator` | ✅ Использует координатор (PR-2.1) |
| `text_processing` | `text_processing` | `StreamingWorkflowIntegration` | ✅ Нативный модуль |
| `audio_generation` | `audio_generation` | `StreamingWorkflowIntegration` | ✅ Адаптер (PR-2.1) |
| `database` | `database` | `ModuleCoordinator` | ✅ Адаптер (PR-2.1) |
| `session_management` | `session` | `ModuleCoordinator` | ✅ Адаптер (PR-2.1) |
| `memory_management` | `memory` | `MemoryWorkflowIntegration` | ✅ Адаптер (PR-2.1) |
| `interrupt_handling` | `interrupt` | `InterruptWorkflowIntegration` | ✅ Адаптер (PR-2.1) |
| `text_filtering` | `text_filtering` | `StreamingWorkflowIntegration` | ✅ Адаптер (PR-2.1) |
| `update` | `update` | `ModuleCoordinator` | ⚠️ Требует обновления |

**Правила:**
- Все модули реализуют `UniversalModuleInterface` (нативно или через адаптер)
- Регистрация через `ModuleCoordinator.register(capability, module, config)`
- Получение модулей через `coordinator.get(capability)`
- Прямые межмодульные вызовы запрещены
- В `grpc_service_manager.py` нет прямых импортов модулей (только через адаптеры)

**Фича-флаги и Kill-switches:**
- `features.use_module_coordinator` — включение/выключение использования координатора
- `kill_switches.disable_module_coordinator` — экстренное отключение координатора
- Конфигурация в `unified_config.py` (FeaturesConfig, KillSwitchesConfig)

**Диаграмма потока:**
```
gRPC → GrpcServiceManager → ModuleCoordinator → Modules (adapters/native)
                                                      ↓
