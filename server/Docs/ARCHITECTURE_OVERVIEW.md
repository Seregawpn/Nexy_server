...
- **Ingress**: один публичный вход — HTTPS:443 через Nginx (`server/nginx/`). Внутренние адреса `127.0.0.1:50051`, `127.0.0.1:8080`, `127.0.0.1:8081` не публикуются в скриптах.
- **Cache-Control**: `/appcast.xml` — `max-age=60`, `/updates/health` — `max-age=30`, `/health` — `max-age=30`. Проверяется вручную и в CI.
