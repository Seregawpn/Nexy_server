...
## 2.3 Database Durability & Protection (обязательный канон)
...
## 2.2 Session Management и Concurrency (обновлено 21 января 2026)
...
Сервер поддерживает AI-управляемую автоматизацию браузера через модуль `browser_use` (Feature ID: F-2025-015).

**Возможности:**
- Выполнение веб-задач с помощью AI-агента (поиск, клики, заполнение форм)
- Персистентные сессии браузера (браузер не закрывается между задачами)
...
|------------|--------------|----------|
| `BROWSER_USE_ENABLED` | `true` | Включить модуль browser automation |
| `BROWSER_USE_KEEP_OPEN` | `true` | Сохранять браузер открытым между задачами |
| `BROWSER_USE_TIMEOUT` | `120` | Таймаут задачи (секунды) |
| `BROWSER_USE_MAX_STEPS` | `50` | Максимум шагов агента на задачу |
| `BROWSER_USE_MODEL` | `gemini-2.5-flash` | LLM модель для browser-агента |

**Автоматическая установка:**
При запуске сервера, если браузер (Chromium) не установлен, модуль автоматически выполняет `playwright install chromium`. Если это занимает >2 секунд, воспроизводится голосовое уведомление (только macOS).

**Команды:**
- `browser_use` — Выполнить веб-задачу
- `close_browser` — Явно закрыть персистентную сессию браузера
