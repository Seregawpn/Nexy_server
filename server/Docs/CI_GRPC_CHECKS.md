  ...
  grpc-checks:
    ...
    steps:
      ...
      - name: gRPC Smoke Test
        run: |
          python scripts/grpc_smoke.py nexy-server.canadacentral.cloudapp.azure.com 443 || echo "⚠️ Smoke test skipped (server may be down)"
        continue-on-error: true
      
      - name: Health/Status Check
        run: |
          python scripts/check_grpc_health.py nexy-server.canadacentral.cloudapp.azure.com 443 || echo "⚠️ Health check skipped (server may be down)"
        continue-on-error: true
      
      - name: Port Check
        run: |
          nc -zv nexy-server.canadacentral.cloudapp.azure.com 50051 || echo "⚠️ Port check skipped (Nginx reverse proxy)"
        continue-on-error: true
...
- name: Port Check
  run: |
    nc -zv nexy-server.canadacentral.cloudapp.azure.com 50051 || echo "Port check skipped"
```
...
```yaml
      - name: Version Consistency Check
        run: |
          ...
          HEALTH_VERSION=$(curl -s https://nexy-server.canadacentral.cloudapp.azure.com/health | jq -r '.latest_version')
          APPCAST_VERSION=$(curl -s https://nexy-server.canadacentral.cloudapp.azure.com/updates/appcast.xml | grep -o 'sparkle:version="[^"]*"' | cut -d'"' -f2)
      ...
      - name: Validate Release Size (PR-8)
        ...
        run: |
          ...
          APPCAST_XML=$(curl -s -k https://nexy-server.canadacentral.cloudapp.azure.com/updates/appcast.xml || echo "")
...
python scripts/grpc_smoke.py nexy-server.canadacentral.cloudapp.azure.com 443 || echo "⚠️ Smoke test skipped"

# 3. Health check
echo "3. Running health check..."
python scripts/check_grpc_health.py nexy-server.canadacentral.cloudapp.azure.com 443 || echo "⚠️ Health check skipped"
...
- name: gRPC Compatibility Checks
  ...
  run: |
    python scripts/grpc_smoke.py nexy-server.canadacentral.cloudapp.azure.com 443 || true
    python scripts/check_grpc_health.py nexy-server.canadacentral.cloudapp.azure.com 443 || true
```
...
4. **Версии согласованы:**
   ```bash
   curl -s https://nexy-server.canadacentral.cloudapp.azure.com/health | jq '.latest_version, .latest_build'
   curl -s https://nexy-server.canadacentral.cloudapp.azure.com/updates/appcast.xml | grep sparkle:version
   ```
