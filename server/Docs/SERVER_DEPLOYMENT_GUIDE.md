...
## 3.1) Staging -> Production Cutover (обязательно)

Цель: сначала полностью настроить и проверить сервер на staging VM, затем быстро перевести тот же release в production без ручной перенастройки.

### Stage 1 — staging deploy и настройка
...
curl -fsS https://<STAGING_HOST>/status
curl -fsS https://<STAGING_HOST>/updates/health
python3 server/scripts/grpc_smoke.py --host <STAGING_HOST> --port 443
```

### Stage 2 — production cutover (после approve)
...
Правило:
- до прохождения Stage 1 production deploy запрещён;
- cutover выполняется тем же проверенным `--tag`, без дополнительных изменений кода.
