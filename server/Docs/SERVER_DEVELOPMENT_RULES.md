...
- [ ] Манифест собран через `server/updates/scripts/generate_manifest.py` (поля `artifact.size` и `artifact.sha256` проставлены автоматически)
...
**⚠️ ВАЖНОЕ ПРАВИЛО:** При обновлении версии в манифесте на сервере (`version` и `build`) вручную указываем только версию/ссылки:
1. **`artifact.size`** — заполняется автоматически генератором манифеста
2. **`artifact.sha256`** — заполняется автоматически генератором манифеста
...
3. **🔴 КРИТИЧНО: Обновление манифеста:** при обновлении версии (`version`/`build`) использовать `server/updates/scripts/generate_manifest.py`; `artifact.size`/`artifact.sha256` генерируются автоматически
