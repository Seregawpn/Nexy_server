# Nexy Server Development Rules (v2.0)
...
- [`../COMPLIANCE_REPORT.md`](../COMPLIANCE_REPORT.md) — **отчет о 100% соответствии всем правилам разработки (обновлен 5 ноября 2025)**
...
- `Docs/VERSION_FORMAT_CRITICAL_FIX.md` — канон форматов версий (строки) — единственный источник истины
...
- `Docs/decisions/` — директория с ADR документами (ADR-001: Модульная архитектура)
...
**Источник истины:** `Docs/VERSION_FORMAT_CRITICAL_FIX.md` — единственный источник истины

**Перед деплоем — обязательный чеклист:**
- [ ] Версии совпадают во всех компонентах (manifest, appcast, health)
- [ ] Размер файла совпадает с GitHub Content-Length
- [ ] Локальные DMG/артефакты отсутствуют на сервере
- [ ] Подписи (Ed25519) валидны
- [ ] Все поля версий — строковые (`"1.0.0"`)

**Детальная проверка:** см. `Docs/VERSION_FORMAT_CRITICAL_FIX.md` и `Docs/UPDATE_SYSTEM_FIXES.md` (таблица компонентов, проверки, скрипты)
...
В PR для rollout-изменений должно быть:
...
## 8. STATE_CATALOG и Interaction Matrix

- Любое изменение осей состояний или читателей/писателей фиксируется в `Docs/STATE_CATALOG.md` (сервер) и отражается в клиентском `client/config/interaction_matrix.yaml`.
...
|------------|--------------|--------------|
| 📋 **Compliance** | [`../COMPLIANCE_REPORT.md`](../COMPLIANCE_REPORT.md) | **Отчет о 100% соответствии всем правилам (обновлен 5 ноября 2025)** |
...
| **Область** | **Документ** | **Описание** |
|------------|--------------|--------------|
| 🔴 Версионирование | `Docs/VERSION_FORMAT_CRITICAL_FIX.md` | **Канон форматов версий** (строки) — источник истины |
...
## 14. Decision-логи и метрики (наблюдаемость решений)
...
## 17. Единые правила (зафиксированы окончательно)

1. **Форматы версий:** только строковые `"1.0.1"`, источник — `Docs/VERSION_FORMAT_CRITICAL_FIX.md`
2. **Размеры и подписи:** всегда сверяются с GitHub CDN (`Content-Length`, `SHA256`, `Ed25519`) — см. `Docs/UPDATE_SYSTEM_FIXES.md`
3. **Локальные файлы:** не допускаются — проверка в CI
4. **Health/AppCast/Manifest:** должны возвращать одинаковые версии и размеры
5. **CI-gate:** блокирует релиз при расхождении версий или размеров
6. **Rollback:** только через kill-switch и post-mortem лог-окно 5 мин
7. **ADR:** обязателен при любых изменениях FSM, guards или осей
8. **Документация:** любое изменение → обновить `Docs/ARCHITECTURE_OVERVIEW.md` и `Docs/STATE_CATALOG.md`
