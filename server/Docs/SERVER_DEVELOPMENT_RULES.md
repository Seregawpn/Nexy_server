...
- `server/Docs/ARCHITECTURE_OVERVIEW.md` — архитектура, FSM, таблицы исключений, backpressure, graceful shutdown
- `server/Docs/STATE_CATALOG.md` — оси состояний, метрики, владельцы (ответственный: Tech Lead клиента)
- `server/Docs/BACKPRESSURE_README.md` — политика лимитов, конфиг, troubleshooting
- `server/Docs/CI_GRPC_CHECKS.md` — CI-workflow, проверки, валидация размеров
- `server/Docs/RELEASE_AND_UPDATE_GUIDE.md` — канон системы обновлений (форматы версий, публикация, манифест, синхронизация размеров)
- `server/Docs/_archive/RAMP_PLAN.md` — историческая справка по раскатке (не канон)
- `server/Docs/SERVER_DEPLOYMENT_GUIDE.md` — процедура отката и kill-switch
- `server/Docs/ADR_TEMPLATE.md` — шаблон решений (ADR) с полями для осей/guards
...
- Единственный proto-файл — `server/modules/grpc_service/streaming.proto`. Изменения обновляют его и `server/Docs/GRPC_PROTOCOL_AUDIT.md`.
...
- [ ] `server/Docs/STATE_CATALOG.md` обновлён, если затронуты оси/читатели/писатели; приложена ссылка или дифф на внешний канон матрицы взаимодействий (клиентский `interaction_matrix.yaml`)
...
- [ ] CI-проверки (lint, contract, validate_release_size) выполняются по `server/Docs/CI_GRPC_CHECKS.md`
...
**Источник истины:** `server/Docs/RELEASE_AND_UPDATE_GUIDE.md` — раздел `Version Source of Truth` — единственный источник истины
...
**Детальная проверка:** см. `server/Docs/RELEASE_AND_UPDATE_GUIDE.md` и `server/Docs/DEPLOY_INCIDENT_RUNBOOK.md` (проверки, скрипты, troubleshooting).
...
**Источник истины:** `server/Docs/RELEASE_AND_UPDATE_GUIDE.md`
...
**CI-валидация:** см. `server/Docs/CI_GRPC_CHECKS.md` -> шаг `validate_release_size`
...
- Все race-condition кейсы из [server/Docs/STATE_CATALOG.md] и таблицы edge-кейсов должны покрываться тестами гейта.

---

## 6. Ответы ассистента и MCP команды

**Историческая справка:** `server/Docs/_archive/MCP_INTEGRATION_SUMMARY.md`.
...
**Документация:**
- `server/Docs/ARCHITECTURE_OVERVIEW.md` → раздел "Ответы ассистента и MCP команды"
- `server/Docs/_archive/MCP_INTEGRATION_SUMMARY.md` — итоговый summary (исторический).
- `server/Docs/_archive/MCP_COMMAND_INTEGRATION_PLAN.md` — исторический план (для справки).
...
**Документация:**
- `server/Docs/GRPC_PROTOCOL_AUDIT.md` — полный аудит протокола, таблицы полей, правила совместимости
...
**Лимиты стримов/таймаутов:** см. `server/Docs/BACKPRESSURE_README.md` (политика лимитов, конфиг, troubleshooting)
...
**ВАЖНО**: Все флаги (env и config) должны быть зарегистрированы в `server/Docs/FEATURE_FLAGS.md` ДО использования в коде.

**Правила**:
1. **Новый флаг запрещён без добавления в `server/Docs/FEATURE_FLAGS.md`** и прохождения `scripts/verify_feature_flags.py`
...
5. **Формат регистрации**: см. `server/Docs/FEATURE_FLAGS.md` (таблица с колонками: name, type, owner, default, scope, kill_switch, description)
6. **Исключения**: env переменные, которые не являются feature flags (например, `USE_GITHUB`, `USE_TLS`), должны быть добавлены в список исключений в `scripts/verify_feature_flags.py` и документированы в `server/Docs/FEATURE_FLAGS.md` (раздел "Исключения")

**Пример**:
```bash
# Перед использованием нового флага
# 1. Добавить в server/Docs/FEATURE_FLAGS.md
...
- Любое изменение осей состояний или читателей/писателей фиксируется в `server/Docs/STATE_CATALOG.md` (сервер) и отражается во внешнем каноне матрицы взаимодействий (клиентский `interaction_matrix.yaml`).
...
**Ответственный за актуальность:** Tech Lead клиента (указан в `server/Docs/STATE_CATALOG.md`)
...
- Статусы в `server/Docs/_archive/CURRENT_STATUS_REPORT.md`, `server/Docs/ARCHITECTURE_OVERVIEW.md`, релиз-заметки и отчёты используют только факты: ссылки на тест-планы, логи запусков, хэши билдов.
...
**Процедура отката и kill-switch:** см. `server/Docs/SERVER_DEPLOYMENT_GUIDE.md` → раздел "Rollback"
...
4. **Post mortem (5-минутное окно логов)**
   ...
   - Документировать причину отката в `server/Docs/_archive/CURRENT_STATUS_REPORT.md`
...
**Детальный runbook:** см. также `server/Docs/_archive/CANARY_CHECKLIST.md` → раздел "Rollback"
...
|------------|--------------|--------------|
| 🧱 Архитектура | `server/Docs/ARCHITECTURE_OVERVIEW.md` | Архитектура, FSM, таблицы исключений, backpressure, graceful shutdown |
| 🔄 Обновления | `server/Docs/_archive/GITHUB_UPDATE_SYSTEM.md` | Пайплайн деплоя, подписи, GitHub интеграция |
| 🧩 FSM/States | `server/Docs/STATE_CATALOG.md` | Оси состояний, метрики, владельцы |
| ⚙️ Роллаут | `server/Docs/_archive/RAMP_PLAN.md` | План раскатки трафика, гвардрайлы, этапы |
| 📦 Backpressure | `server/Docs/BACKPRESSURE_README.md` | Политика лимитов, конфиг, troubleshooting |
| 🔍 CI Checks | `server/Docs/CI_GRPC_CHECKS.md` | CI-workflow, проверки, валидация размеров |
| 📝 ADR | `server/Docs/ADR_TEMPLATE.md` | Шаблон решений (ADR) с полями для осей/guards |
| ✅ Canary | `server/Docs/_archive/CANARY_CHECKLIST.md` | Чеклист для canary выкатки |
| 🚀 Beta Gate | `server/Docs/_archive/BETA_GATE_CHECKLIST.md` | Чеклист для beta gate |
| 📊 Протокол gRPC | `server/Docs/GRPC_PROTOCOL_AUDIT.md` | Аудит протокола, контракт-таблицы |

### Критические фиксы

| **Область** | **Документ** | **Описание** |
|------------|--------------|--------------|
| 🔴 Версионирование | `server/Docs/RELEASE_AND_UPDATE_GUIDE.md` | **Канон системы обновлений** (включая форматы версий) — источник истины |
| 🔧 Update Fixes | `server/Docs/RELEASE_AND_UPDATE_GUIDE.md` | **Канон обновлений** — синхронизация размеров с GitHub CDN |
...
- Шаблон PR-чеклиста: `server/Docs/_archive/PR_CHECKLIST_TEMPLATE.md`
...
**FSM-таблицы:** размещаются в `server/modules/<domain>/fsm.yaml` (или в коде модуля) и документируются в `server/Docs/ARCHITECTURE_OVERVIEW.md` → раздел "FSM Map".
...
**План раскатки:** см. `server/Docs/_archive/RAMP_PLAN.md` (этапы, гвардрайлы, метрики)
...
**Шаблон:** `server/Docs/ADR_TEMPLATE.md` (с полями для осей/guards)

**Требования:**
- ADR должен содержать раздел "Affected Axes / Guards"
- Ссылка на `server/Docs/STATE_CATALOG.md` и внешний канон матрицы взаимодействий (клиентский `interaction_matrix.yaml`)
- Обновления в `server/Docs/ARCHITECTURE_OVERVIEW.md` и `server/Docs/STATE_CATALOG.md`

---

## 18. Единые правила (зафиксированы окончательно)

1. **Форматы версий:** только строковые `"1.0.1"`, источник — `server/Docs/RELEASE_AND_UPDATE_GUIDE.md` (раздел `Version Source of Truth`)
2. **Размеры и подписи:** всегда сверяются с GitHub CDN (`Content-Length`, `SHA256`, `Ed25519`) — см. `server/Docs/RELEASE_AND_UPDATE_GUIDE.md`
...
9. **Документация:** любое изменение → обновить `server/Docs/ARCHITECTURE_OVERVIEW.md` и `server/Docs/STATE_CATALOG.md`
