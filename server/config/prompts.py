...
PROMPT_MESSAGES = (
    ...
    "Slot reuse policy for the active Messages clarification chain is strict.\n"
    "- If recipient is already known from active goal or recent dialogue, MUST treat contact as already known and MUST NOT ask for contact again.\n"
    "- If message body is already known from active goal or recent dialogue, MUST treat message body as already known and MUST NOT ask for it again.\n"
    "- Do NOT reset the active messaging chain unless the current turn explicitly starts a different task.\n\n"
    ...
    "- If the active Messages clarification chain already knows the recipient and the current turn supplies the message body, execute send_message now.\n"
    "- If the current turn explicitly names the recipient but does not provide the message body, ask only for message text.\n"
    "- If the current turn provides the message body but the recipient is already known from the active chain, do not ask for the recipient again.\n"
    "- If the current turn starts or replaces the task with an explicitly named recipient, that recipient is already resolved for the new task and asking who the message is for is invalid.\n"
    "- For active follow-up turns like 'tell her ...', 'reply that ...', or 'say I am outside', resolve the pronoun to the known recipient from the active chain; pronoun text must never become the literal contact payload.\n"
...
PROMPT_WHATSAPP = (
    ...
    "Slot reuse policy for the active WhatsApp clarification chain is strict.\n"
    "- If recipient is already known from active goal or recent dialogue, MUST treat contact as already known and MUST NOT ask for contact again.\n"
    "- If message body is already known from active goal or recent dialogue, MUST treat message body as already known and MUST NOT ask for it again.\n"
    "- Do NOT reset the active WhatsApp chain unless the current turn explicitly starts a different task.\n\n"
    ...
    "- Reuse recipient/content from previous turns only when the active goal or recent dialogue clearly establishes the same unfinished WhatsApp task.\n"
    "- If there is no active WhatsApp clarification chain, do not invent recipient or message content from unrelated older turns.\n"
    "- If the active WhatsApp clarification chain already knows the recipient and the current turn provides the message body, you MUST return Action JSON with command send_whatsapp_message now.\n"
    "- In that situation, plain text only like 'Sending a WhatsApp message to Mom...' is invalid because the action payload is required.\n"
    "- If the active Current goal or recent dialogue clearly says the unfinished task is WhatsApp, neutral follow-ups like 'Tell Mom I arrived' MUST stay on WhatsApp and MUST NOT downgrade to send_message.\n"
    "- If the active WhatsApp chain already knows the recipient and the current turn provides the message body, command send_message is invalid; you must use send_whatsapp_message.\n"
    "- If route is whatsapp or the active goal clearly points to WhatsApp, a plain natural-language promise like 'Sending a WhatsApp message to Mom saying you arrived.' is invalid unless it is accompanied by Action JSON with command send_whatsapp_message.\n"
    "- For the exact follow-up shape 'Tell Mom ...' after an active WhatsApp clarification chain, you must emit Action JSON now; do not answer with narration only.\n"
    "- If the current turn starts or replaces the task with an explicitly named recipient, that recipient is already resolved for the new WhatsApp task and asking who the message is for is invalid.\n"
    "- Ask one short clarifying question only if required execution fields are missing in current request (contact and/or message).\n\n"
...
CATEGORY_GENERAL_DECISION_POLICY = (
    ...
    "- Decide by intended user outcome, not by isolated keywords.\n"
...
CATEGORY_CONFLICT_RESOLUTION_POLICY = (
    ...
    "- browser vs system_control: websites/browser tabs/player tasks => browser; opening or closing desktop apps => system_control.\n"
    "- browser vs google_search: interactive website operation => browser; informational/factual answer => google_search.\n"
...
CATEGORY_RULES_MESSAGES = (
    ...
    "- Choose when user wants Apple Messages/iMessage/SMS communication: send, read, reply, check conversation status.\n"
    "- Choose messages even when recipient or message text is still missing; missing slots belong to clarification, not to none.\n"
    "- Choose for generic wording like send a message, send a text, tell her, reply to him when WhatsApp is not explicit.\n"
    "- Continue when active goal or short-term memory already points to a Messages thread.\n"
...
CATEGORY_RULES_WHATSAPP = (
    ...
    "- Choose only when WhatsApp is explicit in current turn or active goal.\n"
    "- Continue when active goal already points to WhatsApp and user gives reply/follow-up content.\n"
...
CATEGORY_RULES_BROWSER = (
    ...
    "- Choose for interactive website tasks: open a site, navigate, click, log in, fill forms, play content on a site.\n"
...
CATEGORY_RULES_GOOGLE_SEARCH = (
    ...
    "- Choose for factual/informational requests: latest news, prices, comparisons, current events, weather, who is, summarize.\n"
...
CATEGORY_RULES_SYSTEM_CONTROL = (
    ...
    "- Choose for OS-level app lifecycle tasks: open app, close app, quit app.\n"
    "- Choose open Safari instead / open Telegram instead as system_control, not browser, because the goal is desktop app control.\n"
...
GENERATOR_EXECUTION_POLICY = (
    ...
    "- If the active WhatsApp chain already knows the recipient and the current turn provides the message body, execute send_whatsapp_message now instead of asking again.\n"
    "- If the active Messages chain already knows the recipient and the current turn provides the message body, execute send_message now instead of asking again.\n"
    "- If the active clarification chain already knows the recipient and current turn uses a pronoun like her, him, them, or this person, resolve that pronoun to the known recipient in args.contact.\n"
    ...
    "- Treat imperative follow-up phrasing like 'tell Mom I arrived', 'tell her I will be late', 'say I am outside', or 'reply that I got home' as the message body when the active chain already knows the recipient.\n"
    "- If the active WhatsApp chain already knows the recipient and the current turn is imperative content for that recipient, send the WhatsApp message now.\n"
    "- If the active Messages chain already knows the recipient and the current turn is imperative content for that recipient, send the message now.\n"
    ...
    "- Example: active goal is 'User wants to send a WhatsApp message to Mom. Missing detail: message text.' and current turn is 'Tell Mom I arrived' -> command send_whatsapp_message with contact Mom and message 'I arrived'.\n"
    "- Example: active goal is 'User wants to send a message to Sophia. Missing detail: message text.' and current turn is 'Tell her I will be late' -> command send_message with contact Sophia and message 'I will be late'.\n"
    ...
    "- Example: current turn is 'No, send a message to Sophia instead' after a different active task -> ask only 'What would you like to say to Sophia?' and never 'Who should I send the message to?'.\n"
...
PROMPT_RUNTIME_MEMORY_USAGE_POLICY = (
    ...
    "- For active messaging/WhatsApp clarification chains, MUST keep the known recipient and MUST NOT ask who to send it to again.\n"
    "- For active messaging/WhatsApp clarification chains, MUST keep the known message body and MUST NOT ask for it again.\n"
    "- Do not drop a known slot from short-term memory when the current turn clearly continues the same messaging chain.\n"
...
PROMPT_INTENT_CLASSIFIER = (
    ...
    "Classify by user GOAL (what the user wants to achieve), not by isolated words.\n"
    ...
    "Noisy-input examples (category only):\n"
    ...
    "Use this only when the destination app is WhatsApp. "
    "If WhatsApp is explicit, do not choose messages. "
    "Examples: 'send to Sophia in WhatsApp how are you doing' => whatsapp; "
    "'read my WhatsApp messages' => whatsapp. "
    "Do NOT use for iMessage/SMS-only intent.\n"
    ...
    "Use this when the destination app is Messages/iMessage/SMS and WhatsApp is not intended. "
    "This is also the default messaging category for generic 'message/text/send it/tell her/reply' requests when WhatsApp is not explicit. "
    ...
    "Do NOT use when WhatsApp is explicit.\n"
    ...
    "Use browser when user expects website interaction steps, not just factual answer text. "
    "Examples: 'open YouTube and play lo-fi' => browser; "
    "'go to amazon and add this item to cart' => browser. "
    "Do NOT use for pure factual lookup requests.\n"
    ...
    "Use this when user wants an answer, not interactive browsing steps. "
    ...
    "Do NOT use if user explicitly asks to operate a website UI.\n"
    ...
    "Use for desktop application control, not website tasks. "
    "Examples: 'open Telegram' => system_control; "
    "'close Spotify' => system_control. "
    "Do NOT use for opening websites.\n"
    ...
    "- none: User goal cannot be mapped reliably to a category after disambiguation and context checks.\n\n"
...
def build_intent_classifier_prompt() -> str:
    ...
    return PROMPT_INTENT_CLASSIFIER
...
def build_system_prompt(
    ...
    section_registry = [
        ("header", PROMPT_HEADER, True),
        ("system_control", PROMPT_SYSTEM_CONTROL, system_control_enabled),
        ("messages", PROMPT_MESSAGES, messages_enabled),
        ("whatsapp", PROMPT_WHATSAPP, whatsapp_enabled),
        ("describe", PROMPT_DESCRIBE, describe_enabled),
        ("web_search", PROMPT_WEB_SEARCH, effective_google_search_enabled),
        ("browser", PROMPT_BROWSER, browser_enabled),
        ("payment", PROMPT_PAYMENT, payment_enabled),
        ("capability", PROMPT_CAPABILITY, capability_enabled),
    ]
    section_registry.extend(
        [
            ("noisy_intent_policy", PROMPT_NOISY_INTENT_POLICY, True),
        ]
    )
    section_registry.append(("unified_policy", PROMPT_UNIFIED_POLICY, True))
    ...
    if strict_single_route_mode:
        parts.append(PROMPT_FOOTER_MINIMAL.replace("allowed_commands", allowed_commands_text))
    else:
