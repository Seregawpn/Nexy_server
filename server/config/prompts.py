...
PROMPT_MESSAGES = (
    ...
    "- Inside the current unfinished messaging clarification chain, reuse a field already stated explicitly in recent short-term memory and ask only for the still-missing field.\n"
    "- If recent short-term memory shows user already gave the contact (for example: 'send a message to Sophia') and current turn gives only the message text, you MUST treat contact as already known and MUST NOT ask for contact again.\n"
    "- If recent short-term memory shows user already gave the message text and current turn resolves only the contact, you MUST keep the message text and ask only for the still-missing field, if any.\n"
...
PROMPT_WHATSAPP = (
    ...
    "- Inside the current unfinished WhatsApp clarification chain, reuse a field already stated explicitly in recent short-term memory and ask only for the still-missing field.\n"
    "- If recent short-term memory shows user already gave the contact and current turn gives only the message text, you MUST treat contact as already known and MUST NOT ask for contact again.\n"
    "- If recent short-term memory shows user already gave the message text and current turn resolves only the contact, you MUST keep the message text and ask only for the still-missing field, if any.\n"
...
PROMPT_NOISY_INTENT_POLICY = (
    "[Noisy Input Intent Normalization]\n\n"
...
PROMPT_RUNTIME_MEMORY_USAGE_POLICY = (
    ...
    "- Build a complete user-context snapshot before answering: recent turns, stable profile facts, and relevant prior outcomes.\n"
    "- Maintain continuity as a trusted assistant: answer as if you fully understand ongoing user context, but never invent missing facts.\n"
    "- Do not skip medium/long inspection even when short-term seems sufficient; use them to validate context consistency.\n"
    "- Before composing any final answer, read the full short-term timeline top-to-bottom.\n"
    ...
    "- Before any final response decision, analyze the full available memory context (short + medium + long) for relevance.\n"
    ...
    "- For active messaging/WhatsApp clarification chains, treat explicitly stated slots in short-term memory as authoritative until user changes the task.\n"
    "- If short-term memory already contains recipient/contact for the active messaging chain and the current turn supplies only message body, you MUST keep the known recipient and MUST NOT ask who to send it to again.\n"
    "- If short-term memory already contains message body for the active messaging chain and the current turn supplies only recipient/contact, you MUST keep the known message body and MUST NOT ask for it again.\n"
    "- Do not drop a known slot from short-term memory just because the current turn does not repeat it verbatim.\n"
    ...
    "- Use medium/long-term only when request explicitly depends on history/profile.\n"
    "- If required memory fact is missing/ambiguous, say unavailable and do not invent.\n"
...
PROMPT_INTENT_CLASSIFIER = (
    ...
    "Use short-term context when current input is ambiguous, short, or referential.\n\n"
    ...
    "- For short/referential follow-ups, prioritize CURRENT_TURN first; use PREVIOUS_TURN_n only when CURRENT_TURN is insufficient.\n"
    "- Never ignore previous turns when current input is short or referential.\n\n"
    "Memory-first decision policy for classification:\n"
    "- After short-term timeline read, use full memory context (short + medium + long) only to validate relevance and consistency.\n"
    "- Determine if current request is memory-related (recall, profile facts, prior agreement) or not.\n"
    "- If request is NOT memory-related, classify only by current user goal; memory must not override explicit current intent.\n"
    ...
    "- If a messaging action is present but channel is unclear, use short_term_memory to continue the active channel/thread; if still unresolved, choose none.\n"
    "- For referential messaging turns after a messaging prompt (e.g., 'send her ...' after 'What message would you like to send to Sophia?'), use short_term_memory to resolve the active messaging category.\n"
    "- Never rely on downstream route rewrites or validators: your category decision is final.\n"
    "- Do not choose whatsapp from generic messaging verbs alone; whatsapp requires explicit current-turn marker or explicit active WhatsApp thread in short_term_memory.\n"
    "- If the active clarification thread is Messages/iMessage/SMS and the current turn has no explicit WhatsApp marker, choose messages, not whatsapp.\n"
    ...
    "Use this only when the destination app is Messages/iMessage/SMS and WhatsApp is not intended. "
    ...
    "- For short or referential inputs (e.g., 'continue', 'tell me more', 'go on', 'what else', 'продолжи', 'дальше'), "
    ...
    "- Recover what the user is trying to achieve from the recent dialogue and classify the current goal accordingly.\n"
...
def build_system_prompt(
    ...
    Args:
        primary_route: Route selected by classifier for strict prompt assembly
    ...
    route_text = str(primary_route or "").strip().lower()
    if route_text:
        system_control_enabled = system_control_enabled and route_text == "system_control"
        messages_enabled = messages_enabled and route_text == "messages"
        whatsapp_enabled = whatsapp_enabled and route_text == "whatsapp"
        describe_enabled = describe_enabled and route_text == "describe"
        effective_google_search_enabled = bool(effective_google_search_enabled and route_text == "google_search")
        browser_enabled = browser_enabled and route_text == "browser"
        payment_enabled = payment_enabled and route_text == "payment"
        capability_enabled = capability_enabled and route_text == "capability"
    ...
    if not strict_single_route_mode:
