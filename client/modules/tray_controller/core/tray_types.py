...
class TrayIconGenerator:
    ...
    def create_circle_icon(status: TrayStatus, size: int = 16) -> TrayIcon:
        ...
        logger.debug(
            f"🎯 TRAY DEBUG: TrayIconGenerator.create_circle_icon status={status} (type: {type(status)})"
        )
        ...
        return TrayIcon(
            status=status, icon_type=icon_types[status], color=colors[status], size=size
        )
