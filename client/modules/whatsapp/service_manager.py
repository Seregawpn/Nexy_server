...
class WhatsappServiceManager:
    ...
    def _resolve_script_path(self) -> str:
        ...
        default_path = (
            self._root_dir
            / "Whatsapp/whatsapp-mcp-ready/node_modules/@iflow-mcp/whatsapp-mcp-ts/build/index.js"
        )
    ...
    def _resolve_node_executable(self) -> str:
        ...
        if which_node:
            logger.info(
                f"Configured node path '{node_path}' not found, falling back to '{which_node}'"
            )
            return which_node

        # 3. Try common locations
        common_paths = ["/opt/homebrew/bin/node", "/usr/local/bin/node", "/usr/bin/node"]
        for path in common_paths:
            if os.path.exists(path):
                logger.info(
                    f"Configured node path '{node_path}' not found, falling back to found '{path}'"
                )
                return path
    ...
    async def start(
        ...
        auth_callback: Callable[[], None] | None = None,
            ...
            try:
                ...
                try:
                    ...
                    if "attached to a different loop" in str(e):
                        logger.warning(
                            f"⚠️ Loop mismatch during process wait/stop: {e}. continuing..."
                        )
                    else:
                        raise e
    ...
    def clear_auth_cache(self):
        ...
        try:
            ...
            auth_info_dir = (
                Path.home() / "Library" / "Application Support" / "Nexy" / "whatsapp_auth"
            )
    ...
    def handle_output_line(self, decoded_line: str):
        ...
        if (
            "Auth failure" in decoded_line
            or "Authentication failure" in decoded_line
        ...
        try:
            ...
            if (
                auth_success
                or "connected to WA" in msg
                or "Application setup complete" in msg
                or "Connection opened" in msg
            ):
