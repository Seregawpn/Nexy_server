...
class WelcomePlayer:
    ...
    async def play_welcome(self) -> WelcomeResult:
        ...
        try:
            ...
            logger.info(
                f"🔍 [WELCOME_PLAYER] config.enabled={self.config.enabled}, config.use_server={self.config.use_server}"
            )
            ...
            if not self.config.enabled:
                ...
                result = WelcomeResult(
                    success=False, method="none", duration_sec=0.0, error=error_msg
                )
            ...
            logger.info(
                f"TRACE [WELCOME_PLAYER] _play_server_audio() returned: success={server_result.success}"
            )
            logger.info(
                f"🔍 [WELCOME_PLAYER] Серверное аудио получено: success={server_result.success}, error={server_result.error}"
            )
            ...
            logger.warning(
                f"⚠️ [WELCOME_PLAYER] Серверное воспроизведение не удалось: {server_result.error}"
            )
            ...
            if fallback_result.success:
                logger.info(
                    "✅ [WELCOME_PLAYER] Локальное приветствие (fallback) воспроизведено успешно"
                )
                self.state = WelcomeState.COMPLETED
                if self._on_completed:
            ...
            error_msg = (
                ...
                or "Воспроизведение приветствия (сервер + fallback) не удалось"
            )
            logger.error(f"❌ [WELCOME_PLAYER] {error_msg}")
            self.state = WelcomeState.ERROR

            result = WelcomeResult(success=False, method="none", duration_sec=0.0, error=error_msg)
            ...
            result = WelcomeResult(success=False, method="error", duration_sec=0.0, error=error_msg)
    ...
    async def _play_server_audio(self) -> WelcomeResult:
        ...
        try:
            ...
            logger.info(
                f"🔍 [WELCOME_PLAYER] audio_data.shape={audio_data.shape}, dtype={audio_data.dtype}"
            )
            ...
            logger.info(
                f"🔍 [WELCOME_PLAYER] sample_rate={sample_rate}, channels={channels}, duration={duration_sec:.2f}s"
            )
            ...
            return WelcomeResult(
                success=True, method="server", duration_sec=duration_sec, metadata=metadata
            )
