    ...
    def load_config(self) -> WelcomeConfig:
        ...
        try:
            ...
            config = WelcomeConfig(
                ...
                ignore_microphone_permission=welcome_config.get(
                    "ignore_microphone_permission", False
                ),
                force_permission_checks=welcome_config.get("force_permission_checks", False),
            )

            logger.info(
                f"✅ [WELCOME_CONFIG] Конфигурация загружена: enabled={config.enabled}, text='{config.text[:30]}...', sample_rate={config.sample_rate}Hz (из server_audio_format)"
            )
            return config
