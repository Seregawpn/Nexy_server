        ...
        for row in cursor.fetchall():
            ...
            if not contact_data.get("display_name"):
                try:
                    ...
                    if contact_info.get("display_label") and contact_info.get("display_label") != contact_data["id"]:
                        contact_data["display_name"] = contact_info["display_label"]
                        logger.info(f"Имя получено из {contact_info.get('source')}: {contact_info.get('display_label')}")
                except Exception as e:
...
def get_last_message(conn: sqlite3.Connection) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not text and attributed_body:
            ...
            if extracted_text:
                text = extracted_text
                logger.debug(f"Extracted text from attributedBody in get_last_message: {text[:50]}...")
    ...
    try:
        ...
        if matches0:
            ...
            for phrase in matches0:
                ...
                if (len(phrase_clean) > 4 and
                    not phrase_clean.lower().startswith('ns') and
                    'streamtyped' not in phrase_clean.lower() and
                    'streamtype' not in phrase_clean.lower() and
                    'attribute' not in phrase_clean.lower() and
                    'dictionary' not in phrase_clean.lower()):
        ...
        if matches0_5:
            for match in matches0_5:
                if (len(match) >= 3 and
                    'streamtyped' not in match.lower() and
                    not match.startswith('NS')):
                    logger.info(f"Extracted text via method 0.5 (NSKeyedArchiver format): {match[:100]}...")
                    return match
        ...
        if matches0_6:
            for match in matches0_6:
                if (len(match) >= 2 and
                    'streamtyped' not in match.lower() and
                    not match.startswith('NS')):
                    logger.info(f"Extracted text via method 0.6 (flexible NSKeyedArchiver): {match[:100]}...")
                    return match
        ...
        if matches:
            ...
            for match in matches:
                ...
                if (len(cleaned) >= 2 and 
                    ...
                    not cleaned.startswith('kIM') and
                    not cleaned.startswith('__') and
                    'streamtyped' not in cleaned.lower() and
                    'streamtype' not in cleaned.lower() and
                    'Attribute' not in cleaned and
                    'Dictionary' not in cleaned and
                    'Number' not in cleaned and
                    'Value' not in cleaned and
                    # Проверяем, что это не только технические символы
                    len(re.findall(r'[a-zA-Zа-яА-ЯёЁ]', cleaned)) >= 1):  # Минимум 1 буква
        ...
        if matches:
            ...
            for phrase in matches:
                phrase_clean = phrase.strip()
                if (len(phrase_clean) > 5 and
                    not phrase_clean.startswith('NS') and
                    not phrase_clean.startswith('kIM') and
                    'Attribute' not in phrase_clean and
                    'Dictionary' not in phrase_clean):
                    valid_phrases.append(phrase_clean)
        ...
        if matches:
            valid_texts = []
            technical_terms = ['streamtyped', 'streamtype', 'NSKeyedArchiver', 'NSArchiver', 'NSAttributedString']
            for match in matches:
                # Фильтруем служебные строки и технические термины
                match_lower = match.lower()
                if (len(match.strip()) > 10 and
                    not match.strip().startswith('NS') and
                    not any(term in match_lower for term in technical_terms) and
                    not 'Attribute' in match and
                    not 'Dictionary' in match and
        ...
        technical_words = [
            'NSString', 'NSObject', 'NSDictionary', 'NSNumber', 
            'NSValue', 'NSAttributedString', 'kIMMessagePartAttributeName',
            'streamtyped', 'streamtype', 'stream', 'typed', 'type',
            'NSKeyedArchiver', 'NSArchiver', 'NSData', 'NSMutable',
            'kIM', 'IM', 'Message', 'Attribute', 'Dictionary', 'Number',
            'Value', 'Object', 'String', 'Data', 'Archiver'
        ]
        filtered_words = [w for w in words if w not in technical_words and not w.lower().startswith('ns')]
        ...
        technical_words_lower = [w.lower() for w in [
            'NSString', 'NSObject', 'NSDictionary', 'NSNumber', 
            'NSValue', 'NSAttributedString', 'kIMMessagePartAttributeName',
            'streamtyped', 'streamtype', 'stream', 'typed', 'type',
            'NSKeyedArchiver', 'NSArchiver', 'NSData', 'NSMutable',
        ...
        filtered_words = [w for w in words if w.lower() not in technical_words_lower and not w.lower().startswith('ns')]
        ...
        logger.warning(f"Could not extract text from attributedBody using any method. Decoded preview: {decoded[:100]}")
        return None
    ...
    if group_by_date:
        ...
        for date_str in sorted_dates:
            ...
            for time_str, msg in messages_by_date[date_str]:
                ...
                sender = "You" if is_from_me else (msg.get("display_name") or msg.get("contact_id", "Contact"))
                ...
                if not text or text.strip() == "":
                    ...
                    if attributed_body:
                        ...
                        if extracted_text and extracted_text.strip():
                            ...
                            technical_terms = ['streamtyped', 'streamtype', 'nsstring', 'nsobject', 'nsdictionary']
                            extracted_lower = extracted_text.lower()
                            if not any(term in extracted_lower for term in technical_terms):
                                text = extracted_text
                                logger.info(f"[MCP Messages] Extracted text from attributedBody in format_messages: {text[:100]}...")
                            else:
                                logger.warning(f"[MCP Messages] Extracted text contains technical terms, skipping: {extracted_text[:100]}...")
                                text = None
        ...
        for msg in reversed(messages):
            ...
            if not text or text.strip() == "":
                ...
                if attributed_body:
                    ...
                    if extracted_text and extracted_text.strip():
                        text = extracted_text
                        logger.debug(f"Extracted text from attributedBody in format_messages (simple): {text[:50]}...")
