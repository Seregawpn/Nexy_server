...
class ModeController:
    ...
    def register_transition(self, transition: ModeTransition):
        ...
        logger.debug(
            f"📝 Зарегистрирован переход: {transition.from_mode.value} → {transition.to_mode.value}"
        )
        ...
        async with self._lock:
            try:
                ...
                if not force and not self.can_switch_to(new_mode):
                    logger.warning(
                        f"⚠️ Нельзя переключиться из {self.current_mode.value} в {new_mode.value}"
                    )
                    return False
                ...
                if transition and transition.action:
                    logger.info(
                        f"🔄 Выполнение перехода: {transition.from_mode.value} → {transition.to_mode.value}"
                    )
