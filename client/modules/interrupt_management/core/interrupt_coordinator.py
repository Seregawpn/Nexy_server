...
class InterruptCoordinator:
    ...
    async def trigger_interrupt(self, event: InterruptEvent) -> bool:
        ...
        if len(self.active_interrupts) >= self.config.max_concurrent_interrupts:
            logger.warning(
                f"⚠️ Достигнут лимит активных прерываний: {self.config.max_concurrent_interrupts}"
            )
            return False
        ...
        async with self._lock:
            ...
            logger.info(
                f"🔄 Запуск прерывания {event.type.value} (приоритет: {event.priority.value})"
            )
            ...
            if handler:
                ...
                if self.metrics.total_interrupts > 0:
                    self.metrics.average_processing_time = (
                        self.metrics.average_processing_time * (self.metrics.total_interrupts - 1)
                        + processing_time
