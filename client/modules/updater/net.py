...
class UpdateHTTPClient:
    ...
    def __init__(self, timeout: int = 30, retries: int = 3, ssl_verify: bool = True):
        ...
        logger.info(
            f"HTTP клиент инициализирован: timeout={timeout}s, retries={retries}, ssl_verify={ssl_verify}"
        )
    ...
    def get_manifest(self, url: str) -> dict[str, Any]:
        ...
        if (
            ...
            raise ValueError(
                "URL должен использовать HTTPS для безопасности (кроме localhost для тестирования)"
            )
    ...
    def _parse_xml_manifest(self, xml_content: str) -> dict[str, Any]:
        ...
        try:
            ...
            manifest = {
                ...
                "file_size": file_size,
                "minimum_system_version": enclosure.get(
                    ...
                    for chunk in response.stream(1024 * 1024):  # 1MB chunks
                        ...
                        if on_progress:
                            try:
                                ...
                                logger.debug(
                                    f"download_file progress callback error: {callback_error}"
                                )
