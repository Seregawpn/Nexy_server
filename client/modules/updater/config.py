...
class UpdaterConfig:
    ...
    def __post_init__(self):
        ...
        if (
            self.manifest_url
            and not self.manifest_url.startswith("https://")
            and not self.manifest_url.startswith("http://localhost")
            and not self.manifest_url.startswith("http://127.0.0.1")
        ):
            raise ValueError(
                "manifest_url должен использовать HTTPS (кроме localhost для тестирования)"
            )
