    ...
    def _is_lock_valid(self) -> bool:
        ...
        try:
            ...
            if self.pid_check and "pid" in lock_info:
                ...
                if pid == current_pid:
                    print(
                        f"⚠️ DEBUG: Lock file PID ({pid}) совпадает с текущим PID ({current_pid}) - это не дублирование"
                    )
                ...
                try:
                    ...
                    print(
                        f"🔍 DEBUG: Checking lock PID {pid}: name={process_name}, cmdline={cmdline[:100]}"
                    )
                    ...
                    is_debug_script = (
                        process_name in ["python3", "Python"]
                        and "debug_lock_validation.py" in cmdline
                    )
                    is_test_script = (
                        process_name in ["python3", "Python"]
                        and "test_duplicate_detection.py" in cmdline
                    )

                    print(
                        f"🔍 DEBUG: Process checks: is_nexy_app={is_nexy_app}, is_python_main={is_python_main}"
                    )
