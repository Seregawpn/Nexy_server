...
class NetworkManager:
    ...
    async def _update_metrics(self):
        ...
        new_metrics = NetworkMetrics(ping_time=ping_time, last_updated=time.time())
    ...
    async def _emit_status_change(
        self, old_status: NetworkStatus, new_status: NetworkStatus, details: dict[str, Any]
    ):
    ...
    async def _emit_quality_change(self, new_quality: NetworkQuality):
        ...
        event = NetworkEvent(
            ...
            details={
                "quality": new_quality.value if hasattr(new_quality, "value") else str(new_quality)
            },
            timestamp=time.time(),
        )
        ...
        logger.debug(
            f"Network quality changed: {new_quality.value if hasattr(new_quality, 'value') else str(new_quality)}"
        )
