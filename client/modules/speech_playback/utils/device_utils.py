...
def normalize_audio(audio_data: np.ndarray, target_dtype: Any = np.int16) -> np.ndarray:
    ...
    logger.debug(
        f"🔍 [AUDIO_DEBUG] Нормализация аудио: shape={audio_data.shape}, dtype={audio_data.dtype} -> {target_dtype}"
    )
    try:
...
def resample_audio(audio_data: np.ndarray, original_rate: int, target_rate: int) -> np.ndarray:
    ...
    """
    logger.debug(
        f"🔍 [AUDIO_DEBUG] Изменение частоты дискретизации: {original_rate}Hz -> {target_rate}Hz"
    )
    try:
        ...
        if audio_data.dtype not in [np.float32, np.float64]:
            logger.warning(
                f"⚠️ [RESAMPLE] Неожиданный dtype: {audio_data.dtype}, приводим к float32"
            )
        ...
        logger.debug(
            f"🔍 [AUDIO_DEBUG] Частота дискретизации изменена: {len(audio_data)} -> {len(result)} сэмплов"
        )
        return result
