...
class AudioRouteMonitor:
    ...
    def start(self) -> None:
        ...
        self._poll_thread = threading.Thread(
            target=self._poll_loop, name="device_poll", daemon=True
        )
    ...
    def _check_for_changes(self, reason: str) -> None:
        ...
        with self._lock:
            ...
            changed = old_snapshot is None or old_snapshot.input_uid != new_snapshot.input_uid

            if changed:
                old_name = old_snapshot.input_name if old_snapshot else "(none)"
                logger.info(
                    "🔄 Device changed (%s): %s → %s", reason, old_name, new_snapshot.input_name
                )
