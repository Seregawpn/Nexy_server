...
class GrpcClient:
    ...
    def _create_default_config(self) -> dict[str, Any]:
        ...
        try:
            ...
            for server_name, server_config in servers_config.items():
                ...
                logger.info(
                    f"🔌 [DEBUG] Loaded server '{server_name}': {server_dict['address']}:{server_dict['port']}, ssl={server_dict['use_ssl']}, ssl_verify={server_dict['ssl_verify']}"
                )
    ...
    def _initialize_servers(self):
        ...
        try:
            ...
            for name, server_config in servers_config.items():
                ...
                logger.info(
                    f"🔌 [DEBUG] _initialize_servers creating ServerConfig for '{name}': ssl_verify={ssl_verify_from_config}"
                )
                ...
                logger.info(
                    f"🔌 [DEBUG] Created ServerConfig for '{name}': ssl_verify={config.ssl_verify}"
                )
    ...
    def _get_server_audio_format(self) -> dict[str, Any]:
        ...
        if UnifiedConfigLoader is not None:
            try:
                ...
                logger.warning(
                    f"⚠️ Не удалось загрузить server_audio_format из UnifiedConfigLoader: {e}"
                )
        ...
        try:
            ...
            return {"sample_rate": 48000, "channels": 1, "dtype": "int16"}
    ...
    def _set_default_server(self):
        ...
        try:
            ...
            logger.info(
                f"🔌 [DEBUG] Available servers: {list(self.connection_manager.servers.keys())}"
            )
        ...
        *,
        ...
        try:
            ...
            if hasattr(screen_info, "get"):
                ...
                logger.warning(
                    f"⚠️ Неизвестный тип screen_info: {type(screen_info)}, используем значения по умолчанию"
                )
                screen_width = None
                screen_height = None
    ...
    async def _generate_welcome_audio_in_loop(
        ...
        *,
            ...
            async for response in stub.GenerateWelcomeAudio(request, timeout=rpc_timeout):
                ...
                if content == "audio_chunk":
                    ...
                    metadata = {
                        ...
                        "dtype": getattr(
                            response.metadata, "dtype", None
                        ),  # Проверяем dtype из metadata
                    }
        ...
        final_sample_rate = (
            int(metadata_sr)
            if metadata_sr is not None
            else int(server_audio_format.get("sample_rate", 48000))
        )
        final_channels = (
            int(metadata_ch)
            if metadata_ch is not None
            else int(server_audio_format.get("channels", 1))
        )
        ...
        if final_channels > 1:
            try:
                ...
                logger.warning(
                    "⚠️ Не удалось изменить форму аудио по каналам, оставляю одномерный массив"
                )
        ...
        calculated_duration = (
            actual_samples / float(final_sample_rate) if final_sample_rate > 0 else 0.0
        )
