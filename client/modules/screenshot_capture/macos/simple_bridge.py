...
class SimpleCoreGraphicsBridge:
    ...
    def _capture_via_coregraphics(self, config: ScreenshotConfig, region: Optional[Tuple[int, int, int, int]]) -> ScreenshotResult:
        ...
        try:
            ...
            if config.format == ScreenshotFormat.WEBP:
                return self._encode_cgimage_to_webp(cg_image, config, start_time, {
                    "bridge_type": "simple_coregraphics",
                    "capture_method": "coregraphics_direct"
                })
            else:
                return self._encode_cgimage_to_jpeg(cg_image, config, start_time, {
                    "bridge_type": "simple_coregraphics",
                    "capture_method": "coregraphics_direct"
                })
    ...
    def _convert_to_webp_bytes(self, image_path: Path, config: ScreenshotConfig) -> bytes:
        ...
        try:
            ...
            logger.debug(f"✅ Конвертировано в WebP: {image_path.name}, {len(webp_bytes)} bytes, quality={webp_quality}")
            return webp_bytes
