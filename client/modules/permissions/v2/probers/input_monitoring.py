...
class InputMonitoringProber(BaseProber):
    ...
    async def probe(self, probe_kind: Literal["light", "heavy"]) -> ProbeResult:
        ...
        if (
            self._consecutive_create_fail >= 3
            and ev.permission_denied_hint is not True
            and tap_created is False
        ):
            ev = ProbeEvidence(
                tap_created=tap_created,
                tap_enabled=tap_enabled,
    ...
    async def _capability_event_tap(
        self,
    ) -> tuple[bool | None, bool | None, str | None, str | None, str | None]:
        """
