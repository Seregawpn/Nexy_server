...
**Источник истины:** [`Docs/FLOW_INTERACTION_SPEC.md`](../../Docs/FLOW_INTERACTION_SPEC.md) — разделы 3 (Контракты событий) и 4.4 (PTT/Listening Flow).

#### Подписывается на:
- События режима LISTENING (см. раздел 4.4 канона)
- События прерываний (см. раздел 4.6 канона)

#### Публикует:
- `mode.request` — запросы смены режима (см. раздел 3.1 канона)

Полные контракты payload и последовательность событий описаны в каноническом документе.
...
**Источник истины:** [`Docs/FLOW_INTERACTION_SPEC.md`](../../Docs/FLOW_INTERACTION_SPEC.md) — разделы 3 (Контракты событий) и 4.5 (Processing Flow).

#### Подписывается на:
- События цепочки PROCESSING (см. раздел 4.5 канона)
- События прерываний (см. раздел 4.6 канона)

#### Публикует:
- `mode.request` — возврат в SLEEPING (см. раздел 3.1 канона)
- События отмены при прерывании (см. раздел 4.6 канона)

Полные контракты payload и последовательность этапов обработки описаны в каноническом документе.
