...
class BaseWorkflow(ABC):
    ...
    def _task_done_callback(self, task: asyncio.Task[Any]):
        ...
        if task.cancelled():
            ...
            logger.error(
                f"❌ {self.workflow_name}: ошибка в задаче {task.get_name()} - {task.exception()}"
            )

    async def _wait_for_event(
        self, event_type: str, timeout: float = 30.0, session_filter: bool = True
    ) -> dict[str, Any] | None:
        """
        ...
        async def event_handler(event):
            ...
            try:
                ...
                logger.error(
                    f"❌ {self.workflow_name}: ошибка обработки события {event_type} - {e}"
                )
    ...
    async def _publish_mode_request(
        self, target_mode: AppMode, source: str, session_id: str | None = None, priority: int = 50
    ):
