...
class ListeningWorkflow(BaseWorkflow):
    ...
    async def _setup_subscriptions(self):
        ...
        # Мониторинг режимов
        await self.event_bus.subscribe(
            "app.mode_changed", self._on_mode_changed, EventPriority.MEDIUM
        )

        # Активность голоса (если доступно)
        await self.event_bus.subscribe(
            "voice.activity_detected", self._on_voice_activity, EventPriority.LOW
        )
    ...
    async def _on_recording_start(self, event):
        ...
        try:
            ...
            self.debounce_task = self._create_task(
                self._debounce_check(session_id), "debounce_check"
            )
            ...
            self.timeout_task = self._create_task(
                self._timeout_monitor(session_id), "timeout_monitor"
            )
        ...
        try:
            ...
            if self.listening_start_time:
                ...
                if duration < self.debounce_threshold:
                    logger.warning(
                        f"🎤 ListeningWorkflow: запись слишком короткая ({duration:.2f}с), возможно случайное нажатие"
                    )
    ...
    async def _timeout_monitor(self, session_id: str):
        ...
        try:
            ...
            logger.warning(
                f"🎤 ListeningWorkflow: достигнут максимальный таймаут ({self.max_listening_duration}с)"
            )
            ...
            logger.debug(
                f"🎤 ListeningWorkflow: мониторинг таймаута отменен для сессии {session_id}"
            )
        except Exception as e:
        ...
        base_status.update(
            {
                ...
                "last_voice_activity": self.last_voice_activity.isoformat()
                if self.last_voice_activity
                else None,
                "debounce_active": self.debounce_task is not None and not self.debounce_task.done(),
                "timeout_monitoring": self.timeout_task is not None
