...
class ErrorHandler:
    ...
    async def handle_error(
        self, severity: Any, category: Any, message: str, context: dict[str, Any] | None = None
    ):
        """Обработать ошибку"""
        try:
            if context is None:
    ...
    def get_error_history(
        ...
        try:
            ...
            if severity:
                filtered_history = [
                    error for error in filtered_history if error["severity"] == severity
                ]

            if category:
                filtered_history = [
                    error for error in filtered_history if error["category"] == category
                ]
