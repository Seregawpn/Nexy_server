...
def decide_permission_restart_schedule(
    ...
    *,
    ...
    """
    try:
        engine = get_engine("decide_permission_restart_schedule")
        ctx = create_ctx_from_snapshot(snapshot)
        return engine.decide(snapshot, source="permission_restart_gateway", ctx=ctx, extra=extra)
    except Exception as exc:
        logger.warning(
            f"decision_engine_fallback gateway=decide_permission_restart_schedule "
            f"ctx={create_ctx_from_snapshot(snapshot).to_log_string()} "
            f"error={exc} fallback_to=legacy"
        )
        _log_decision(
...
def decide_permission_restart_wait(
    ...
    *,
    ...
    """
    try:
        engine = get_engine("decide_permission_restart_wait")
        ctx = create_ctx_from_snapshot(snapshot)
        return engine.decide(snapshot, source="permission_restart_gateway", ctx=ctx, extra=extra)
    except Exception as exc:
        logger.warning(
            f"decision_engine_fallback gateway=decide_permission_restart_wait "
            f"ctx={create_ctx_from_snapshot(snapshot).to_log_string()} "
            f"error={exc} fallback_to=legacy"
        )
        _log_decision(
