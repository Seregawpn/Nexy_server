...
class SimpleModuleCoordinator:
    ...
    def __init__(self):
        ...
        self._tal_refresh_task: asyncio.Task[Any] | None = None  # Задача периодического обновления
        self._idle_metrics_task: asyncio.Task[Any] | None = (
            None  # Задача периодического сбора idle метрик
        )
    ...
    def _ensure_state_manager(self) -> ApplicationStateManager:
        ...
        if self.state_manager is None:
            raise RuntimeError(
                "ApplicationStateManager не инициализирован. Вызовите initialize() сначала."
            )
    ...
    def _ensure_bg_loop(self) -> asyncio.AbstractEventLoop:
        ...
        if self._bg_loop is None:
            raise RuntimeError(
                "Фоновый event loop не инициализирован. Вызовите initialize() сначала."
            )
    ...
    def _force_focus_activation_for_tray_once(self, reason: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                "[FOCUS] nsapp_activator does not support force_activate kwarg, using direct fallback"
            )
    ...
    async def _create_integrations(self):
        ...
        try:
            ...
            print(
                f"✅ Интеграции созданы через IntegrationFactory: {len(self.integrations)} integrations, {len(self.workflows)} workflows"
            )
        except Exception as e:
    ...
    async def _setup_critical_subscriptions(self):
        ...
        try:
            ...
            await self._ensure_event_bus().subscribe(
                "permissions.first_run_started", self._on_permissions_started, EventPriority.HIGH
            )
            await self._ensure_event_bus().subscribe(
            ...
            await self._ensure_event_bus().subscribe(
                "permissions.changed", self._on_permissions_changed, EventPriority.HIGH
            )
    ...
    async def _setup_coordination(self):
        ...
        try:
            ...
            await self._ensure_event_bus().subscribe(
                EventTypes.APP_STARTUP, self._on_app_startup, EventPriority.HIGH
            )
            await self._ensure_event_bus().subscribe(
                EventTypes.APP_SHUTDOWN, self._on_app_shutdown, EventPriority.HIGH
            )
            await self._ensure_event_bus().subscribe(
                EventTypes.APP_MODE_CHANGED, self._on_mode_changed, EventPriority.MEDIUM
            )

            # Подписываемся на события пользовательского завершения
            await self._ensure_event_bus().subscribe(
                EventTypes.TRAY_QUIT_CLICKED, self._on_user_quit, EventPriority.HIGH
            )

            # Подписываемся на готовность tray (gate-механизм)
            await self._ensure_event_bus().subscribe(
                EventTypes.TRAY_INTEGRATION_READY, self._on_tray_ready, EventPriority.CRITICAL
            )
            ...
            try:
                ...
                await self._ensure_event_bus().subscribe(
                    EventTypes.SCREENSHOT_ERROR, self._on_screenshot_error, EventPriority.MEDIUM
                )
            except Exception as e:
            ...
            for name in startup_order:
                if name in self.integrations:
                    ...
                    if name in [
                        ...
                        state_manager = self._ensure_state_manager()
                        first_run_in_progress = state_manager.get_state_data(
                            StateKeys.FIRST_RUN_IN_PROGRESS, False
                        )
                        restart_pending = state_manager.get_state_data(
                            StateKeys.PERMISSIONS_RESTART_PENDING, False
                        )
                    ...
                    if (
                        name in ["first_run_permissions", "permission_restart"]
                        and not self._tray_ready
                    ):
                        focus_cfg = self._get_focus_config()
                        max_wait_sec = max(
                            1.0, float(focus_cfg.get("tray_fallback_timeout_sec", 6.0))
                        )
                        wait_start = time.time()
                        logger.info(
                            f"⏳ [TRAY_GATE] Waiting for tray before starting {name} (max {max_wait_sec}s)..."
                        )
                        print(
                            f"⏳ [TRAY_GATE] Ожидание tray перед запуском {name} (максимум {max_wait_sec}s)..."
                        )
                        ...
                        if self._tray_ready:
                            logger.info(
                                f"✅ [TRAY_GATE] Tray ready after {waited_ms}ms wait - proceeding with {name}"
                            )
                            print(
                                f"✅ [TRAY_GATE] Tray готов после {waited_ms}ms ожидания - продолжаем с {name}"
                            )
                        else:
                            allow_fallback = bool(
                                focus_cfg.get("allow_tray_startup_fallback", True)
                            )
                            if allow_fallback:
                                forced = self._force_focus_activation_for_tray_once(
                                    reason=f"tray_not_ready_before_{name}"
                                )
                                if forced:
                                    ...
                                    while (
                                        not self._tray_ready
                                        and (time.time() - retry_start) < retry_window_sec
                                    ):
                                        await asyncio.sleep(0.1)
                            if self._tray_ready:
                                logger.info(
                                ...
                                logger.warning(
                                    f"⚠️ [TRAY_GATE] Tray not ready after {waited_ms}ms - proceeding anyway with {name}"
                                )
                                print(
                                    f"⚠️ [TRAY_GATE] Tray не готов после {waited_ms}ms - продолжаем с {name}"
                                )
                    ...
                    if name == "first_run_permissions" and success:
                        logger.info(
                            "✅ [PERMISSIONS] Permissions check completed, continuing startup..."
                        )
                        print(
                            "✅ [PERMISSIONS] Проверка разрешений завершена, продолжаем запуск..."
                        )
                    ...
                    if name == "first_run_permissions" and not success:
                        ...
                        if restart_pending:
                            logger.warning(
                                "⚠️ [PERMISSIONS] Restart required - starting permission_restart before stopping..."
                            )
                            print(
                                "⚠️ [PERMISSIONS] Требуется рестарт - запускаем permission_restart..."
                            )

                            # Запускаем permission_restart чтобы он мог обработать restart_pending
                            if "permission_restart" in self.integrations:
                                try:
                                    pr_success = await self.integrations[
                                        "permission_restart"
                                    ].start()
                                    if pr_success:
                                        logger.info(
                                            "✅ [PERMISSIONS] permission_restart started successfully"
                                        )
                                        print("✅ [PERMISSIONS] permission_restart запущен успешно")
                                    else:
                                        logger.error(
                                            "❌ [PERMISSIONS] permission_restart failed to start"
                                        )
                                        print(
                                            "❌ [PERMISSIONS] permission_restart не удалось запустить"
                                        )
                                except Exception as e:
                                    logger.error(
                                        f"❌ [PERMISSIONS] Error starting permission_restart: {e}"
                                    )
                                    print(
                                        f"❌ [PERMISSIONS] Ошибка запуска permission_restart: {e}"
                                    )
                        else:
                            logger.warning(
                                "⛔ [PERMISSIONS] Missing permissions - blocking startup until granted"
                            )
                        ...
                        logger.info(
                            "🛑 [PERMISSIONS] Stopping further module loading until permissions are granted"
                        )
    ...
    async def stop(self) -> bool:
        ...
        try:
            ...
            if self._tal_hold_active:
                ...
                reason = (
                    f"shutdown_before_tray_ready"
                    if not self._tray_ready
                    else "shutdown_after_tray_ready"
                )
                logger.warning(
                    f"⚠️ [SHUTDOWN] TAL hold активен при остановке (tray={tray_status}) - явно снимаем"
                )
    ...
    async def run(self):
        ...
        try:
            ...
            logger.info(
                "⏳ CRITICAL: Ожидание готовности ControlCenter (tray имеет собственную retry-логику)"
            )
            ...
            if tray_controller:
                ...
                logger.error(
                    "❌ Не удалось получить tray_controller - используем fallback app.run()"
                )
            ...
            if self._tal_hold_active:
                ...
                logger.warning(
                    f"⚠️ [FATAL] TAL hold активен в finally блоке (tray={tray_status}) - явно снимаем"
                )
                try:
                    self._release_tal_hold(reason=reason)
            ...
            if self._duplicate_instance_detected:
                ...
                raise SystemExit(
                    1
                )  # Пробрасываем исключение для корректного завершения через main.py
    ...
    async def _on_app_startup(self, event):
        ...
        try:
            ...
            logger.info(
                "🔒 [COORDINATOR] Waiting for V2 Orchestrator to publish system.ready_to_greet"
            )
    ...
    async def _on_mode_changed(self, event):
        ...
        try:
            ...
            printable_mode = (
                getattr(new_mode, "value", None) or str(new_mode)
    ...
    async def _on_screenshot_captured(self, event):
        ...
        try:
            ...
            logger.info(
                f"Screenshot captured: path={path}, size={size_bytes}, dims={width}x{height}, session={session_id}"
            )
        except Exception as e:
    ...
    async def _on_permissions_completed(self, event):
        ...
        try:
            ...
            if all_granted:
                ...
                logger.warning(
                    f"⚠️ [PERMISSIONS] Some permissions missing: {missing} (session={session_id})"
                )
    ...
    async def _on_permissions_changed(self, event):
        ...
        try:
            ...
            if permission == "microphone" and new_status == "granted":
                logger.info(
                    f"🎤 [COORDINATOR] Mic granted, waiting for other permissions (session={session_id})"
                )
            ...
            if is_timeout and new_status == "denied":
                ...
                settings_required_permissions = permission_config.get(
    ...
    async def _on_tray_ready(self, event):
        ...
        try:
            ...
            if self._tray_start_time:
                ...
                logger.info(
                    f"✅ [TRAY_GATE] Tray ready in {duration_ms}ms - releasing gate for blocking operations"
                )
            else:
    ...
    def _hold_tal_until_tray_ready(self):
        ...
        try:
            ...
            print(
                f"🛡️ [ANTI_TAL] _hold_tal_until_tray_ready() ВХОД (tal_hold_active={self._tal_hold_active})"
            )
            logger.info(
                f"🛡️ [ANTI_TAL] _hold_tal_until_tray_ready() ВХОД (tal_hold_active={self._tal_hold_active})"
            )
            ...
            if self._tal_hold_active:
                logger.debug(
                    f"TAL=hold (ts={time.time():.2f}, reason=duplicate_call, already_active=True)"
                )
                ...
                if self._tal_refresh_task is None or (
                    hasattr(self._tal_refresh_task, "done") and self._tal_refresh_task.done()
                ):
                    if self._bg_loop and self._bg_loop.is_running():
                        print(
                            f"🛡️ [ANTI_TAL] Фоновый loop доступен - запускаем периодическое обновление (duplicate call)"
                        )
                        logger.info(
                            f"🛡️ [ANTI_TAL] Фоновый loop доступен - запускаем периодическое обновление (duplicate call)"
                        )
                        ...
                        def schedule_refresh():
                            try:
                                ...
                                self._tal_refresh_task = self._ensure_bg_loop().create_task(
                                    self._periodically_refresh_tal_hold()
                                )
                                print(
                                    f"🛡️ [ANTI_TAL] Задача _periodically_refresh_tal_hold() создана в фоновом loop (duplicate call)"
                                )
                                logger.info(
                                    f"🛡️ [ANTI_TAL] Задача _periodically_refresh_tal_hold() создана в фоновом loop (duplicate call)"
                                )
                                ...
                                logger.error(
                                    f"❌ [ANTI_TAL] Ошибка создания задачи в фоновом loop (duplicate call): {task_err}"
                                )
                        ...
                        try:
                            ...
                            print(
                                f"🛡️ [ANTI_TAL] Event loop активен: {loop} - запускаем периодическое обновление"
                            )
                            logger.info(
                                f"🛡️ [ANTI_TAL] Event loop активен: {loop} - запускаем периодическое обновление"
                            )

                            self._tal_refresh_task = asyncio.create_task(
                                self._periodically_refresh_tal_hold()
                            )
                            print(
                                f"🛡️ [ANTI_TAL] Задача _periodically_refresh_tal_hold() создана (duplicate call): {self._tal_refresh_task}"
                            )
                            logger.info(
                                f"🛡️ [ANTI_TAL] Задача _periodically_refresh_tal_hold() создана (duplicate call)"
                            )
                        except RuntimeError as loop_err:
                            print(
                                f"❌ [ANTI_TAL] КРИТИЧНО: Event loop не активен при duplicate call! {loop_err}"
                            )
                            logger.error(
                                f"❌ [ANTI_TAL] КРИТИЧНО: Event loop не активен при duplicate call! {loop_err}"
                            )
            ...
            logger.info(
                f"TAL=hold (ts={self._tal_hold_start:.2f}, auto_term_enabled={auto_term_enabled})"
            )
            print(
                f"🛡️ [ANTI_TAL] TAL удержание установлено (auto_term_enabled={auto_term_enabled}) - будет снято после tray.ready или через 120s"
            )
            ...
            if self._bg_loop and self._bg_loop.is_running():
                logger.info(
                    f"🛡️ [ANTI_TAL] Используем фоновый event loop для периодического обновления: {self._bg_loop}"
                )
                ...
                def schedule_tasks():
                    try:
                        ...
                        logger.info(
                            f"🛡️ [ANTI_TAL] Задача _periodically_refresh_tal_hold() создана в фоновом loop"
                        )

                        # Планируем автоматическое снятие по таймауту (120s - увеличено)
                        timeout_task = self._ensure_bg_loop().create_task(
                            self._release_tal_hold_after_timeout()
                        )
                        logger.info(
                            f"🛡️ [ANTI_TAL] Задача _release_tal_hold_after_timeout() создана в фоновом loop"
                        )
                    except Exception as task_err:
                        logger.error(
                            f"❌ [ANTI_TAL] Ошибка создания задач в фоновом loop: {task_err}"
                        )
                        import traceback
                ...
                try:
                    ...
                    logger.info(
                        f"🛡️ [ANTI_TAL] Задача _periodically_refresh_tal_hold() создана: {self._tal_refresh_task}"
                    )

                    # Планируем автоматическое снятие по таймауту (120s - увеличено)
                    timeout_task = asyncio.create_task(self._release_tal_hold_after_timeout())
                    logger.info(
                        f"🛡️ [ANTI_TAL] Задача _release_tal_hold_after_timeout() создана: {timeout_task}"
                    )
                except RuntimeError as loop_err:
        ...
        try:
            ...
            if not self._tal_hold_active:
                if reason == "duplicate_call":
                    logger.debug(
                        f"TAL=released (ts={time.time():.2f}, reason={reason}, had_active_hold=False)"
                    )
                else:
                    logger.debug(
                        f"TAL=released (ts={time.time():.2f}, reason={reason}, had_active_hold=False, duplicate_release=True)"
                    )
                return

            if not hasattr(self, "_tal_hold_start") or self._tal_hold_start is None:
                logger.debug(
                    f"TAL=released (ts={time.time():.2f}, reason={reason}, had_active_hold=False, no_start_time)"
                )
        ...
        try:
            ...
            while (time.time() - start_time) < max_wait:
                ...
                try:
                    ...
                    )  # Конвертируем в миллисекунды для метрики
                    ...
                    logger.debug(
                        f"🔄 [ANTI_TAL] TAL assertion обновлён (tray ещё не готов, elapsed={elapsed:.1f}s)"
                    )
    ...
    def _start_idle_metrics_collection(self):
        ...
        try:
            ...
            if self._idle_metrics_task is not None and not self._idle_metrics_task.done():
                logger.debug(
                    "📊 [METRICS] Сбор idle метрик уже запущен, пропускаем повторный запуск"
                )
                return
            ...
            if self._bg_loop and self._bg_loop.is_running():
                ...
                def schedule_task():
                    try:
                        asyncio.set_event_loop(self._bg_loop)
                        self._idle_metrics_task = self._ensure_bg_loop().create_task(
                            self._collect_idle_metrics_periodically()
                        )
                        logger.debug("📊 [METRICS] Задача сбора idle метрик создана в фоновом loop")
                    except Exception as task_err:
                        logger.warning(
                            f"⚠️ [METRICS] Ошибка создания задачи сбора idle метрик: {task_err}"
                        )
                ...
                try:
                    loop = asyncio.get_running_loop()
                    self._idle_metrics_task = asyncio.create_task(
                        self._collect_idle_metrics_periodically()
                    )
                    logger.debug("📊 [METRICS] Задача сбора idle метрик создана в текущем loop")
                except RuntimeError:
                    logger.warning(
                        "⚠️ [METRICS] Event loop не активен, idle метрики не будут собираться"
                    )
        except Exception as exc:
    ...
    async def _on_permissions_restart_pending(self, event):
        ...
        try:
            ...
            logger.error(
                f"❌ [PERMISSIONS] Error handling permissions.first_run_restart_pending: {e}"
            )
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "integrations": {
                name: integration.get_status() for name, integration in self.integrations.items()
            },
        }
