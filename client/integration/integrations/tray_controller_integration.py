...
class TrayControllerIntegration:
    ...
    def __init__(
        ...
        if config is None:
            ...
            enable_notifications = tray_cfg.get(
                "enable_notifications", tray_basic.get("show_notifications", True)
            )
            debug_mode = tray_cfg.get(
                "debug_mode", (config_data.get("app") or {}).get("debug", False)
            )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(
                "🔧 [TRAY_METRICS] tray.will_init - начало инициализации TrayControllerIntegration"
            )
            logger.info(
                "🔧 [TRAY_METRICS] tray.will_init - начало инициализации TrayControllerIntegration"
            )
            ...
            for attempt in range(1, max_retries + 1):
                ...
                if success:
                    logger.info(
                        f"✅ [TRAY_METRICS] Initialize succeeded (attempt {attempt}/{max_retries}, {attempt_duration_ms}ms)"
                    )
                    break
            ...
            if not success:
                logger.error(
                    f"❌ [TRAY_METRICS] Initialization failed after retries ({init_duration_ms}ms total) – auto-disabling tray"
                )
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            for attempt in range(1, max_retries + 1):
                attempt_start = time.time()
                success = await (
                    self.tray_controller.start() if self.tray_controller else asyncio.sleep(0)
                )
                attempt_duration_ms = int((time.time() - attempt_start) * 1000)
                if success:
                    logger.info(
                        f"✅ [TRAY_METRICS] Start succeeded (attempt {attempt}/{max_retries}, {attempt_duration_ms}ms)"
                    )
                    break
            ...
            if not success:
                logger.error(
                    f"❌ [TRAY_METRICS] Start failed after retries ({start_duration_ms}ms total) – auto-disabling tray"
                )
                self._disabled_due_to_errors = True
                self.tray_controller = None
                return True
            ...
            logger.info(
                f"✅ [TRAY_METRICS] tray.ready - TrayControllerIntegration запущен за {start_duration_ms}ms"
            )
            logger.info(
                f"✅ [TRAY_METRICS] tray.ready - TrayControllerIntegration запущен за {start_duration_ms}ms"
            )
    ...
    async def _setup_event_handlers(self):
        ...
        try:
            ...
            await self.event_bus.subscribe(
                "voice.mic_opened", self._on_voice_mic_opened, EventPriority.HIGH
            )
            await self.event_bus.subscribe(
            ...
            await self.event_bus.subscribe(
                "system.notification", self._on_system_notification, EventPriority.MEDIUM
            )
    ...
    async def _sync_with_app_mode(self):
        ...
        try:
            ...
            if current_mode in self.mode_to_status:
                ...
                logger.info(
                    f"🔄 Синхронизация с режимом приложения: {current_mode.value} → {target_status.value}"
                )
    ...
    async def _on_mode_changed(self, event):
        ...
        try:
            ...
            if new_mode in self.mode_to_status:
                ...
                logger.debug(
                    f"TrayIntegration: mapping mode -> status: {new_mode} -> {target_status}"
                )
    ...
    async def _on_permissions_blocked(self, event):
        """Обработка начала проверки разрешений без отдельного tray-режима."""
        try:
            logger.info("🔐 [TRAY] First-run started: сохраняем только канонические 3 режима")
            await self._sync_with_app_mode()
        except Exception as e:
            logger.error(f"❌ Ошибка при обработке старта проверки разрешений: {e}")
    ...
    def _apply_status_ui_sync(self, status: TrayStatus):
        ...
        try:
            ...
            if not icon_path:
                logger.error(
                    f"❌ КРИТИЧНО: Не удалось создать иконку для status={status}. "
    ...
    def _ui_tick(self, _timer):
        ...
        try:
            ...
            try:
                logger.debug(
                    f"UI tick: current={getattr(current, 'value', 'None')}, desired={getattr(desired, 'value', 'None')}"
                )
            except Exception:
                pass
            ...
            logger.debug(
                f"UI tick: обновление иконки {getattr(current, 'value', 'None')} -> {desired.value}"
            )
            ...
            try:
                ...
                prev_value = (
                    getattr(prev_status, "value", str(prev_status)) if prev_status else "None"
                )
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "tray_controller": {
                ...
                "current_status": self.tray_controller.current_status.value
                if self.tray_controller
                else None,
            },
            "config": {
