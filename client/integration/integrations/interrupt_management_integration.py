...
class InterruptManagementIntegration:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            dependencies = InterruptDependencies(state_manager=self.state_manager)
            ...
            await self.event_bus.subscribe(
                "app.startup", self._on_app_startup, EventPriority.MEDIUM
            )
            await self.event_bus.subscribe(
                "app.shutdown", self._on_app_shutdown, EventPriority.HIGH
            )
            await self.event_bus.subscribe(
                "app.state_changed", self._on_app_state_changed, EventPriority.HIGH
            )
            ...
            await self.event_bus.subscribe(
                "interrupt.cancel", self._on_interrupt_cancel, EventPriority.HIGH
            )
        ...
        try:
            ...
            if self.config.enable_speech_interrupts:
                self._coordinator.register_handler(
                    InterruptType.SPEECH_STOP, self._handle_speech_stop
                )
                self._coordinator.register_handler(
                    InterruptType.SPEECH_PAUSE, self._handle_speech_pause
                )

            if self.config.enable_recording_interrupts:
                self._coordinator.register_handler(
                    InterruptType.RECORDING_STOP, self._handle_recording_stop
                )

            if self.config.enable_session_interrupts:
                self._coordinator.register_handler(
                    InterruptType.SESSION_CLEAR, self._handle_session_clear
                )

            if self.config.enable_full_reset:
                self._coordinator.register_handler(
                    InterruptType.FULL_RESET, self._handle_full_reset
                )
    ...
    async def _on_interrupt_request(self, event):
        ...
        try:
            ...
            logger.debug(
                f"🛑 InterruptManager: interrupt.request - type={interrupt_type}, data.session_id={data.get('session_id')}, event.session_id={event.get('session_id')}, final.session_id={session_id}"
            )
            ...
            if interrupt_type == "speech_stop":
                ...
                if (now - last_ts) < self._interrupt_dedup_window_sec:
                    ...
                    logger.info(
                        f"🛑 InterruptManager: останавливаем речь (session_id={session_id})"
                    )
        ...
        try:
            ...
            if self.event_bus:
                try:
                    await self.event_bus.publish(
                        "mode.request",
                        {"target": AppMode.PROCESSING, "source": "interrupt_management"},
                    )
                except Exception as e:
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "running": self._running,
            "interrupts": {
                "active_count": len(coordinator.active_interrupts)
                if hasattr(coordinator, "active_interrupts")
                else 0,
                "total_count": len(coordinator.interrupt_history)
                if hasattr(coordinator, "interrupt_history")
                else 0,
                "is_running": coordinator.is_interrupting()
                if hasattr(coordinator, "is_interrupting")
                else False,
            },
        }
