...
class NetworkManagerIntegration:
    ...
    def __init__(
        ...
        if config is None:
            ...
            config = NetworkManagerConfig(
                check_interval=net_cfg.get(
                    "check_interval", (config_data.get("network") or {}).get("keepalive_time", 10)
                ),
                ping_timeout=net_cfg.get(
                    "ping_timeout", (config_data.get("network") or {}).get("keepalive_timeout", 3)
                ),
    ...
    async def stop(self) -> bool:
        ...
        try:
            ...
            if hasattr(self.error_handler, "handle"):
                await self.error_handler.handle(
                    error=e, category="network", severity="error", context={"where": "network.stop"}
                )
            else:
    ...
    async def _on_network_event(self, event: NetworkEvent):
        ...
        try:
            ...
            if event.event_type == "network.status_changed":
                ...
                await self.event_bus.publish(
                    "network.quality_changed",
                    {"quality": event.details.get("quality"), "timestamp": event.timestamp},
                )
    ...
    async def _update_tray_tooltip(self, status: dict[str, Any]):
        ...
        try:
            ...
            await self.event_bus.publish("tray.update_tooltip", {"tooltip": tooltip})
