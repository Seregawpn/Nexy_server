...
class InstanceManagerIntegration:
    ...
    def __init__(
        ...
        if env_lock_file:
            ...
            logger.info(
                "[INSTANCE_MANAGER] Using lock file from env NEXY_INSTANCE_LOCK_FILE=%s",
                env_lock_file,
            )

        lock_file = env_lock_file or self.config.get(
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            await self.event_bus.subscribe(
                "instance.check_request", self._on_instance_check_request
            )
        ...
        try:
            ...
            if status == InstanceStatus.DUPLICATE:
                ...
                try:
                    ...
                    logger.warning(
                        "🚫 InstanceManager: duplicate instance detected — returning False for clean shutdown"
                    )
                except Exception:
                    pass
                ...
                try:
                    await self.event_bus.publish(
                        "signal.duplicate_instance",
                        {"message": "Nexy уже запущен", "sound": "error"},
                    )
            ...
            if not lock_acquired:
                ...
                await self.error_handler.handle(
                    Exception("Failed to acquire lock"), category="runtime", severity="error"
                )
                return False
            ...
            try:
                ...
                await self.event_bus.publish(
                    "instance.status_checked",
                    {"status": InstanceStatus.SINGLE.value, "lock_info": lock_info},
                )
            except Exception as e:
    ...
    async def _on_instance_check_request(self, event: dict[str, Any]) -> None:
        ...
        try:
            ...
            await self.event_bus.publish(
                ...
                {"status": status.value, "timestamp": asyncio.get_event_loop().time()},
            )
        except Exception as e:
            await self.error_handler.handle(e, category="runtime", severity="error")
