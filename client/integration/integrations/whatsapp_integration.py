...
class WhatsappIntegration(BaseIntegration):
    ...
    def __init__(
        ...
        self._qr_monitor_task: asyncio.Task[Any] | None = None
    ...
    async def _ensure_service_running(self):
        ...
        if self.service_manager.process:
            logger.info(
                "WhatsApp service already running (PID=%s)", self.service_manager.process.pid
            )
            return
    ...
    async def _handle_auth_success(self):
        ...
        await self.event_bus.publish(
            "system.notification",
            {"title": "WhatsApp Connected", "message": "WhatsApp authentication successful."},
        )
    ...
    async def _on_request(self, event: dict[str, Any]):
        ...
        logger.info(
            "[%s] _on_request: command=%s, session=%s, args=%s",
            FEATURE_ID,
            command,
            session_id,
            args,
        ...
        decision = decide_whatsapp_action(snapshot)
        if (
            snapshot.whatsapp_status != WhatsappStatus.CONNECTED
            and decision == Decision.NOTIFY_USER
        ):
            logger.info(
            ...
            async def display_qr_async():
                ...
                try:
                    if snapshot.whatsapp_status in (
            ...
            await self.event_bus.publish(
                "grpc.tts_request",
                {"session_id": session_id, "text": speak_text, "source": "whatsapp"},
            )
            return

        # Acknowledge start
        await self.event_bus.publish(
            f"actions.{command}.started", {"session_id": session_id, "feature_id": feature_id}
        )
        ...
        try:
            ...
            if command == "send_whatsapp_message":
                result = await self.mcp_client.send_whatsapp_message(
                    contact=args.get("contact"), message=args.get("message")
                )
            elif command == "read_whatsapp_messages":
                result = await self.mcp_client.read_whatsapp_messages(contact=args.get("contact"))
            else:
            ...
            if result.startswith("Error") or result.startswith("Failed"):
                ...
                await self.event_bus.publish(
                    f"actions.{command}.completed",
                    {"session_id": session_id, "feature_id": feature_id, "message": result},
                )

                if command == "send_whatsapp_message":
                    await self.event_bus.publish(
                        "grpc.tts_request",
                        {"session_id": session_id, "text": "Message sent.", "source": "whatsapp"},
                    )
            ...
            speak_text = (
                f"I found multiple contacts for that name: {choices_str}. Please say the full name."
            )
            ...
            logger.warning(
                "☠️ Auth Error caught during command. Forcing service restart to generate fresh QR."
            )
