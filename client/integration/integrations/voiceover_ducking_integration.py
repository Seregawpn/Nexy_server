...
class VoiceOverDuckingIntegration(BaseIntegration):
    ...
    async def _do_initialize(self) -> bool:
        ...
        try:
            ...
            if snapshot.first_run:
                ...
                logger.info(
                    "ℹ️ VoiceOverDuckingIntegration: first-run not completed, postponing init until permissions.first_run_completed"
                )
                # Всё равно подписываемся на permissions_ready для последующего старта
                await self.event_bus.subscribe(
                    "system.permissions_ready", self._on_permissions_ready
                )
            ...
            if await self._maybe_initialize_controller():
                ...
                logger.info(
                    "ℹ️ VoiceOverDuckingIntegration: awaiting Accessibility permission before init"
                )
    ...
    async def handle_mode_change(self, event: dict[str, Any]) -> None:
        ...
        try:
            ...
            await self.error_handler.handle(
                exc, category="runtime", severity="warning", context={"where": "handle_mode_change"}
            )
    ...
    async def handle_keyboard_press(self, event: dict[str, Any]) -> None:
        ...
        try:
            ...
            await self.error_handler.handle(
                ...
                category="runtime",
                severity="warning",
                context={"where": "handle_keyboard_press"},
            )
    ...
    async def handle_shutdown(self, event: dict[str, Any]) -> None:
        ...
        try:
            ...
            await self.error_handler.handle(
                exc, category="runtime", severity="warning", context={"where": "handle_shutdown"}
            )
    ...
    async def manual_duck(self, reason: str = "manual") -> bool:
        ...
        try:
            ...
            await self.error_handler.handle(
                exc, category="runtime", severity="warning", context={"where": "manual_duck"}
            )
            return False
    ...
    async def manual_release(self, force: bool = False) -> bool:
        ...
        try:
            ...
            await self.error_handler.handle(
                exc, category="runtime", severity="warning", context={"where": "manual_release"}
            )
            return False
    ...
    async def _on_permissions_ready(self, event: dict[str, Any]) -> None:
        ...
        if await self._maybe_initialize_controller():
            self._awaiting_permissions = False
            logger.info(
                "✅ VoiceOverDuckingIntegration: controller initialized after permissions_ready"
            )
