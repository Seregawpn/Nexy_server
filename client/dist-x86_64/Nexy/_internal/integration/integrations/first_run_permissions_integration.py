...
2. Если какое-то не GRANTED → активирует и ждёт получения с таймаутом:
   - Microphone: показывает диалог, ждёт без таймаута (пользователь может принять решение)
   - Accessibility/Input Monitoring/Screen Capture: после 10 секунд ожидания автоматически
     открывает System Settings и показывает help dialog
   - Hard-limit: после открытия Settings ждёт max_wait_after_settings_open_sec (60s по умолчанию)
   - По истечении таймаута продолжает запуск БЕЗ блокировки (разрешение может быть выдано позже)
...
ВАЖНО: Изменение контракта (v2):
- Раньше: БЛОКИРОВАЛ запуск остальных интеграций пока ВСЕ разрешения не будут получены
- Теперь: После таймаута продолжает запуск, даже если разрешения не выданы
- Риск: Если разрешение будет выдано ПОСЛЕ таймаута (пользователь выдал вручную в Settings),
  может потребоваться перезапуск приложения для активации (особенно для Accessibility/Input)

БЛОКИРУЕТ запуск остальных интеграций только до таймаута или выдачи всех разрешений!
"""
...
from modules.permissions.first_run.status_checker import (
    ...
    mark_screen_capture_prompt_shown,
    get_bundle_id,
)
...
class FirstRunPermissionsIntegration:
    ...
    def __init__(
        ...
        try:
            ...
            # Загружаем параметры first_run из permissions.first_run
            first_run_config = config_data.get("permissions", {}).get("first_run", {})
            self.max_wait_sec = first_run_config.get("max_wait_sec", 30.0)
            self.open_settings_after_sec = first_run_config.get("open_settings_after_sec", 10.0)
            
            logger.info(
                "[FIRST_RUN_PERMISSIONS] Configuration loaded: "
                "enabled=%s, max_wait_sec=%.1f, open_settings_after_sec=%.1f, required_permissions=%s",
                self.enabled,
                self.max_wait_sec,
                self.open_settings_after_sec,
                self.required_permissions,
            )
        except Exception as e:
            logger.warning(f"⚠️ [FIRST_RUN_PERMISSIONS] Не удалось загрузить конфигурацию: {e}, используем значения по умолчанию")
            self.required_permissions = [
                "microphone",
                "screen_capture",
                "accessibility",
    ...
    async def start(self) -> bool:
        ...
        2. Если какое-то не GRANTED → активируем и ждём с таймаутом:
           - Microphone: без таймаута (пользователь может принять решение)
           - Accessibility/Input/Screen: после 10s ожидания открывает Settings,
             затем ждёт max_wait_after_settings_open_sec (60s по умолчанию)
           - По таймауту продолжает запуск БЕЗ блокировки
        3. Когда все получены → проверяем, нужен ли перезапуск
        4. Перезапуск нужен для Accessibility/Input Monitoring/Screen Capture (CGEventTap)

        ВАЖНО: Изменение контракта (v2):
        - БЛОКИРУЕТ запуск остальных интеграций только до таймаута или выдачи всех разрешений
        - После таймаута продолжает запуск, даже если разрешения не выданы
        - Риск: поздняя выдача разрешения может потребовать перезапуска приложения
        """
        try:
            ...
            # REQ-004: use selector for restart_completed_fallback check
            is_post_restart = self.flag_file.exists() and selectors.is_restart_completed_fallback(self.state_manager)
            ...
            if all_granted:
                if is_post_restart:
                    logger.info("✅ [PERMISSIONS] Перезапуск после выдачи разрешений подтвержден")
    ...
    async def _activate_and_wait_for_permission(
        ...
        *,
        ...
        """
        Активирует запрос разрешения и ждёт его получения с таймаутом и UX-подсказками.

        ВАЖНО: Изменение контракта (v2):
        - Раньше: бесконечное ожидание до выдачи разрешения (блокировало запуск)
        - Теперь: таймаут для Settings-разрешений (accessibility/input/screen) с продолжением запуска
        
        Поведение:
        - Microphone: без таймаута (пользователь может принять решение в диалоге)
        - Accessibility/Input/Screen: после 10s ожидания открывает Settings и показывает help dialog,
          затем ждёт max_wait_after_settings_open_sec (60s по умолчанию)
        - По истечении таймаута продолжает запуск БЕЗ блокировки (разрешение может быть выдано позже)
        ...
        # Разрешения, требующие открытия Settings (не показывают диалог автоматически)
        settings_required_permissions = {
            PermissionType.ACCESSIBILITY,
            PermissionType.INPUT_MONITORING,
            PermissionType.SCREEN_CAPTURE,
        }
        requires_settings = permission_type in settings_required_permissions
        
        # Таймауты из конфига
        # open_settings_after_sec: когда открывать Settings для разрешений типа Accessibility/Screen Capture
        # max_wait_sec: унифицированный максимальный таймаут ожидания для КАЖДОГО разрешения
        
        # Сначала проверяем текущий статус
        old_status = check_func()
        ...
        if old_status == PermissionStatus.GRANTED:
            logger.info(f"✅ [{permission_type.value}] Уже выдано!")
            logger.info(f"[PERMISSIONS] {permission_type.value} granted")
            return

        # Активируем (покажет диалог для NOT_DETERMINED, если доступен)
        logger.info(f"⏳ [{permission_type.value}] Активация запроса разрешения...")
        await activate_func()
        
        # Session guard для Screen Capture: отмечаем что диалог уже показан
        if permission_type == PermissionType.SCREEN_CAPTURE:
            mark_screen_capture_prompt_shown()

        # Отслеживание времени ожидания
        start_time = time.time()
        settings_opened = False
        help_dialog_task: Optional[asyncio.Task] = None
        attempt = 0
        
        try:
            while True:
                # 1. Проверяем статус задачи диалога (если запущена)
                if help_dialog_task and help_dialog_task.done():
                    try:
                        result = help_dialog_task.result()
                        help_dialog_task = None
                        
                        if result == "quit":
                            logger.info(f"🛑 [{permission_type.value}] User requested Quit via dialog.")
                            import sys
                            sys.exit(0)
                        elif result == "open_settings":
                            logger.info(f"📋 [{permission_type.value}] User requested to Open Settings again.")
                            open_settings_func()
                            # Мы не сбрасываем settings_opened, чтобы не спамить диалогами
                            # Диалог закрылся, пользователь в настройках. Ждем разрешения.
                    except Exception as e:
                        logger.warning(f"⚠️ [{permission_type.value}] Error in help dialog task: {e}")
                        help_dialog_task = None
                ...
                if attempt % 5 == 0:
                    dialog_status = "active" if help_dialog_task and not help_dialog_task.done() else "inactive"
                    logger.info(f"⏳ [{permission_type.value}] Waiting... (status={status.value}, elapsed={elapsed_sec:.1f}s, dialog={dialog_status})")

                # Если получили разрешение -> Ура, но сначала подтвердим стабильность
                if status == PermissionStatus.GRANTED:
                    logger.info(f"✅ [{permission_type.value}] Первичный GRANTED получен. Подтверждаем стабильность...")
                    
                    # Confirmation retry: 1-2 дополнительных проверки с delay 2s
                    # для защиты от stale cache ответов macOS
                    confirmation_passed = True
                    for confirm_attempt in range(2):
                        await asyncio.sleep(2.0)
                        confirm_status = check_func()
                        logger.debug(f"  [{permission_type.value}] Confirmation #{confirm_attempt+1}: {confirm_status.value}")
                        
                        if confirm_status != PermissionStatus.GRANTED:
                            logger.warning(f"⚠️ [{permission_type.value}] Confirmation failed: {confirm_status.value}")
                            confirmation_passed = False
                            break
                    
                    if confirmation_passed:
                        logger.info(f"✅ [{permission_type.value}] Подтверждено! Статус стабилен.")
                        logger.info(f"[PERMISSIONS] {permission_type.value} granted")
                        
                        # Публикуем событие permissions.changed при первом GRANTED
                        ...
                        # КРИТИЧНО: Если диалог висит — закрываем его (cancel task)
                        if help_dialog_task and not help_dialog_task.done():
                            logger.info(f"✅ [{permission_type.value}] Closing help dialog (permission granted)...")
                            help_dialog_task.cancel()
                            try:
                                await help_dialog_task
                            except asyncio.CancelledError:
                                pass
                        
                        return
                    else:
                        # Confirmation failed - continue waiting loop
                        logger.info(f"⏳ [{permission_type.value}] Продолжаем ожидание (confirmation failed)...")
                        continue
                ...
                if status == PermissionStatus.DENIED:
                    ...
                    logger.info(f"[PERMISSIONS] {permission_type.value} denied")
                    
                    # Публикуем событие permissions.changed при DENIED
                    await self._publish_permission_changed(
                        permission=permission_type,
                        old_status=old_status,
                        new_status=status,
                ...
                # Если NOT_DETERMINED -> Пользователь ещё не нажал кнопку
                if status == PermissionStatus.NOT_DETERMINED:
                    # УНИФИЦИРОВАННЫЙ ТАЙМАУТ (max_wait_sec из конфига)
                    # После этого таймаута переходим к следующему разрешению
                    if elapsed_sec >= self.max_wait_sec:
                        # СПЕЦИАЛЬНЫЙ СЛУЧАЙ: Микрофон с stale cache
                        # AVFoundation может возвращать cached NOT_DETERMINED даже после выдачи разрешения
                        # В этом случае предполагаем GRANTED
                        if permission_type == PermissionType.MICROPHONE:
                            logger.warning(
                                f"⚠️ [microphone] Timeout after {elapsed_sec:.1f}s with NOT_DETERMINED - "
                                f"assuming GRANTED (AVFoundation cache may be stale). Proceeding..."
                            )
                            logger.info("[PERMISSIONS] microphone timeout_assumed_granted")
                            
                            assumed_status = PermissionStatus.GRANTED
                            await self._publish_permission_changed(
                                permission=permission_type,
                                old_status=old_status,
                                new_status=assumed_status,
                                session_id=session_id,
                                source="permissions.stale_cache_assumed"
                            )
                            await self._publish_status_checked(
                                permission=permission_type,
                        ...
                        # Для остальных разрешений: timeout = DENIED
                        logger.warning(
                            f"⏱️ [{permission_type.value}] Unified timeout after {elapsed_sec:.1f}s "
                            f"(max_wait_sec={self.max_wait_sec}s). Proceeding to next permission..."
                        )
                        logger.info(f"[PERMISSIONS] {permission_type.value} timeout_proceed_to_next")
                        
                        timeout_status = PermissionStatus.DENIED
                        
                        logger.warning(
                            f"⚠️ [{permission_type.value}] Timeout: продолжаем БЕЗ разрешения. "
                            f"Если разрешение будет выдано позже, может потребоваться перезапуск."
                        )
                        
                        await self._publish_permission_changed(
                            permission=permission_type,
                            old_status=old_status,
                            new_status=timeout_status,
                            session_id=session_id,
                            source="permissions.timeout",
                            is_timeout=True
                        )
                        await self._publish_status_checked(
                            permission=permission_type,
                        ...
                        # Закрываем диалог если был открыт
                        if help_dialog_task and not help_dialog_task.done():
                            help_dialog_task.cancel()
                            try:
                                await help_dialog_task
                    ...
                    # Для разрешений, требующих Settings: открываем Settings после open_settings_after_sec
                    # И запускаем диалог в ФОНЕ (не блокируя цикл проверки)
                    if requires_settings and not settings_opened and elapsed_sec >= self.open_settings_after_sec:
                        logger.info(f"📋 [{permission_type.value}] NOT_DETERMINED дольше {self.open_settings_after_sec}s - открываем Settings...")
                        open_settings_func()
                        settings_opened = True
                        # settings_open_time = time.time() # Not used currently
                        
                        # Показываем help dialog в фоне (Non-blocking)
                        if not help_dialog_task:
                            logger.info(f"📋 [{permission_type.value}] Launching help dialog (background task)...")
                            help_dialog_task = asyncio.create_task(
                                self._show_permission_help_dialog(permission_type, open_settings_func)
                            )
                
                await asyncio.sleep(check_interval)
            
        except asyncio.CancelledError:
            # Если нас отменили — чистим за собой
            if help_dialog_task and not help_dialog_task.done():
                help_dialog_task.cancel()
                try:
                    await help_dialog_task
    ...
    async def _show_permission_help_dialog(self, permission_type: PermissionType, open_settings_func) -> str:
        ...
        try:
            ...
            try:
                ...
                stdout, stderr = await asyncio_proc.communicate()
                ...
                if "Quit" in output:
                    ...
                    # Повторно открываем настройки (будет обработано вызывающим кодом или здесь)
                    # open_settings_func() # Caller handles re-opening if needed, but we can do it here too safety
                    return "open_settings"
                else:
                    return "continue"
            
            except asyncio.CancelledError:
                logger.info("🛑 Help dialog cancelled - killing osascript...")
                if asyncio_proc:
                    try:
                        asyncio_proc.terminate()
                        # Даем немного времени на завершение
                        try:
                            await asyncio.wait_for(asyncio_proc.wait(), timeout=0.5)
                        except asyncio.TimeoutError:
                            logger.warning("⚠️ osascript did not terminate - force killing")
                            asyncio_proc.kill()
                    except Exception as e:
                        logger.warning(f"⚠️ Error killing osascript: {e}")
                raise
    ...
    async def _publish_permission_changed(
        ...
        *,
        ...
        """
        Публикует событие permissions.changed.
        
        ВАЖНО: При source="permissions.timeout" устанавливается is_timeout=True,
        чтобы подписчики могли различать таймаут от реального отказа (DENIED).
        new_status может быть "denied" как для реального отказа, так и для таймаута,
        но is_timeout явно указывает на таймаут.
        """
        payload = {
            "permission": permission.value,
        ...
        logger.info(
            "[FIRST_RUN_PERMISSIONS] permissions.changed -> %s (%s → %s, session=%s, source=%s, is_timeout=%s)",
            permission.value,
            old_status.value,
            new_status.value,
            session_id,
