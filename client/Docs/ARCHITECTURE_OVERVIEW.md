...
- `browser_automation` — НОВЫЙ: клиентский модуль browser-use для AI-управляемой автоматизации браузера (Chromium/Playwright).
...
**Ключевой модуль (сервер): `browser_use`** (Feature ID: F-2025-015)
- AI-управляемая автоматизация браузера: выполнение веб-задач, поиск, клики, заполнение форм
- Автоматическая установка браузера при первом запуске (Chromium через Playwright)
- Персистентные сессии: браузер остается открытым между последовательными задачами
- Команда `close_browser` для явного закрытия сессии
