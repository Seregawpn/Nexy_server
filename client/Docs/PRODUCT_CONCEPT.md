...
3. Проигрывается короткий звуковой сигнал активации (listen_start)
4. Активируется микрофон
5. Начинается распознавание речи
6. Пользователь говорит команду
```
...
3. Установка завершена → `updater.update_completed`
4. Если пользователь НЕ выбирал Quit (`USER_QUIT_INTENT=false`) → перезапуск приложения
5. Если пользователь выбрал Quit (`USER_QUIT_INTENT=true`) → перезапуск пропускается
```
...
Примечание: клавиатурный монитор работает в observe-only режиме и не блокирует системные шорткаты macOS.
