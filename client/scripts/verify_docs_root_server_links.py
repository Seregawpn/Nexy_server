...
def resolve_ref(doc_file: Path, ref: str) -> Path:
    ...
    if ref.startswith("client/Docs/"):
        return ROOT / ref.replace("client/Docs/", "Docs/", 1)
    return ROOT / ref
        ...
        for md in sorted(base.glob("*.md")):
            ...
            for line in text.splitlines():
                if md.name == "README.md":
                    if line.strip().startswith("## Отсутствующие документы"):
                        in_missing_section = True
                    elif line.strip().startswith("## ") and in_missing_section:
                        in_missing_section = False
                for ref in BACKTICK_RE.findall(line):
                    if in_missing_section:
                        continue
                    if should_skip(ref) or not is_path_like(ref):
                        continue
                    if ref.startswith("server/") and not (ROOT / "server").exists():
                        # Client-only checkout: server tree absent by design.
                        continue
