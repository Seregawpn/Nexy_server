    ...
    def check_logs_specs(self) -> bool:
        ...
        try:
            ...
            process = subprocess.Popen(
                ["open", "-W", str(self.app_path)], stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            ...
            for pattern, description in checks:
                result = subprocess.run(
                    [
                        ...
                        "compact",
                        "--predicate",
                        f'process == "Nexy" && eventMessage CONTAINS "{pattern}"',
                    ],
                    capture_output=True,
                    text=True,
                    timeout=10,
        ...
        try:
            ...
            if server_name:
                ...
                if connected:
                    ...
                    self.log_warn(
                        f"⚠️ gRPC подключение к {server_name} не удалось (сервер может быть недоступен)"
                    )
...
def main():
    ...
    parser.add_argument(
        "--smoke", action="store_true", help="Smoke mode: быстрые проверки без сборки и сервера"
    )
