...
def extract_pkg_app(pkg_path: Path, extract_dir: Path) -> Path | None:
    ...
    code, _, stderr = run_cmd(["tar", "-xf", str(payload), "-C", str(app_extract_dir)], check=False)
    ...
    if code != 0:
        ...
        try:
            ...
            code, _, _ = run_cmd(
                ["cd", str(app_extract_dir), "&&", "cpio", "-i", "<", str(temp_cpio)], check=False
            )
            if temp_cpio.exists():
                temp_cpio.unlink()
...
def validate_app_architectures(app_path: Path) -> bool:
    ...
    resource_binaries = [
        ...
        (
            "SwitchAudioSource",
            app_path / "Contents" / "Resources" / "resources" / "audio" / "SwitchAudioSource",
        ),
...
def validate_dmg(dmg_path: Path) -> bool:
    ...
    try:
        code, _, stderr = run_cmd(
            ["hdiutil", "attach", str(dmg_path), "-mountpoint", str(mount_point), "-quiet"],
            check=False,
        )
