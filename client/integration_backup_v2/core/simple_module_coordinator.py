...
class SimpleModuleCoordinator:
    ...
    def _start_background_loop(self):
        ...
        def _runner():
            ...
            def _bg_exception_handler(loop, context):
                exc = context.get("exception")
                if isinstance(exc, BlockingIOError) and getattr(exc, "errno", None) == 35:
                    handle = context.get("handle")
                    handle_repr = repr(handle) if handle else ""
                    message = context.get("message", "")
                    if "PollerCompletionQueue._handle_events" in handle_repr or "PollerCompletionQueue._handle_events" in message:
                        logger.debug("Ignoring grpc aio PollerCompletionQueue BlockingIOError")
                        return
