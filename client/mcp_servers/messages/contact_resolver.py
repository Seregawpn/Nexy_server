        ...
        if result.returncode == 0:
            ...
            if output and output != "{}":
                try:
                    ...
                    if display_label:
                        logger.info(f"Имя найдено через Contacts.framework (Swift): {display_label}")
                        return contact_data
            ...
            if output and output != "":
                ...
                if len(parts) >= 3:
                    ...
                    if full_name and full_name != "missing value":
                        logger.info(f"Имя найдено через Contacts.framework (AppleScript): {full_name}")
                        return {
...
def find_contacts_by_name(name: str) -> List[Dict[str, Any]]:
    ...
    if cache_key in _CONTACT_NAME_CACHE:
        ...
        if current_time - cached_time < _CACHE_TTL:
            logger.info(f"Использован кэш для поиска по имени '{name}' (возраст: {int(current_time - cached_time)}с)")
            return contacts
        else:
            # Кэш устарел, удаляем
...
def clear_expired_cache() -> int:
    ...
    current_time = time.time()
    expired_keys = [
        key for key, (_, cached_time) in _CONTACT_NAME_CACHE.items()
        if current_time - cached_time >= _CACHE_TTL
    ]
