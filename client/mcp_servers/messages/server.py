...
async def handle_find_contact(arguments: dict[str, Any]) -> list[TextContent]:
    ...
    try:
        ...
        return [
            TextContent(
                type="text",
                text=f"Error searching for contact: {str(e)}"
            )
        ]
    finally:
        conn.close()
...
async def handle_read_messages(arguments: dict[str, Any]) -> list[TextContent]:
        ...
        try:
            ...
            if not last_message:
                return [
                    TextContent(
                        type="text",
                        text="No messages found in database."
                    )
                ]
            ...
            return [
                TextContent(
                    type="text",
                    text=f"Error getting last message: {str(e)}"
                )
            ]
        finally:
            conn.close()
    ...
    if not isinstance(contact, str):
        return [
            TextContent(
                type="text",
                text="Error: 'contact' must be a string"
            )
        ]
    ...
    try:
        ...
        if not is_phone_or_email:
            ...
            if len(all_contacts) == 1:
                ...
                if not all_messages:
                    ...
                    result_lines = [f"Found {len(all_contacts)} contacts matching '{contact}', but no messages found:"]
                ...
                for contact_name, msgs in messages_by_contact.items():
                    ...
                    formatted = messages_db.format_messages(msgs, contact_name, group_by_date=True, show_chat_info=False, messages_conn=conn)
            ...
            if contact_name:
                ...
                if contacts:
                    ...
                    return [
                        TextContent(
                            type="text",
                            text=f"No messages found with {contact_id}"
                        )
                    ]
        ...
        for i, msg in enumerate(messages[:3]):  # Проверяем первые 3 сообщения
            ...
            logger.info(f"[MCP Messages] Message #{i+1} text (first 100 chars): {msg_text[:100] if msg_text else '(empty)'}")
            if not msg_text or msg_text.strip() == "":
                attributed_body = msg.get("attributed_body")
                logger.info(f"[MCP Messages] Message #{i+1} has empty text, attributed_body present: {attributed_body is not None}")
                if attributed_body:
        ...
        for line in lines:
            ...
            if line.startswith("💬") or line.startswith("Service:") or line.startswith("Last ") and "messages:" in line:
                continue
            
            # Начинаем собирать сообщения после заголовка даты или сразу с сообщения
            if line.startswith("📅") or (":" in line and ("[" in line or line.strip().startswith("You:"))):
        ...
        if not simplified_lines or len(simplified_lines) < 2:
            ...
            logger.info(f"[MCP Messages] Simplified text (first 500 chars): {simplified_text[:500]}")
...
async def handle_send_message(arguments: dict[str, Any]) -> list[TextContent]:
    ...
    if not contact:
        return [
            TextContent(
                type="text",
                text="Error: 'contact' parameter is required"
            )
        ]
    ...
    if not isinstance(contact, str) or not isinstance(message_text, str):
        return [
            TextContent(
                type="text",
                text="Error: 'contact' and 'message' must be strings"
            )
        ]
        ...
        if is_phone_or_email:
            ...
            if not phone_numbers:
                raise NoPhoneNumbersError(
                    contact,
                    f"No phone numbers found for contact '{contact}'"
                )
            ...
            if len(phone_numbers) > 1:
                ...
                if specified_phone and specified_phone in phone_numbers:
                    ...
                    if conn:
                        try:
                            phone_number = messages_db.get_phone_with_last_message(conn, phone_numbers)
                            if phone_number:
                                logger.info(f"Выбран номер с последним сообщением: {phone_number} из {phone_numbers}")
                            else:
                                phone_number = phone_numbers[0]
                                logger.warning(f"Не удалось выбрать номер, используем первый: {phone_number}")
                        finally:
                            conn.close()
                    else:
                        phone_number = phone_numbers[0]
                        logger.warning(f"Не удалось подключиться к БД, используем первый номер: {phone_number}")
            else:
                phone_number = phone_numbers[0]
