...
class TelegramWrapper:
    ...
    async def connect(self):
        ...
        if not DEFAULT_API_ID or not DEFAULT_API_HASH:
            ...
            logger.warning(
                "TELEGRAM_API_ID/HASH not set. Connection might fail if not embedded in session."
            )

        self.client = TelegramClient(
            StringSession(self._session_str), int(DEFAULT_API_ID or 0), DEFAULT_API_HASH or "dummy"
        )
        if self.client:
    ...
    async def send_message(self, chat_id: Any, text: str) -> dict[str, Any]:
        ...
        return {"id": sent.id, "chat_id": sent.chat_id, "date": sent.date.isoformat()}
