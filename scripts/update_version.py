#!/usr/bin/env python3
"""Centralized release version sync.

Editable source of truth for release version: client/config/unified_config.yaml app.version.
VERSION is kept as a compatibility-derived file for legacy consumers.
Manifest/config/docs are derived and synced by this script.
...
def _read_version() -> str:
    if not CLIENT_UNIFIED_YAML.exists():
        raise FileNotFoundError(f"Client unified config not found: {CLIENT_UNIFIED_YAML}")
    data = yaml.safe_load(CLIENT_UNIFIED_YAML.read_text(encoding="utf-8")) or {}
    value = str(data.get("app", {}).get("version", "")).strip()
    return _validate_version(value)


def _write_client_version(version: str) -> bool:
    data = yaml.safe_load(CLIENT_UNIFIED_YAML.read_text(encoding="utf-8")) or {}
    app = data.setdefault("app", {})
    current = str(app.get("version", "")).strip()
    if current == version:
        return False
    app["version"] = version
    CLIENT_UNIFIED_YAML.write_text(
...
def _write_compat_version_file(version: str) -> bool:
...
def main() -> int:
    ...
    parser.add_argument("--read", action="store_true", help="Read current config owner version")
    ...
    try:
        ...
        if args.check:
            ...
            if current != version:
                print(f"DRIFT: config.app.version={current}, expected={version}")
                return 2
            print(f"OK: config.app.version={current}")
            return 0

        changed: list[str] = []

        if _write_client_version(version):
            changed.append(str(CLIENT_UNIFIED_YAML.relative_to(PROJECT_ROOT)))

        if _write_compat_version_file(version):
        ...
        print(f"✅ Version synchronized from source (client config): {version}")
        if unique_changed:
            print("Updated files:")
