...
RELEASE_INBOX_PRIMARY = ROOT_DIR / "client/release_inbox"
RELEASE_INBOX_LEGACY = ROOT_DIR / "server/release_inbox"
...
def _channel_inbox_target(name: str) -> str:
    mapping = {
        "stable": "production",
        "beta": "staging",
    }
    ...
    if target is None:
        ...
        raise RuntimeError(f"Unsupported inbox target mapping for channel '{name}'. Allowed: {available}")
    return target
...
def _load_version(manifest_file: Path) -> str:
    ...
    raise RuntimeError(
        f"Cannot resolve version: VERSION is missing/empty, SERVER_VERSION is not set, and {manifest_file.name} version is empty"
    )


def _resolve_release_inbox(channel_name: str, version: str) -> Path:
    if RELEASE_INBOX_LEGACY.exists() and not RELEASE_INBOX_PRIMARY.exists():
        raise RuntimeError(
            ...
            "Use only client/release_inbox."
        )
    return RELEASE_INBOX_PRIMARY / _channel_inbox_target(channel_name) / version
...
def _ensure_manifest_pair_consistency(filename: str) -> None:
    canonical = MANIFESTS_DIR / filename
    legacy = LEGACY_MANIFESTS_DIR / filename
    if not legacy.exists():
        return
    if not canonical.exists():
        raise RuntimeError(
            f"Legacy manifest exists without canonical manifest: {legacy}. "
            f"Canonical owner is server/updates/manifests/{filename}."
        )
    try:
    ...
    if canonical_payload != legacy_payload:
        raise RuntimeError(
            f"Manifest owner conflict: server/updates/manifests/{filename} differs from "
            f"server/server/updates/manifests/{filename}. Remove legacy divergence first."
        )


def _ensure_manifest_owner_consistency() -> None:
    _ensure_manifest_pair_consistency("manifest.json")
    _ensure_manifest_pair_consistency("manifest_beta.json")
...
def _publish_channel(channel: ChannelConfig, dry_run: bool) -> int:
    ...
    release_inbox = _resolve_release_inbox(channel.name, version)
