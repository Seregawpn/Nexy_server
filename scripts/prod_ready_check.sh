...
config = get_config()
subscription_enabled = bool(config.subscription.enabled)
forward_actions = bool(config.features.forward_assistant_actions)
...
if not forward_actions:
    print("forward_assistant_actions=false (shared runtime config disables actions)")
