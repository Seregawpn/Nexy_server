...
```
Guards:
- canonical packaging inbox path only: `client/release_inbox/<target>/<version>`
  - beta publish reads from: `client/release_inbox/staging/<version>`
  - stable publish (blocked) would read from: `client/release_inbox/production/<version>`
