...
| `assistant_runtime_v2` | feature | assistant-runtime | false | server | `disable_assistant_runtime_v2` | Изолированный experimental routing в новый assistant runtime v2 через единый workflow entrypoint |
| `messages_enabled` | feature | messages | true | server | - | Включить обработку iMessage-команд в серверном prompt/router |
| `google_search_enabled` | feature | browser | true | server | - | Включить обработку Google Search intents и команд |
...
| `disable_assistant_runtime_v2` | kill_switch | assistant-runtime | false | server | - | Аварийно отключить experimental assistant runtime v2 и вернуть current runtime |
...
| `ASSISTANT_RUNTIME_V2` | env_override | assistant-runtime | false | server | - | Env переменная для включения experimental assistant runtime v2 |
| `NEXY_KS_DISABLE_ASSISTANT_RUNTIME_V2` | kill_switch | assistant-runtime | false | server | - | Env kill-switch для мгновенного отката assistant runtime v2 |
| `BROWSER_USE_ENABLED` | env_config | browser | true | server | - | Standalone fallback only; в монорепо manual owner живёт в `client/config/unified_config.yaml -> features.browser_use/browser` |
| `MESSAGES_ENABLED` | env_config | messages | true | server | - | Standalone fallback only; в монорепо manual owner живёт в `client/config/unified_config.yaml -> features.messages` |
| `SUBSCRIPTION_ENABLED` | env_config | payment | true | server | - | Standalone fallback only; в монорепо manual owner живёт в `client/config/unified_config.yaml -> features.payment` |
