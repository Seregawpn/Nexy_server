...
- Ошибка до старта стрима → `error_message`; после частичных данных → один terminal `error_message` (без silent termination и без смены gRPC статуса).
...
7. Стриминг ответа:
   - `text_chunk` (опционально; допускается один ранний preview для первого читаемого фрагмента)  
   - `audio_chunk` (обязательны `sample_rate`, `channels`, `dtype`, `shape`)  
8. Завершение:
   - 정상ное: `end_message`
   - ошибка до стрима: `error_message`
   - ошибка после чанков: terminal `error_message` без смены gRPC статуса

Requirements:
- Raw streaming от LLM и semantic emit — разные слои: `LangChainGeminiProvider` стримит raw chunks, а `StreamingWorkflowIntegration` централизованно решает, когда публиковать наружу `text_chunk` и когда синтезировать sentence-based `audio_chunk`.
- Ранний `text_chunk` preview допускается только как request-scoped text-only emit; он не открывает второй audio path и не отменяет sentence-based TTS.
- Если preview уже был отправлен, следующий sentence-based `text_chunk` не должен повторно публиковать тот же полный префикс; workflow обязан отправлять только новый видимый suffix.
...
Sequence:
1. Обновления проверяются и устанавливаются согласно `Docs/RELEASE_AND_UPDATE_GUIDE.md`.
