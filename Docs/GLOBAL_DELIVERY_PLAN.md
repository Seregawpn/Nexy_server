...
**Критерии готовности:**
- ✅ **Этап 1 — Клиент:** 20 интеграций реализованы (проверка: `ls integration/integrations/*.py | wc -l`)
- ⚠️ **Этап 2 — Масштабирование:** gRPC сервер настроен на 100 воркеров (проверка: `grep max_workers modules/grpc_client/config.py`)
- ✅ **Этап 3 — Первый запуск:** FirstRunPermissionsIntegration работает (проверка: флаг `~/.Nexy/permissions_first_run_completed.flag`)
- ⚠️ **Этап 4 — Упаковка:** чек-листы выполнены (проверка: тесты из раздела "ТЕСТ 1.1-1.3")
- ⚠️ **Этап 5 — Тестирование:** чек-листы выполнены (проверка: тесты из раздела "ТЕСТ 3.1-3.2")
- ⚠️ **Этап 6 — Видео:** готовность зависит от этапов 4-5

### **📊 Измеримые улучшения архитектуры:**
- **МАСШТАБИРОВАНИЕ:** gRPC сервер настроен на 100 воркеров (проверка: `grep max_workers modules/grpc_client/config.py || echo "не найдено"`)
- **МОДУЛЬНАЯ СТРУКТУРА:** gRPC файлы в `modules/grpc_client/` (проверка: `find modules/grpc_client -type f | wc -l`)
- **МОНИТОРИНГ:** Метрики в логах (проверка: `grep -c "metric:" logs/nexy.log || echo "0"`)
- **ТЕСТИРОВАНИЕ:** Скрипты в `scripts/` (проверка: `ls scripts/*test*.py | wc -l`)
- **СОВМЕСТИМОСТЬ:** Версии в `requirements.txt` (проверка: `pip list | grep -E "grpcio|protobuf"`)
- **СИСТЕМА ПЕРВОГО ЗАПУСКА:** Флаг `~/.Nexy/permissions_first_run_completed.flag` (проверка: `test -f ~/.Nexy/permissions_first_run_completed.flag && echo "ok" || echo "not found"`)
- **СЕРВЕРНАЯ ГЕНЕРАЦИЯ:** События `welcome.message.generated` в логах (проверка: `grep -c "welcome.message.generated" logs/nexy.log || echo "0"`)
- **АВТОПЕРЕЗАПУСК:** События `permission_restart.*` в логах (проверка: `grep -c "permission_restart" logs/nexy.log || echo "0"`)
- **20 ИНТЕГРАЦИЙ:** Файлы в `integration/integrations/` (проверка: `ls integration/integrations/*.py | wc -l`)
...
## ✅ 3) СИСТЕМА ПЕРВОГО ЗАПУСКА + СЕРВЕРНАЯ ГЕНЕРАЦИЯ - ЗАВЕРШЕНО
**Цель**: Создание системы первого запуска и переход на серверную генерацию приветствия.

✅ **ЭТАП 3 ПОЛНОСТЬЮ ЗАВЕРШЕН (24.10.2025):**

#### **🔐 СИСТЕМА ПЕРВОГО ЗАПУСКА:**
- ✅ **FirstRunPermissionsIntegration:** Новая интеграция для запроса разрешений при первом запуске
- ✅ **Умные паузы:** 13 секунд между диалогами для комфортного UX (источник истины: `config/unified_config.yaml` → `permissions.first_run.activation_hold_duration_sec`)
...
-#### **🏗️ ОБНОВЛЕННЫЙ ПОРЯДОК ИНИЦИАЛИЗАЦИИ:**
- ✅ **Правильная последовательность:** instance_manager → hardware_id → first_run_permissions → permission_restart → tray → остальные
- ✅ **Блокирующая логика:** first_run_permissions блокирует запуск остальных интеграций
- ✅ **Event-driven система:** События для мониторинга процесса первого запуска
...
- ✅ Все 20 интеграций + workflows реализованы и протестированы
...
**Действия:**
```bash
grep -A 10 "updater:" config/unified_config.yaml
...
**Действия:**
```bash
cd client && python client/main.py &
CLIENT_PID=$!
sleep 5
...
## 🔐 Definition of Ready: TCC/Перезапуск

**Цель**: Зафиксировать критерии готовности к релизу для системы разрешений (TCC) и автоматического перезапуска.

### ✅ Обязательные требования для релиза

#### 1. Критические разрешения (TCC)
После выдачи критических TCC (микрофон, экран, Accessibility) **ОБЯЗАТЕЛЕН** автоматический перезапуск приложения:
- ✅ **Микрофон** (`AVCaptureDevice.authorizationStatus`)
- ✅ **Screen Capture** (`CGPreflightScreenCaptureAccess`)
- ✅ **Accessibility** (`AXIsProcessTrusted`)
- ✅ **Input Monitoring** (CGEvent)

**Проверка**:
```bash
# Логи должны содержать:
grep "permission_restart.scheduled" ~/Library/Logs/Nexy/nexy.log
grep "permission_restart.executing" ~/Library/Logs/Nexy/nexy.log
```

#### 2. Целевые метрики

**Метрика 1: `tcc_prompt_duration_ms`** (p95)
- **Цель**: ≤ 5 секунд
- **Измерение**: Время от первого запроса разрешения до завершения всех запросов
- **Источник**: `first_run_permissions_integration.py`
- **Лог**: `[FIRST_RUN_PERMISSIONS] Duration: X ms`

**Метрика 2: `permission_flow_success`** (rate)
- **Цель**: ≥ 99%
- **Измерение**: Процент успешных завершений потока разрешений (без критических ошибок)
- **Источник**: События `permissions.first_run_completed` vs `permissions.first_run_failed`
- **Лог**: Счётчик успешных/неуспешных завершений

**Метрика 3: `permission_restart_success_rate`** (rate)
- **Цель**: ≥ 98%
- **Измерение**: Процент успешных автоматических перезапусков после предоставления разрешений
- **Источник**: События `permission_restart.executing` → проверка успешного перезапуска (новый процесс видит разрешения)
- **Лог**: `[PERMISSION_RESTART] Restart executed successfully`

**Метрика 4: `permission_restart_latency_ms`** (p95)
- **Цель**: ≤ 15 секунд (от предоставления разрешения до готовности нового процесса)
- **Измерение**: Время от `permission_restart.scheduled` до готовности нового процесса
- **Источник**: `permission_restart_integration.py` + логи
- **Лог**: Таймстампы событий `scheduled` → `executing` → `new_process_ready`

#### 3. Автоматический перезапуск (Permission Restart)

**Требования к поведению**:
- ✅ Перезапуск **ОБЯЗАТЕЛЕН** после выдачи критических разрешений в режиме первого запуска
- ✅ Перезапуск **ОБЯЗАТЕЛЕН** при обнаружении изменений разрешений через мониторинг (после первого запуска)
- ✅ Перезапуск **ОТКЛАДЫВАЕТСЯ** если:
  - Доступно обновление (`respect_updates: true` + `update_available: true`) → пропуск (обновление само перезапустит)
  - Приложение активно (`respect_active_sessions: true` + `appMode != SLEEPING`) → ждёт до 10 минут
  - Идёт установка обновления (`update_in_progress: true`) → ждёт до 10 минут
- ✅ Перезапуск **ПРИНУДИТЕЛЬНЫЙ** после 10 минут ожидания (таймаут)

**Проверка**:
```bash
# Проверить что перезапуск запланирован:
grep "Scheduling application restart" ~/Library/Logs/Nexy/nexy.log

# Проверить что перезапуск выполнен:
grep "permission_restart.executing" ~/Library/Logs/Nexy/nexy.log

# Проверить блокировки:
grep "Update available - skipping" ~/Library/Logs/Nexy/nexy.log
grep "current_mode" ~/Library/Logs/Nexy/nexy.log | grep -v SLEEPING
```

#### 4. Логирование и наблюдаемость

**Обязательные логи**:
- ✅ `permissions.first_run_started` — начало потока разрешений
- ✅ `permissions.first_run_completed` — успешное завершение
- ✅ `permissions.first_run_failed` — неуспешное завершение (с контекстом ошибки)
- ✅ `permission_restart.scheduled` — перезапуск запланирован (с причиной и задержкой)
- ✅ `permission_restart.executing` — перезапуск выполняется
- ✅ `permission_restart.skipped` — перезапуск пропущен (с причиной)

**Формат логов**:
```
YYYY-MM-DD HH:MM:SS - Nexy - LEVEL - [session_id] - Message
```

**Decision-логи** (см. `.cursorrules` раздел 8.x):
```
...
**Обязательные параметры в `unified_config.yaml`**:
```yaml
permission_restart:
  enabled: true                              # Включить механизм
  restart_delay_sec: 5.0                    # Задержка перед перезапуском (показать уведомление)
  max_restart_attempts: 3                    # Защита от циклов
  respect_active_sessions: true              # Не прерывать пользователя
  respect_updates: true                      # Учитывать обновления
  critical_permissions:                      # Список критических разрешений
    - microphone
    - accessibility
    - input_monitoring
    - screen_capture
...
**Обязательные тесты** (см. критерии в этом документе, раздел «Тестирование»):
- ✅ Базовый перезапуск по одному разрешению
- ✅ Перезапуск после нескольких разрешений
- ✅ Отсрочка при активном режиме LISTENING/PROCESSING
- ✅ Блокировка во время обновления
- ✅ Проверка флага `NEXY_DISABLE_AUTO_RESTART` (dry-run)
- ✅ Максимум попыток (`max_restart_attempts`)
- ✅ Негативные проверки (нет ложных срабатываний)

**Канонический чек-лист** (см. пункты ниже):
- ✅ Флаг `permissions_first_run_completed.flag` создаётся после первого запуска
- ✅ Последовательность диалогов: Микрофон → Accessibility → Screen Capture
- ✅ Паузы между диалогами: 7 секунд
- ✅ Ожидания перезапуска: SLEEPING режим или таймаут 10 минут
- ✅ Метрики: `tcc_prompt_duration_ms` ≤ 5 сек (p95), `permission_flow_success` ≥ 99%

### 🚫 Критерии блокировки релиза

**НЕ выпускаем релиз если**:
- ❌ Метрика `tcc_prompt_duration_ms` (p95) > 5 секунд
- ❌ Метрика `permission_flow_success` < 99%
- ❌ Метрика `permission_restart_success_rate` < 98%
- ❌ Автоматический перезапуск не работает для критических разрешений
- ❌ Нет логов `permission_restart.scheduled/executing` после предоставления разрешений
- ❌ Перезапуск происходит во время активных сессий (LISTENING/PROCESSING) без ожидания
- ❌ Перезапуск конфликтует с системой обновлений (двойной перезапуск)

### 📊 Отчёт готовности

**Перед релизом ОБЯЗАТЕЛЬНО предоставить**:
1. ✅ Метрики: `tcc_prompt_duration_ms`, `permission_flow_success`, `permission_restart_success_rate`, `permission_restart_latency_ms`
2. ✅ Логи: 10 успешных сценариев первого запуска + перезапуска
3. ✅ Тесты: Все сценарии из чек‑листа выполнены
4. ✅ Конфигурация: Проверена в `unified_config.yaml`
5. ✅ Документация: Обновлены `STATE_CATALOG.md`, `interaction_matrix.yaml` (если менялись оси/правила)

**Шаблон отчёта**:
```markdown
## Отчёт готовности TCC/Перезапуск (версия X.Y.Z)

### Метрики
- tcc_prompt_duration_ms (p95): X сек ✅/❌
- permission_flow_success: X% ✅/❌
- permission_restart_success_rate: X% ✅/❌
- permission_restart_latency_ms (p95): X сек ✅/❌

### Тесты
- [ ] Базовый перезапуск по одному разрешению
- [ ] Перезапуск после нескольких разрешений
- [ ] Отсрочка при активном режиме
- [ ] Блокировка во время обновления
- [ ] Все негативные проверки

### Логи
- Флаг: [ссылка на логи первого запуска]
- Перезапуск: [ссылка на логи перезапуска]
- Метрики: [ссылка на логи с метриками]

### Вывод
✅ ГОТОВО К РЕЛИЗУ / ❌ НЕ ГОТОВО (указать причины)
```
...
- ✅ **ЭТАП 1 ЗАВЕРШЕН:** Все 20 интеграций + workflows реализованы
...
- `Docs/PACKAGING_FINAL_GUIDE.md` — единственная инструкция по упаковке (PyInstaller + подпись + нотарификация).
...
- Email для Certbot: `admin@yourdomain.com` (замените на ваш email)
...
- Apple ID (email): `YOUR_APPLE_ID@example.com` (замените на ваш Apple ID)
- Team ID (App ID Prefix): `YOUR_TEAM_ID` (замените на ваш Team ID из Apple Developer)
...
- Developer ID Application: "Developer ID Application: <Your Name> (YOUR_TEAM_ID)" (замените на ваш Team ID)
- Developer ID Installer:  "Developer ID Installer: <Your Name> (YOUR_TEAM_ID)" (замените на ваш Team ID)
...
codesign --deep --force --options runtime \
  ...
  --sign "Developer ID Application: <Your Name> (YOUR_TEAM_ID)" Nexy.app

# Сборка и подпись инсталлятора (.pkg)
productbuild --component Nexy.app /Applications Nexy.pkg
productsign --sign "Developer ID Installer: <Your Name> (YOUR_TEAM_ID)" Nexy.pkg Nexy.pkg
