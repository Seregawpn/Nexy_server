# 🏗️ Детальное Предложение Архитектуры

**Дата:** 2025-12-13  
**Статус:** Полный анализ и предложение
...
│   └── Проверка разрешений
│
├── _on_recording_start() (строки 735-1300)
...
│
└── _on_recording_stop() (строки 1549-1770)
...
1. Пользователь нажимает Ctrl+N
   └─ InputProcessingIntegration публикует voice.recording_start
...
3. Пользователь отпускает Ctrl+N
   └─ InputProcessingIntegration публикует voice.recording_stop

4. VoiceRecognitionIntegration._on_recording_stop()
   ├─ Google деактивация (строки 1549-1707)
   └─ AVF деактивация (строки 1769-1770)
```
...
## 🎯 Часть 2: Идеальная Архитектура (КАК ДОЛЖНО БЫТЬ)
...
#### Принцип 4: Четкая инициализация
...
### 2.2 Идеальная структура модулей
...
modules/audio_avf/
├── __init__.py
│   └─ from .core.avf_manager import AVFManager, AVFConfig
│
├── core/
│   ├── __init__.py
...
class AVFManager:
    ...
    def is_active(self) -> bool:
        ...
        if self._engine is None:
            return False
        return self._engine.is_input_active()
```
...
│   │   │   ├── initialize() -> bool
│   │   │   ├── start_recording(callback: Callable) -> bool
│   │   │   ├── stop_recording() -> bool
│   │   │   └── recognize(audio_data: AudioData) -> str
│   │   └── types.py
│   │
...
### 2.3 Идеальная структура интеграции
...
class VoiceRecognitionIntegration:
    ...
    def __init__(self, event_bus, state_manager, error_handler, config):
        ...
        # Флаги состояния УДАЛЕНЫ - используем только state_manager
    ...
    async def _on_google_audio_chunk(self, recognizer, audio):
        ...
        self._google_manager._audio_data = audio
...
│                                                          │
│  ┌──────────────────────────────────────────────────┐  │
│  │ AVF Логика (строки 228-280, 832-1148, 1549-1770) │  │
│  │ - Инициализация AVF                              │  │
│  │ - Активация микрофона                            │  │
│  │ - Деактивация микрофона                          │  │
│  │ - Получение диагностики                          │  │
...
│                                                          │
│  ┌──────────────────────────────────────────────────┐  │
│  │ Google Логика (строки 370-450, 1148-1300, 1549-1770)│
...
## 🔄 Часть 4: План Миграции (Детальный)
...
1. Создать `modules/audio_avf/core/avf_manager.py`:
   ...
   class AVFManager:
       ...
       async def initialize(self) -> bool:
           # Логика из VoiceRecognitionIntegration.initialize() (строки 228-280)
       
       async def activate(self, duration_sec: float = 1.0) -> DeviceInfo:
           # Логика из _get_device_info_via_avf() (строки 1282-1340)
       
       async def deactivate(self) -> bool:
           # Логика из _on_recording_stop() (строки 1769-1770)
   ```
...
3. Перенести AVF логику из `_on_recording_start()`:
   ```python
...
1. Создать `modules/audio_google/core/google_manager.py`:
   ...
   class GoogleManager:
       ...
       async def initialize(self) -> bool:
           # Логика из VoiceRecognitionIntegration.initialize() (строки 370-450)
       
       async def activate(self, callback: Callable) -> bool:
           # Логика из _on_recording_start() (строки 1148-1300)
       
       async def deactivate(self) -> bool:
           # Логика из _on_recording_stop() (строки 1549-1707)
   ```
...
1. Удалить AVF логику из `VoiceRecognitionIntegration`:
   ...
   - Удалить строки 1549-1770 (деактивация AVF)
