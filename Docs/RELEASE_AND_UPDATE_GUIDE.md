...
**Текущий релиз документации:** `v2.0.0.40`
...
- Runtime owner-path:
  - production: `/home/azureuser/voice-assistant`
  - staging: `/home/azureuser/voice-assistant-staging`
...
2. Канонический inbox channel-path содержит артефакты:
   - beta publish source: `client/release_inbox/staging/<version>/`
   - stable publish source: `client/release_inbox/production/<version>/`
...
az vm run-command invoke \
  ...
  --scripts "grep -q '^GEMINI_API_KEY=' /home/azureuser/voice-assistant-staging/config.env && echo OK || (echo MISSING && exit 1)"
```
...
- артефакты читаются только из target-scoped inbox:
  - beta -> `client/release_inbox/staging/<version>/`
  - stable -> `client/release_inbox/production/<version>/`
...
# beta/staging runtime
bash server/scripts/update_server_version.sh X.Y.Z.W --runtime-target staging
```
...
bash server/server/scripts/deploy_server_one_click.sh --tag vX.Y.Z.W --runtime-target staging
...
az vm run-command invoke \
  ...
  --name NexyNew \
  --command-id RunShellScript \
  --scripts "sudo sed -i 's/^UPDATE_BETA_ENABLED=.*/UPDATE_BETA_ENABLED=true/' /home/azureuser/voice-assistant-staging/config.env || echo 'UPDATE_BETA_ENABLED=true' | sudo tee -a /home/azureuser/voice-assistant-staging/config.env; sudo systemctl start voice-assistant-staging"
```
...
# beta/staging manifest owner-path
bash server/scripts/update_manifest_remote_locked.sh --channel beta --runtime-target staging \
  --resource-group NexyNewRG \
  --vm NexyNew \
  --url https://github.com/Seregawpn/Nexy_production/releases/download/UpdateBeta/Nexy.dmg \
  --size <SIZE_BYTES> \
  --sha256 <SHA256> \
  --version X.Y.Z.W \
  --build X.Y.Z.W
...
az vm run-command invoke \
  ...
  --name NexyNew \
  --command-id RunShellScript \
  --scripts "sudo sed -i 's/^UPDATE_BETA_ENABLED=.*/UPDATE_BETA_ENABLED=false/' /home/azureuser/voice-assistant-staging/config.env || echo 'UPDATE_BETA_ENABLED=false' | sudo tee -a /home/azureuser/voice-assistant-staging/config.env; sudo systemctl restart voice-assistant-staging"
```
