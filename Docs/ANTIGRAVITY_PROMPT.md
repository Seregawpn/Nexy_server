...
- Обязательно включать блок **"Информация об изменениях"**:
  - что изменено (кратко, по пунктам);
  - список файлов;
  - причина/цель изменений;
  - как проверено (валидация/проверки).
- Блок **"Информация об изменениях"** размещать в каждом отчете сразу после раздела `## Verification`.
- Если изменений нет, обязательно явно указать: **"Изменения не вносились"**.
