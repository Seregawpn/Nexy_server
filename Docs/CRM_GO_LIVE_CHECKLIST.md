...
- [x] `CRM_CONSOLIDATED_RULES.md` объявлен immutable
- [x] Изменения правил → только через `type: decision`
...
- [x] Ассистент A и B правят одну задачу
- [x] Один получает 409
- [x] Корректно перечитывает snapshot
- [x] Отказывается/адаптируется

### Сценарий 2: Шум (Noise Control)
- [x] Ассистент создаёт документ без влияния
- [x] НЕ создаётся задача
- [x] Фиксируется note или audit

### Сценарий 3: Блокер
- [x] Задача заблокирована
- [x] Ассистент не начинает работу
- [x] Создаёт blocker / decision / escalation
