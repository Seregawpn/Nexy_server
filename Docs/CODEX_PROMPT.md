...
- Перед изменениями проверь текущий код на дубликаты/близкую логику (>=70% сходства) и объедини в одного владельца.
- Любой новый флаг/стейт допустим только при явном владельце, границах действия и плане удаления; конфликт с `Docs/FEATURE_FLAGS.md` запрещен.
