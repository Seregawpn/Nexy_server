...
### ✅ Добавление нового функционала (единый маршрут)
...
- **End-to-End**: единый контракт payload + `session_id`, клиент публикует/подписывается через EventBus, сервер исполняет capability через ModuleFactory.

### 🔧 **Статус интеграций (Октябрь 2025):**

**Критерии готовности:**
- ✅ **20 интеграций реализованы** - см. `integration/integrations/` (список файлов)
- ✅ **Workflows реализованы** - ListeningWorkflow и ProcessingWorkflow (см. тесты: `integration/workflows/tests/`)
- ✅ **EventBus функциональна** - логи событий в `logs/nexy.log`, формат: `YYYY-MM-DD HH:MM:SS - Nexy - LEVEL - [session_id] - Message`
- ✅ **Конфигурация централизована** - `config/unified_config.yaml` (проверка: `python -c "from config.unified_config_loader import UnifiedConfigLoader; print(UnifiedConfigLoader().load())"`)
- ⚠️ **Масштабирование** - gRPC сервер настроен на 100 воркеров (проверка: нагрузочные тесты из `scripts/load_test.py`)
- ✅ **Модульная структура** - gRPC файлы в `modules/grpc_client/` (проверка: `find modules/grpc_client -name "*.proto"`)
- ⚠️ **Мониторинг** - метрики в логах (проверка: grep "metric:" logs/nexy.log)
- ⚠️ **Нагрузочное тестирование** - скрипты в `scripts/` (проверка: `ls scripts/*test*.py`)
- ✅ **FirstRunPermissionsIntegration** - флаг `~/Library/Application Support/Nexy/permissions_first_run_completed.flag` (проверка: `test -f "$HOME/Library/Application Support/Nexy/permissions_first_run_completed.flag"`)
- ✅ **PermissionRestartIntegration** - логи событий `permission_restart.*` (проверка: grep "permission_restart" logs/nexy.log)
- ✅ **Серверная генерация** - gRPC TTS (проверка: события `welcome.message.generated` в логах)
- ⚠️ **Упаковка/деплой** - см. `Docs/PACKAGING_FINAL_GUIDE.md` и `packaging/` для инструкций по упаковке

### ✅ **Измеримые улучшения архитектуры:**
- **МАСШТАБИРОВАНИЕ:** gRPC сервер настроен на 100 воркеров (проверка: `grep max_workers modules/grpc_client/config.py`)
...
- **ТЕСТИРОВАНИЕ:** Скрипты нагрузочного тестирования (проверка: `ls scripts/*test*.py | wc -l`)
- **СОВМЕСТИМОСТЬ:** Версии библиотек в `requirements.txt` (проверка: `pip list | grep -E "grpcio|protobuf"`)

### ✅ **РЕШЕННЫЕ ПРОБЛЕМЫ (ОБНОВЛЕНО 09.11.2025):**
- ✅ **РЕШЕНО:** Добавлена `FirstRunPermissionsIntegration` - система первого запуска работает
- ✅ **РЕШЕНО:** Настроен автоматический перезапуск после критических разрешений (`PermissionRestartIntegration`)
- ✅ **РЕШЕНО:** Порядок инициализации обновлен и соответствует реальной архитектуре
- ✅ **РЕШЕНО:** Серверная генерация приветствия вместо локальных MP3 файлов
- ✅ **РЕШЕНО:** Голосовые уведомления об обновлениях (`UpdateNotificationIntegration`) интегрированы
- ✅ **РЕШЕНО:** 20 интеграций реализованы (проверка: `ls integration/integrations/*.py | wc -l` должно быть 20)
- ✅ **РЕШЕНО:** Миграция session_id на ApplicationStateManager - устранено дублирование состояния
...
- ✅ **РЕШЕНО:** Исправлена проблема ложных прерываний в ProcessingWorkflow
...
### 🆕 **НОВЫЕ КОМПОНЕНТЫ (ОКТЯБРЬ 2025):**
- **FirstRunPermissionsIntegration:** Управление разрешениями при первом запуске
  - Флаг: `~/Library/Application Support/Nexy/permissions_first_run_completed.flag`
  - Последовательность: Микрофон → Accessibility → Screen Capture
  - Паузы: 7 секунд между диалогами для комфортного UX
  - Блокирующая логика: приложение ждет завершения перед запуском остальных

- **Модули first_run:**
  - `modules/permissions/first_run/activator.py` - триггер системных диалогов
  - `modules/permissions/first_run/status_checker.py` - проверка без показа диалогов
  - События: `permissions.first_run_started/completed/failed`

- **PermissionRestartIntegration + modules.permission_restart:**
  - Отслеживание `permissions.changed/status_checked` для критических разрешений (микрофон, accessibility, input monitoring, screen capture)
  - Планировщик перезапуска с задержкой, проверкой активных сессий и статуса обновлений
  - Публикует события `permission_restart.scheduled` и `permission_restart.executing`, использует существующий механизм `relaunch_app()`

- **UpdateNotificationIntegration:**
  - Подписка на `updater.update_*` и прогресс-события
  - Озвучка через `speech.playback.request`, сигналы через `signal.play`
  - Настраиваемые интервалы и голос в `update_notification` конфигурации
...
- Добавление нового модуля:
  1) Создать модуль в `modules/<name>` без знания EventBus
  2) Создать интеграцию в `integration/integrations/<name>_integration.py`
...
Последнее обновление: 3 ноября 2025 - исправлен race condition в обработке событий разрешений (критичные подписки настраиваются до инициализации интеграций), добавлен ADR_PERMISSION_EVENT_RACE_FIX.md.

---

## 12) Каталог модулей

### 🖥️ **КЛИЕНТСКИЕ МОДУЛИ (modules/) - 20 МОДУЛЕЙ**

_macOS автоматически управляет активными аудиоустройствами, поэтому отдельный менеджер больше не требуется._
- `browser_use` — автоматизация браузера на клиенте: локальная установка Chromium, выполнение задач через Playwright, интеграция с LangChain.
...
### 🖥️ **КЛИЕНТСКИЕ ИНТЕГРАЦИИ (integration/integrations) - 19 ИЗ 19**
...
- `browser_use_integration.py`
  - Назначение: выполнение задач автоматизации браузера на клиенте.
  - Подписки: `browser.use.request`, `browser.close.request`.
  - Публикует: `browser.started`, `browser.progress`, `browser.completed`, `browser.failed`.
  - Логика: управляет локальным BrowserUseModule, устанавливает Chromium, выполняет задачи.

**✅ ДОБАВЛЕНО:** `first_run_permissions_integration.py` — система первого запуска с умными паузами, и `permission_restart_integration.py` — автоматический перезапуск после выдачи критических разрешений.
...
**Последнее обновление:** 24 октября 2025 - добавлена PermissionRestartIntegration, актуализирована архитектура FirstRunPermissions, обновлен порядок инициализации, готовность к видео-инструкции.
